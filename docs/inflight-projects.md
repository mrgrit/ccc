...
### P27. 6v6 Windows 엔드포인트 + 4과목 Windows 반영 [STATUS: ✅ COMPLETE — 4 과목 (secuops-easy/secuops/soc/attack) 모두 push (2026-05-28)]
...
**DoD**:
- [x] Win 기반: tiny11 dmz 10.20.32.60, agent Active, Sysmon/Security/SCA SIEM 도달 실측
- [x] secuops-easy 6주 Windows 반영 (한 주차씩 실측)
- [x] **secuops 15주 Windows 반영** — W02+W03 통합으로 방화벽 1주 절약 → 새 W03 = Windows 정식 주차
- [x] **soc 15주 Windows 반영** — 15개 lecture 위빙 + W11/W15 lab Windows step
- [x] **attack 15주 Windows 반영** — 15개 lecture 위빙(Red 자기 인식) + W11/W15 lab Windows step
...
**진행 (2026-05-28 심야~새벽)**:
- ✅ Windows 기반: tiny11 dmz 10.20.32.60, agent Active, Sysmon/Security/SCA → SIEM 도달 (실측).
- ✅ secuops-easy 6주 **강의 전수 Windows weave + push**: W1(토폴로지 노드+책상 비유) 6fbb3a8b /
  W2(방화벽 가시성 한계+EDR) 9d9cc4b8 / W3(IPS-EDR 상관) e59f740f / W4(엔드포인트+Sysmon) c10af3a8 /
  W5(직원PC 웹+WAF+EDR) 78701add / W6(capstone 4계층) 09ac8af4.
- 🔄 종합 oem 재설치(Sysmon+Wazuh+OpenSSH+hosts) — SSH 자동 검증 enabler. ISO 재다운로드 느려 OOBE 중.
- ⏳ secuops-easy W5/W6 **랩**(victim 라이브 브라우징→WAF audit/Sysmon 관찰) — SSH 뜨면 즉시 작성·검증·push.
- 다음 (순차): secuops 15주(방화벽 2→1주 통합 + Windows 정식) → soc → attack.

**Next**: SSH 대기(45분 캡 bg). 뜨면 W5/W6 랩 → push → secuops 진행. [[project_secuops_easy]] [[feedback_concise_no_tmi]]

**최종 진행 (2026-05-28 후반)**:
- ✅ secuops W02+W03 통합 (방화벽 운영자 GUI + 침해대응 7) + 새 W03 (Windows 엔드포인트 정식, victim+analyst 페르소나, Sysmon EID 1/3/22/4625 R/B/P 5건) — commits 51e67e97, b62b5d85. nft_edu_gui /api/cleanup 추가(mrgrit/nft_edu_gui 52422ea).
- ✅ secuops W4-W15 lecture 전 주차 Windows 위빙 (~30-50행/주차, 각 주차 고유 내용 — IPS+Sysmon EID 3, SIGMA Windows 룰, ATT&CK 매핑, APT 5단계 timeline 등) — commit 80c593ae.
- ✅ soc W1-W15 lecture 전 주차 Windows 위빙 (분석가 시각, baseline/4 패턴/UEBA/IR SOP 사용자 인터뷰 단계 등) + W11/W15 lab Windows step (EncodedCommand·iwr) — commit 4333743a, 0ec1107f.
- ✅ attack W1-W15 lecture 전 주차 Windows 위빙 (Red 자기 인식 — 흔적 그리기) + W11/W15 lab Windows step (whoami /priv, PTES Stage 5 Win victim) — commit dbef1d42, 257d300a.
- 모든 신규 lab step 0.105 6v6 실측 OK. SSH 일시 lockout 회피 패턴(net use bad creds) 발견.
