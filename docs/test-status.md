...
> 마지막 업데이트: 2026-04-20 03:18

## 요약

- **전체 3,090 케이스** (2,734 기존 + **356 신규 C19·C20**) · 2,734 테스트 수행 (88.5%)
- **엄격 Pass 1121 / 3,090 = 36.3%** (수정 전 997 → +124)
- Fail 839, QA-fallback 631, Error 188, No-exec 23, Untested 288

## C19 agent-ir-ai 누적 결과 (68건)

| verdict | 건수 | 비율 | 기존 재테스트 대비 |
|---------|------|------|---------------------|
| pass | 26 | **38%** | 8.2% 대비 **5배** |
| fail | 33 | 49% | - |
| qa_fb | 9 | 13% | 61% 대비 1/5 |

버그 없는 generator + bastion_prompt=instruction 기본 규칙으로 **처음부터 구조 건강**. 기존 과목은 재테스트·패치로도 pass 전환율 8%에 머물렀지만 C19는 38%. **구조적 문제 vs 프롬프트 문제**의 분리가 명확히 드러남.
