...
> 마지막 업데이트: 2026-04-19 11:15

## 요약

- **전체 3,090 케이스** (2,734 기존 + **356 신규 C19·C20**) · 2,734 테스트 수행 (88.5%)
- **엄격 Pass 992 / 3,090 = 32.1%** (988→+4)
- Fail 571, QA-fallback 976, Error 188, No-exec 7, Untested 356 (C19/C20 전체)

## 재테스트 루프 중간 결과 (78/1009 = 7.7%)

| 원래 qa_fb | 재테스트 후 | 건수 | 비율 | 해석 |
|-----------|-------------|------|------|------|
| → pass | 5 | **6.4%** ↓ | 복잡 과제에서 pass 전환율 하락 |
| → fail | 24 | 30.8% | 실행 유도, verify 실패 |
| → qa_fallback | 45 | **57.7%** ↑ | Path B 비율이 복잡 과제에서 증가 |

- 앞서 43건에서 pass 12% / 실행전환 53% → 78건에서 pass 6% / 실행전환 37%로 하락
- 지금 attack-ai w10-w14 구간 처리 중. **후반부 복잡 공격 스텝일수록 Path B(bastion_prompt 변형 버그) 비중이 증가**함을 확인
- → w19 패턴 override는 **단순 실행형 스텝**에는 효과적, **추상적·복합적 스텝**은 여전히 QA로 흐름
