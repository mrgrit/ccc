...
> 마지막 업데이트: 2026-04-19 10:44

## 요약

- **전체 3,090 케이스** (2,734 기존 + **356 신규 C19·C20**) · 2,734 테스트 수행 (88.5%)
- **엄격 Pass 988 / 3,090 = 32.0%** (983→+5, 재테스트 43건 중 5건 pass 전환)
- Fail 565, QA-fallback 986, Error 188, No-exec 7, Untested 356 (C19/C20 전체)

## 재테스트 루프 중간 결과 (43/1009 = 4.3%)

| 원래 qa_fb | 재테스트 후 | 건수 | 비율 | 해석 |
|-----------|-------------|------|------|------|
| → pass | 5 | 11.6% | w19 패턴이 실행 유도 + verify도 통과 |
| → fail | 18 | 41.9% | 실행은 유도했으나 verify 실패 (skill=shell) |
| → qa_fallback | 20 | 46.5% | 여전히 qa (Path B — bastion_prompt 버그 추정) |

→ 개선 효과는 부분적: **qa_fb의 53% 이상을 실행 경로로 전환**, 그 중 12%는 pass까지 달성
