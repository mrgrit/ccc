...
> 사이버보안 교육 훈련 플랫폼 — 17개 교과목, 510개 실습, AI 에이전트(Bastion), 공방전, 블록체인 성과 관리
...
- 15주 × 17과목 = **255주** 교안 (Markdown, 파일 참조 가이드 포함)
- 15주 × 17과목 × 2(Non-AI + AI) = **510개** 실습 (YAML)
...
- **QLoRA 파인튜닝**: unsloth + PEFT + trl로 30분 내 커스텀 모델 생성 가능
- 데이터셋: `finetune/dataset/` (30건, 13개 카테고리)
...
| 항목 | 값 |
|------|-----|
| 총 테스트 케이스 | 2,734개 (17개 과정, 15주차) |
| 완료 | 2,369 (87%) |
| Pass | 813 (34%) |
| 고유 보안 과제 유형 | 162개 (25개 대분류) |
| Bastion 패치 | 22회 (테스트 중 발견·수정) |

### 과정별 성공률

| 과정 | Pass% | 비고 |
|------|-------|------|
| 보안 솔루션 운영 (secops) | **71%** | 15주 교안 맞춤 재생성 |
| SOC 심화 (soc-adv) | **64%** | |
| SOC 기초 (soc) | **59%** | |
| 컴플라이언스 | **48%** | |
| 클라우드/컨테이너 | **43%** | |
