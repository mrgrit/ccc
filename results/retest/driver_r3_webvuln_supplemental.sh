...
Q=results/retest/queue_r3_webvuln_supplemental.tsv
C=results/retest/cursor_r3_webvuln_supplemental.txt
L=results/retest/run_r3_webvuln_supplemental.log
H="http://192.168.0.103:8003/health"

[ -f "$C" ] || echo 0 > "$C"
cur=$(cat "$C")
total=$(wc -l < "$Q")
echo "=== R3-webvuln-supplemental start $(date -Iseconds) cursor=$cur total=$total pid=$$ ===" >> "$L"
...
while IFS=$'\t' read -r course wk order; do
  ...
  echo "[$(date -Iseconds)] WV #${idx}/${total} ${course} ${wk} ${order}" >> "$L"
  timeout 600 python3 -u scripts/test_step.py "${course}" "${wk#w}" "${order#o}" >> "$L" 2>&1
  echo "$idx" > "$C"
  sleep 2
done < "$Q"
echo "=== R3-webvuln-supplemental DONE $(date -Iseconds) ===" >> "$L"
