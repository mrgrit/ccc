#!/usr/bin/env bash
# R4 fixture sample30 driver — task 시작 직전 fixture inject + retest.
...
cur=$(cat "$C")
total=$(wc -l < "$Q")
echo "=== R4 fixture sample30 driver start $(date -Iseconds) cursor=$cur total=$total pid=$$ ===" >> "$L"
...
while IFS=$'\t' read -r course_path order; do
  ...
  ts=$(date -Iseconds)
  echo "[$ts] R4-FIXTURE #$idx/$total $course w$wk o$order" >> "$L"
  ...
  timeout 600 python3 scripts/test_step.py "$course" "$wk" "$order" --no-augment < /dev/null >> "$L" 2>&1
  rc=$?
...
echo "=== R4 fixture sample30 DONE $(date -Iseconds) ===" >> "$L"
