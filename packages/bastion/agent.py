...
class BastionAgent:
    ...
    def _build_react_system_prompt(self) -> str:
        ...
        return (
            ...
            "\n"
            "## ★ Attack-mode skill 가드레일 (recon vs exploit 분리)\n"
            "  · 'WAF 우회/SQLi 변형/XSS 변형/페이로드 설계' → shell (curl 로 실제 페이로드 POST)\n"
            "  · 'Suricata/Snort/IDS 룰 분석·우회'           → qa (의미 분석) 또는 shell (cat/grep rules.conf)\n"
            "  · '익스플로잇 PoC/RCE 시도/CVE-NNNN exploitation' → shell (실제 명령) 또는 attack_simulate\n"
            "  · 'NFS no_root_squash/SUID/setcap/sudo NOPASSWD/권한 상승' → shell (showmount/mount/find -perm) 또는 qa\n"
            "  · 'nmap 옵션·시그니처 변경/스캔 회피'         → shell (nmap -sS -f --data-length 등)\n"
            "  · 'WPA2 핸드셰이크/EAPOL/aircrack/hashcat'    → shell (aircrack-ng/hashcat 직접)\n"
            "  · '결과 정리/요약/문서화/분석 리포트'         → qa (자연어 종합), shell 결과 가공도 OK\n"
            "  · ★ probe_all/probe_host 는 recon 전용 (포트/배너/정찰). 익스플로잇·페이로드·설명·분석·우회 요청에 호출 금지.\n"
            "  · ★ probe 결과는 항상 인프라 status JSON 만 반환. 위 카테고리 prompt 에 부적합.\n"
            "\n"
            "## 도구 호출 예시 (★ 첫 turn 패턴)\n"
