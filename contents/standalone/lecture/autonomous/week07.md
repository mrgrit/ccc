...
> **접속/호출:** TUI `./dev.sh bastion`, API `http://192.168.0.110:9200`
...
| step | 학습 항목 | OSS 도구 |
|------|----------|---------|
| s1 | Shuffle workflow 기초 | **Shuffle** (modern OSS SOAR) |
| s2 | TheHive 사고 케이스 | **TheHive 5** + Cortex |
| s3 | Webhook 통합 | Wazuh integration |
| s4 | n8n 대안 | **n8n** |
| s5 | StackStorm 대안 | **StackStorm** |
| s6 | Cortex analyzer | Cortex 100+ analyzer |
| s7 | Custom action | Python script |
| s8 | 통합 IR 자동화 | 4 도구 통합 |

### 학생 환경 준비

```bash
# Shuffle (가장 modern OSS SOAR)
docker run -d -p 3001:3001 ghcr.io/shuffle/shuffle-frontend:latest
docker run -d -p 5001:5001 ghcr.io/shuffle/shuffle-backend:latest

# TheHive 5 + Cortex
docker run -d --name thehive -p 9000:9000 strangebee/thehive:5.2
docker run -d --name cortex -p 9001:9001 thehiveproject/cortex:3.1.7

# n8n (workflow automation)
docker run -d -p 5678:5678 docker.n8n.io/n8nio/n8n

# StackStorm (가장 성숙)
curl -sSL https://stackstorm.com/packages/install.sh | bash -s -- --user=admin --password=Pa\$\$w0rd
```

### 핵심 — Shuffle workflow 작성

```yaml
# Shuffle 의 workflow 는 web UI 에서 drag-drop 으로 작성
# 예: Wazuh alert → AbuseIPDB → 자동 차단 + 알림

triggers:
  - id: wazuh_webhook
    type: webhook
    url: http://shuffle:5001/api/v1/hooks/wazuh

actions:
  - id: 1_extract_ip
    name: Extract source IP
    type: jinja
    expression: "{{ $exec.payload.data.srcip }}"
  
  - id: 2_check_abuseipdb
    name: AbuseIPDB lookup
    type: http
    method: GET
    url: "https://api.abuseipdb.com/api/v2/check?ipAddress={{ $1_extract_ip.output }}"
    headers:
  ...
  - id: 3_decide
    name: 차단 결정
    type: condition
    if: "{{ $2_check_abuseipdb.output.data.abuseConfidenceScore > 50 }}"
    then: 4_block_ip
    else: 5_log_only
  ...
  - id: 4_block_ip
    ...
    type: ssh
    host: secu-vm
    command: "nft add element inet filter blocked '{ {{ $1_extract_ip.output }} }'"
  
  - id: 4b_create_thehive
    name: TheHive case 생성
    type: http
    method: POST
    url: http://thehive:9000/api/case
    headers: { Authorization: "Bearer {{ $env.THEHIVE_TOKEN }}" }
    body: |
      {"title":"Auto-block {{ $1_extract_ip.output }}","severity":3,"tlp":2}
  ...
  - id: 4c_slack
    ...
    method: POST
    body: |
      {"text":"🛡 Auto-blocked {{ $1_extract_ip.output }} (AbuseIPDB: {{ $2_check_abuseipdb.output.data.abuseConfidenceScore }}%)"}
```

### TheHive + Cortex (분석 자동화)

```python
from thehive4py.api import TheHiveApi
from thehive4py.models import Case, CaseTask, Alert, AlertArtifact
...
alert = Alert(
    ...
    description="Wazuh detected SQLi pattern",
    severity=3,
    tlp=2,
    type="external",
    source="wazuh",
    ...
    artifacts=[
        ...
        AlertArtifact(dataType="url", data="http://target/login.php"),
    ]
)
response = api.create_alert(alert)

# 2) Case 자동 promotion
case_id = api.promote_alert_to_case(response.json()['id']).json()['id']

# 3) Cortex analyzer 자동 실행
# Web UI 에서 "Run Analyzers" → AbuseIPDB / VirusTotal / Shodan / OpenCTI
# 각 IoC 자동 enrichment

# 4) Responder (자동 차단)
# Web UI → "Run Responder" → "Block-IP-on-Firewall"
```
...
# Drag-drop workflow:
# Webhook → Wazuh data → AbuseIPDB → IF (score > 50) → SSH (nft block) → Slack
```

### StackStorm (Enterprise grade)

```yaml
# /opt/stackstorm/packs/security/rules/wazuh-block.yaml
---
name: wazuh-block-rule
trigger:
  type: webhook
  parameters:
    url: wazuh-webhook
...
Shuffle webhook trigger
  ↓
[Parallel branches]
  ├─ AbuseIPDB / VirusTotal / OpenCTI lookup
  ├─ TheHive case 자동 생성 + Cortex analyzer
  ├─ OPA decision (자동 차단 vs manual review)
  │    ↓ (auto-block)
  ├─ Ansible playbook (nft / firewalld 차단)
  ├─ K8s NetworkPolicy (격리)
  ├─ Slack / PagerDuty 알림
  └─ Langfuse audit log
```

학생은 본 7주차에서 **Shuffle + TheHive + Cortex + n8n + StackStorm** 5 SOAR 도구로 자동 IR 의 4 단계 (alert → 분석 → 결정 → 실행) 사이클을 익힌다.
