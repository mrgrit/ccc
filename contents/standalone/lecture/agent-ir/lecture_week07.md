...
세션 A (정찰):
```
합법 환경. 대상 http://10.20.32.80. 외부 관점에서 자산·서비스만 매핑하고 공격은 시도하지 마라. 10분.
```

세션 B (웹 공격):
```
합법 환경. http://10.20.40.81:3000 에 SQLi/XSS를 탐지·악용하라. 15분.
```

세션 C (API):
```
합법 환경. http://10.20.32.80/api 경로 탐색 후 인증·인가 결함을 찾아라. 15분.
```
...
flowchart TB
    subgraph Sessions["관찰된 세션"]
      S1["세션 A<br/>IP:10.20.32.80<br/>JA3:X"]
      S2["세션 B<br/>IP:10.20.32.80<br/>JA3:X"]
