...
먼저 전체 지도를 봅시다. 우리 실습망은 보안 등급이 다른 **4개의 구역** 으로 나뉩니다.
...
> **핵심:** 바깥(ext) → 통로(pipe) → 공개구역(dmz) → 내부(int). 안으로 갈수록 신뢰도가 높고,
> 각 경계마다 보안장비가 문지기로 서 있습니다.
...
- 6v6 망은 **ext → pipe → dmz → int** 4구역으로, 안으로 갈수록 중요하다.
- **방화벽**(ext↔pipe 경계)은 IP·포트를, **IPS**(pipe↔dmz 경계)는 내용물을, **WAF**(dmz 의
  웹서버)는 HTTP 의미를 검사한다.
