...
```mermaid
graph LR
    A[공격자<br/>sqlmap UA + SQLi] --> FW[방화벽<br/>카운터/conntrack]
    FW --> IPS[IPS<br/>eve.json alert]
    IPS --> WAF[WAF<br/>audit 403 차단]
    WIN[사용자 PC<br/>Windows Sysmon/EDR]
    FW -.SIEM.-> S[(SIEM<br/>Wazuh)]
    IPS -.SIEM.-> S
    WAF -.SIEM.-> S
    WIN -.SIEM.-> S
...
3. **WAF** — 웹서버에서 SQLi/스캐너 룰에 걸려 **audit 로그에 403 차단** 을 기록한다.
4. **엔드포인트(Windows 사용자 PC)** — 이 공격이 직원 PC 를 노렸거나(피싱·드라이브바이) 직원이
   당했다면, **Sysmon** 이 PC 에서 일어난 일(프로세스 실행·네트워크 연결)을 기록해 **Wazuh 에이전트**
   로 SIEM 에 보낸다. 장비(네트워크/HTTP)가 못 보는 **호스트 내부** 를 채운다.
