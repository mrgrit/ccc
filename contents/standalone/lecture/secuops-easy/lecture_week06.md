# Week 06 — WAF 침해대응 30 + 종합: 세 장비가 함께 막는다
...
대소문자 우회는 **t:lowercase**. WAF 의 진짜 힘은 이 변환에 있습니다.
...
| 차단 | (af-packet IDS 모드라 경보 위주) | **deny 로 즉시 403** |

**왜 둘 다 필요한가?** IPS 는 웹뿐 아니라 모든 프로토콜(DNS, TCP 스캔 등)을 보지만 HTTP 의 깊은
의미는 모릅니다. WAF 는 HTTP 만 보지만 파라미터·인코딩·본문까지 정밀하게 이해합니다. **겹치는
부분(웹)은 이중으로 막고, 안 겹치는 부분은 서로 보완** 합니다. 이것이 심층 방어입니다.

---

## 3. 종합 capstone — 한 공격, 네 겹(장비 3 + 엔드포인트)

이제 6주의 핵심을 한자리에 모읍니다. 공격 하나를 보내고, **장비 세 곳 + 엔드포인트 한 곳**, 총
네 곳에서 각각의 흔적을 찾습니다.

```mermaid
graph LR
    A[공격자<br/>ext 10.20.30.202<br/>sqlmap UA + SQLi] --> FW[방화벽<br/>카운터/conntrack]
    FW --> IPS[IPS<br/>eve.json alert]
    IPS --> WAF[WAF<br/>audit 403 차단]
    subgraph USER["user 구역 10.20.33.0/24"]
        WIN[사용자 PC<br/>Windows 10.20.33.60<br/>Sysmon/EDR]
    end
    WIN -- 사내 웹 사용 --> IPS
...
2. **IPS** — 통과한 트래픽에서 sqlmap UA·UNION 패턴을 발견해 **eve.json 에 alert** 를 남긴다.
   직원 PC(user 구역) 가 사내 웹에 접속한 트래픽도 IPS 의 user 다리(eth2)를 거쳐 검사된다.
3. **WAF** — 웹서버에서 SQLi/스캐너 룰에 걸려 **audit 로그에 403 차단** 을 기록한다. 외부 공격자
   요청과 직원 PC 의 악성 클릭 모두 동일한 audit 형식으로 남는다 (`client_ip` 로 구분).
4. **엔드포인트(Windows 사용자 PC, user 구역)** — 이 공격이 직원 PC 를 노렸거나(피싱·드라이브바이)
   직원이 당했다면, **Sysmon** 이 PC 에서 일어난 일(프로세스 실행·네트워크 연결)을 기록해
   **Wazuh 에이전트** 로 SIEM 에 보낸다. 장비(네트워크/HTTP)가 못 보는 **호스트 내부** 를 채운다.

네 흔적이 모두 **SIEM(Wazuh, 10.20.32.100)** 으로 모이므로, 운영자(분석가 PC)는 한 화면에서 "이 공격이
어디서 통과하고 어디서 막혔으며, 엔드포인트에선 무슨 일이 있었는지" 를 시간순으로 봅니다. **한 겹이
놓쳐도 다음 겹이 잡고, 네트워크가 못 본 건 엔드포인트가 보며, 모든 겹이 기록을 남긴다** — 이것이
우리가 6주 동안 배운 심층 방어의 완성입니다.
...
- 세 장비를 **SIEM 으로 통합** 해 한 화면에서 관제한다.

이것은 보안 운영(SecOps) 엔지니어의 가장 기본적이고 핵심적인 역량입니다. 더 깊이 배우고 싶다면
정규 과정의 **보안 솔루션 운영(secuops, 15주)** 과 **보안 운영 센터(SOC)** 과목으로 이어집니다.
...
2. **transform 비교**: 같은 XSS(`%3Cscript%3E`)를 `t:urlDecodeUni` 있는 룰/없는 룰로 막아 보고,
   결과 차이를 캡처해 설명하세요.
3. **deny vs pass**: 한 시나리오를 pass(탐지)로 먼저 넣고 audit 로그를 본 뒤, deny 로 바꿔 403 을
   확인하는 과정을 기록하세요.
4. **종합 capstone**: sqlmap + SQLi 공격을 한 번 보내고, 방화벽 카운터(또는 conntrack) / IPS
   eve.json alert / WAF audit 403 **세 곳의 흔적** 을 각각 캡처해, 한 공격이 세 장비에 어떻게
   다르게 보였는지 한 페이지로 정리하세요. (이번 특강의 최종 보고서)
