...
웹 사용** 도 봅니다. W1 의 **Windows 사용자 PC(10.20.32.60)** 가 사내 웹(juice/dvwa 등)에 접속하면,
그 요청은 같은 dmz 의 웹서버 = **WAF(ModSecurity)** 를 그대로 통과합니다. 그래서 직원 PC 의 모든
웹 요청이 `modsec_audit.log` 에 **출발지 10.20.32.60** 으로 남습니다.
...
둘 다 같은 SIEM 으로 모이므로, 분석가는 "**그 직원 PC 가 어떤 웹 요청을 했고(WAF), 그 뒤 PC 에서
무슨 일이 일어났나(Sysmon)**" 를 한 타임라인으로 봅니다. 즉 WAF 는 외부 공격 차단뿐 아니라 **내부
직원이 위험에 노출되는 순간** 을 포착하는 창(窓)이기도 합니다. (W6 종합에서 이 흐름을 직접 봅니다.)
