...
| 항목 | 내용 예시 |
|------|---------|
| 점검 대상 | web 서버 (10.20.32.80), 포트 80/443/3000 |
...
> http --print=Hh GET http://juice.6v6.lab/
>
> # ab (Apache Bench, 응답 시간·동시 요청 측정)
> ab -n 10 -c 1 http://juice.6v6.lab/   # 10 회 요청 평균
> ```
...
> whatweb -v http://juice.6v6.lab        # -v 상세 모드
>
> # httpie — 헤더만 보기
> http --print=h HEAD http://juice.6v6.lab/
>
> # nuclei (취약 헤더 자동 점검 — 5주차에서 본격 사용)
> nuclei -t http/misconfiguration/http-missing-security-headers.yaml -u http://juice.6v6.lab
> ```
>
...
curl -s http://juice.6v6.lab/ | head -30

# JavaScript 프레임워크 확인
curl -s http://juice.6v6.lab/ | grep -i "angular\|react\|vue" | head -5
```
...
```bash
# 상품 목록 API
curl -s http://juice.6v6.lab/api/Products | python3 -m json.tool | head -30
```
...
```bash
# 검색 API 테스트
curl -s "http://juice.6v6.lab/rest/products/search?q=apple" | python3 -m json.tool | head -20

# 빈 검색어
curl -s "http://juice.6v6.lab/rest/products/search?q=" | python3 -m json.tool | head -20
```
...
for path in "/administration" "/admin" "/api-docs" "/ftp" "/assets/public" "/robots.txt" "/security.txt"; do  # 반복문 시작
  status=$(curl -s -o /dev/null -w "%{http_code}" "http://juice.6v6.lab$path")
...
> gobuster dir -u http://juice.6v6.lab -w /usr/share/wordlists/dirb/common.txt -t 20
>
> # ffuf — 단일 wordlist 퍼징, JSON 출력 가능
> ffuf -w /usr/share/wordlists/dirb/common.txt -u http://juice.6v6.lab/FUZZ -mc 200,301,403
>
> # dirb — 가장 단순, 초보자에 추천
> sudo apt install dirb
> dirb http://juice.6v6.lab /usr/share/wordlists/dirb/common.txt
> ```
>
...
```bash
# FTP 공개 디렉토리 확인
curl -s http://juice.6v6.lab/ftp/ | python3 -m json.tool 2>/dev/null || curl -s http://juice.6v6.lab/ftp/
```
...
```bash
# 검색 엔진 크롤링 제어 파일
curl -s http://juice.6v6.lab/robots.txt
```
...
curl -v http://juice.6v6.lab/ 2>&1 | head -40
```

**출력 분석**:
```
> GET / HTTP/1.1           ← 요청 라인 (메서드, 경로, 프로토콜)
> Host: juice.6v6.lab     ← 요청 헤더 (HAProxy 의 host-header 기반 라우팅 키)
...
```bash
# 로그인 후 쿠키 확인
curl -s -c - http://juice.6v6.lab/ | head -5
```
...
# 서버 응답 헤더에서 정보 추출
echo "[1] 서버 헤더 정보:"
curl -sI http://juice.6v6.lab/ | grep -iE "server|x-powered|x-frame|content-security"
...
## 9. 점검 활동의 Red / Blue / Purple 관점 (15분)

웹 취약점 점검을 단순히 "찾는 활동" 으로만 보면 절반만 보는 셈이다. 실제 점검은
**Red(공격자 시점) / Blue(방어자 시점) / Purple(둘의 협업)** 세 축으로 동시에 굴러간다.
이 과목 의 모든 주차 lab 은 이 R/B/P 패턴 으로 구성된다.

### 9.1 R/B/P 의 의미

| 색 | 입장 | 활동 |
|----|------|------|
| 🔴 **Red** | 공격자 / 점검자 | 도구로 취약점 탐색 (curl/nmap/whatweb/sqlmap/burp). 발견된 issue 정리. |
| 🔵 **Blue** | 방어자 / 운영팀 | Red 활동이 ModSec audit log / Suricata alert / Wazuh dashboard 에서 어떻게 보이는지 관측. |
| 🟣 **Purple** | 협업 / 룰 강화 | Red 가 우회한 부분을 Blue 의 룰로 보완. 또는 Blue 가 over-block 한 부분을 Red 사례로 완화. |

### 9.2 6v6 인프라가 R/B/P 모두 보여주는 이유

```
[Red 점검자: attacker]                     [Blue 운영자: siem dashboard]
        │                                          ▲
        │ 1. curl/nmap/sqlmap 발사                   │ 4. Wazuh alert 시간순 확인
        ▼                                          │
  [fw HAProxy 10.20.30.1] ── 2. nftables log ──>  [siem Wazuh Manager]
        │                                          ▲
        │ 3. Suricata sniff                         │ 3. eve.json alert
        ▼                                          │
  [ips 10.20.31.2 (pipe)] ── alert ─────────────>  │
        │                                          │
        ▼                                          │
  [web 10.20.32.80 + ModSec] ── audit.log ──────>  │
        │
        ▼
  [juiceshop / dvwa 등]
```

학생은 매 주차 같은 점검 활동을 **두 화면 (attacker SSH + Wazuh dashboard)** 으로
동시에 보면서 Red/Blue 의 시각 차이를 학습한다.

### 9.3 본 주차 W01 의 R/B/P 시나리오

| 단계 | 입장 | 활동 |
|------|------|------|
| 🔴 Red | 점검자 | curl 로 /api/Products + /api-docs 조회. whatweb 으로 stack 식별. |
| 🔵 Blue | 운영자 | Wazuh dashboard 의 `rule.groups:web` 검색 → 점검자의 정상 GET 요청 흔적 확인. ModSec 의 audit log 에 X-Recruiting 헤더 노출 등 보안 헤더 결손 alert. |
| 🟣 Purple | 협업 | `X-Powered-By: Express` 노출 제거 + `Content-Security-Policy` 헤더 추가 등 ModSec 룰 보강 권고. |

**중요**: W01 은 "정찰" 단계라 공격 자체가 없다. Blue 도 정상 트래픽으로 본다. 다음
주차 (W02 도구 환경 → W03 정보수집 심화 → ... → W05 SQLi 부터) 부터 Red 의 공격이
실질 시작 되며, 그때 부터 Blue 의 검출 alert 가 본격 발생한다.
...
- [ ] JuiceShop (http://juice.6v6.lab) 접속 성공
...
> **JuiceShop URL:** `http://juice.6v6.lab`

1. 브라우저에서 `http://juice.6v6.lab` 접속
2. 메인 페이지가 로드되면 쇼핑몰 형태의 웹 애플리케이션 확인
3. 우측 상단 **Account → Login** 클릭하여 로그인 페이지 구조 확인 (아직 로그인하지 않음)
4. 주소창에 `http://juice.6v6.lab/#/score-board` 직접 입력
5. Score Board 페이지에서:
   - 난이도별 챌린지 분류 확인 (1~6성)
