...
> http --print=Hh GET http://10.20.40.81:3000/
>
> # ab (Apache Bench, 응답 시간·동시 요청 측정)
> ab -n 10 -c 1 http://10.20.40.81:3000/   # 10 회 요청 평균
> ```
...
> whatweb -v http://10.20.40.81:3000        # -v 상세 모드
>
> # httpie — 헤더만 보기
> http --print=h HEAD http://10.20.40.81:3000/
>
> # nuclei (취약 헤더 자동 점검 — 5주차에서 본격 사용)
> nuclei -t http/misconfiguration/http-missing-security-headers.yaml -u http://10.20.40.81:3000
> ```
>
...
curl -s http://10.20.40.81:3000/ | head -30

# JavaScript 프레임워크 확인
curl -s http://10.20.40.81:3000/ | grep -i "angular\|react\|vue" | head -5
```
...
```bash
# 상품 목록 API
curl -s http://10.20.40.81:3000/api/Products | python3 -m json.tool | head -30
```
...
```bash
# 사용자 등록 페이지 확인
curl -s -o /dev/null -w "Status: %{http_code}\n" http://10.20.40.81:3000/#/register
...
```bash
# 검색 API 테스트
curl -s "http://10.20.40.81:3000/rest/products/search?q=apple" | python3 -m json.tool | head -20

# 빈 검색어
curl -s "http://10.20.40.81:3000/rest/products/search?q=" | python3 -m json.tool | head -20
```
...
for path in "/administration" "/admin" "/api-docs" "/ftp" "/assets/public" "/robots.txt" "/security.txt"; do  # 반복문 시작
  status=$(curl -s -o /dev/null -w "%{http_code}" "http://10.20.40.81:3000$path")
...
> gobuster dir -u http://10.20.40.81:3000 -w /usr/share/wordlists/dirb/common.txt -t 20
>
> # ffuf — 단일 wordlist 퍼징, JSON 출력 가능
> ffuf -w /usr/share/wordlists/dirb/common.txt -u http://10.20.40.81:3000/FUZZ -mc 200,301,403
>
> # dirb — 가장 단순, 초보자에 추천
...
```bash
# FTP 공개 디렉토리 확인
curl -s http://10.20.40.81:3000/ftp/ | python3 -m json.tool 2>/dev/null || curl -s http://10.20.40.81:3000/ftp/
```
...
curl -v http://10.20.40.81:3000/ 2>&1 | head -40
```

**출력 분석**:
```
> GET / HTTP/1.1           ← 요청 라인 (메서드, 경로, 프로토콜)
> Host: 10.20.40.81:3000   ← 요청 헤더
...
```bash
# 로그인 후 쿠키 확인
curl -s -c - http://10.20.40.81:3000/ | head -5
```
...
# 서버 응답 헤더에서 정보 추출
echo "[1] 서버 헤더 정보:"
curl -sI http://10.20.40.81:3000/ | grep -iE "server|x-powered|x-frame|content-security"
...
- [ ] JuiceShop (http://10.20.40.81:3000) 접속 성공
...
> **JuiceShop URL:** `http://10.20.40.81:3000`

1. 브라우저에서 `http://10.20.40.81:3000` 접속
2. 메인 페이지가 로드되면 쇼핑몰 형태의 웹 애플리케이션 확인
3. 우측 상단 **Account → Login** 클릭하여 로그인 페이지 구조 확인 (아직 로그인하지 않음)
4. 주소창에 `http://10.20.40.81:3000/#/score-board` 직접 입력
5. Score Board 페이지에서:
   - 난이도별 챌린지 분류 확인 (1~6성)
