...
> http --print=Hh GET http://juice.6v6.lab/   # 헤더+상태
>
...
echo 'docker run -t owasp/zap2docker-stable zap-baseline.py -t http://juice.6v6.lab -r /tmp/zap_baseline.html'

# 또는 daemon 가동 시 API 호출
echo 'curl "http://localhost:8090/JSON/spider/action/scan/?url=http://juice.6v6.lab&apikey=zap-api-key"'
```
...
> nuclei -u http://juice.6v6.lab -t http/misconfiguration/ -severity medium,high
> ```
...
nikto -h http://juice.6v6.lab -maxtime 60s 2>/dev/null | tail -20
```
...
```bash
# HTML 보고서 + CSV 동시 저장
nikto -h http://juice.6v6.lab -o /tmp/nikto_juice -Format htm -maxtime 60s 2>/dev/null > /dev/null
nikto -h http://juice.6v6.lab -o /tmp/nikto_juice.csv -Format csv -maxtime 60s 2>/dev/null > /dev/null
ls -l /tmp/nikto_juice* | awk '{print $5,$9}'

# 튜닝 옵션 (1=흥미로운 파일 / 2=잘못된 설정 / 3=정보 노출 / 4=XSS)
nikto -h http://juice.6v6.lab -Tuning 1234 -maxtime 30s 2>/dev/null | grep -c '^+'
```
...
> nuclei -u http://juice.6v6.lab -t http/misconfiguration/ -severity medium,high   # 5초 내 완료
> wapiti -u http://juice.6v6.lab -m all -f json -o /tmp/wapiti.json                # OWASP 카테고리 분류
> ```
...
sqlmap -u "http://juice.6v6.lab/rest/products/search?q=test" --batch --level=1 --risk=1 2>&1 | grep -E "Type:|Title:|Payload:|back-end DBMS|web application technology|^---" | head -15
```
...
```bash
# 응답 헤더만 (-I = HEAD)
curl -sI http://juice.6v6.lab | head -8
```
...
> http POST http://juice.6v6.lab/rest/user/login email=admin@juice-sh.op password=wrong
>
> # ffuf — 자격 brute-force (week05 권한 우회 학습 시)
> ffuf -u http://juice.6v6.lab/rest/user/login -X POST -H 'Content-Type: application/json' \
...
```bash
# 쿠키 저장
curl -c /tmp/cookies.txt http://juice.6v6.lab

# 저장된 쿠키로 요청
curl -b /tmp/cookies.txt http://juice.6v6.lab/rest/basket/1

# 쿠키 직접 지정
curl -b "token=abc123" http://juice.6v6.lab/rest/basket/1
```
...
```bash
# User-Agent 변경
curl -H "User-Agent: Mozilla/5.0 (Security Scanner)" http://juice.6v6.lab

# Referer 위조
curl -H "Referer: http://juice.6v6.lab/admin" http://juice.6v6.lab

# 여러 헤더 동시 설정
curl -H "Accept: application/json" \
     -H "Authorization: Bearer fake-token" \
     http://juice.6v6.lab/api/Products/1
```
...
```bash
# 응답 시간 4단계 분리
curl -o /dev/null -s -w "DNS: %{time_namelookup}s\nConnect: %{time_connect}s\nTTFB: %{time_starttransfer}s\nTotal: %{time_total}s\nSize: %{size_download} bytes\n" http://juice.6v6.lab/rest/products/search?q=apple
```
...
>
> ```bash
> ab -n 100 -c 10 http://juice.6v6.lab/             # Apache Bench, 100 요청 / 동시 10
> wrk -t4 -c50 -d10s http://juice.6v6.lab/          # wrk, 10초 부하 시험
> hey -n 100 -c 10 http://juice.6v6.lab/            # hey, Go binary 가벼움
> ```
...
# multipart form 업로드
curl -X POST http://juice.6v6.lab/file-upload \
...
for path in admin robots.txt .env .git/config sitemap.xml ftp api-docs swagger.json package.json; do
  code=$(curl -o /dev/null -s -w "%{http_code}" "http://juice.6v6.lab/$path")
  echo "$code - /$path"
done
```
...
> ffuf -u http://juice.6v6.lab/FUZZ -w /usr/share/seclists/Discovery/Web-Content/common.txt -mc 200,301,302 -t 50
>
> # gobuster — 동일 작업, 다른 wordlist 출력 형식
> gobuster dir -u http://juice.6v6.lab -w /usr/share/wordlists/dirb/common.txt -t 50 -x bak,old,zip
>
> # feroxbuster — Rust 작, 재귀 brute (서브 디렉토리 자동 탐색)
> feroxbuster -u http://juice.6v6.lab -w /usr/share/seclists/Discovery/Web-Content/common.txt
> ```
>
