...
if [ -n "$MAIN_JS" ]; then
  ...
  curl -s "http://juice.6v6.lab/$MAIN_JS" | grep -oE '"/api/[^"]*"|"/rest/[^"]*"' | sort -u | head -30  # silent 모드
fi
...
for ep in "${ENDPOINTS[@]}"; do
  read code size < <(curl -s -o /dev/null -w "%{http_code} %{size_download}" "http://juice.6v6.lab/$ep")
  if [ "$code" != "404" ]; then
    cat=""
    case "$code" in
...
test_methods() {
  ...
  for method in GET POST PUT DELETE PATCH OPTIONS; do
    code=$(curl -s -o /dev/null -w "%{http_code}" -X "$method" "http://juice.6v6.lab$ep")
    printf " %-6s" "$code"
  done
  echo
}
...
test_token() {
  ...
  read code < <(curl -s -o /tmp/whoami.json -w "%{http_code}" \
    http://juice.6v6.lab/rest/user/whoami -H "Authorization: $auth")
...
echo "=== 소스 코드 내 시크릿 검색 ==="
if [ -n "$MAIN_JS" ]; then
  curl -s "http://juice.6v6.lab/$MAIN_JS" | grep -oiE \
    "(api[_-]?key|secret|token|password|auth)['\"]?\s*[:=]\s*['\"][^'\"]{8,}" | head -10
fi

# HTML에서 시크릿 검색
curl -s http://juice.6v6.lab | grep -oiE \
...
for i in $(seq 1 50); do                               # 반복문 시작
  code=$(curl -s -o /dev/null -w "%{http_code}" "http://juice.6v6.lab/rest/products/search?q=test$i")
  if [ "$code" = "429" ]; then
...
>   -t http://juice.6v6.lab/api-docs -f openapi -r api_report.html
>
> # KiteRunner — REST API discovery
> kr scan http://juice.6v6.lab -w routes-large.kite
> ```
...
SWAGGER_URL="http://juice.6v6.lab/swagger.json"
...
curl -s http://juice.6v6.lab/api/Users/1 \
...
curl -s http://juice.6v6.lab/api/Products/1 | python3 -c "  # silent 모드
import sys, json
try:
    data = json.load(sys.stdin).get('data', {})
    print('포함된 필드:')
...
test_pagination "/api/Users (전체)"        "http://juice.6v6.lab/api/Users/"
test_pagination "/api/Products (전체)"     "http://juice.6v6.lab/api/Products/"
test_pagination "/api/Feedbacks (전체)"    "http://juice.6v6.lab/api/Feedbacks/"
test_pagination "/api/Challenges (전체)"   "http://juice.6v6.lab/api/Challenges/"
test_pagination "/api/SecurityQuestions"   "http://juice.6v6.lab/api/SecurityQuestions/"
```
