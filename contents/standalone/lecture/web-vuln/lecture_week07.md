...
printf '%-20s %s\n' '카테고리' '응답 코드'
for label_url in "SQLi=http://10.20.32.80:80/?id=1'+OR+1=1--" \
                  "XSS=http://10.20.32.80:80/?q=<script>alert(1)</script>" \
                  "Path=http://10.20.32.80:80/../../etc/passwd" \
                  "CMDi=http://10.20.32.80:80/?cmd=;id"; do
  label="${label_url%%=*}"
  url="${label_url#*=}"
...
curl -s -o /dev/null -w "  SQLi: %{http_code}\n" "http://10.20.32.80:80/?q='+OR+1=1--"  # silent 모드
curl -s -o /dev/null -w "  XSS: %{http_code}\n" "http://10.20.32.80:80/?q=<script>alert(1)</script>"  # silent 모드
```
