...
> echo http://juice.6v6.lab | waybackurls | grep -E 'robots|sitemap'
>
...
for path in security.txt openid-configuration change-password apple-app-site-association assetlinks.json; do
  code=$(curl -o /dev/null -s -w "%{http_code}" "http://juice.6v6.lab/.well-known/$path")
...
gobuster dir -u http://juice.6v6.lab -w /usr/share/wordlists/dirb/common.txt -t 30 -x html,php,bak

# ffuf (단일 wordlist 퍼징, 멀티스레드)
ffuf -w /usr/share/wordlists/dirb/common.txt -u http://juice.6v6.lab/FUZZ -mc 200,301,403 -t 50
...
for path in \
  ...
  read code size < <(curl -o /dev/null -s -w "%{http_code} %{size_download}" "http://juice.6v6.lab/$path")
...
> ffuf -u http://juice.6v6.lab/FUZZ -w wordlist.txt -e .bak,.old,.swp,.zip,.tar.gz,.tar
> # 또는 파일명·확장자 동시 fuzzing
> ffuf -u http://juice.6v6.lab/FUZZ.FUZZ2 -w names.txt:FUZZ -w exts.txt:FUZZ2 -mode pitchfork
> ```
...
curl -sI http://juice.6v6.lab | grep -iE "server|x-powered|x-generator|x-aspnet|set-cookie"
echo ""
echo "=== Apache (포트 80) ==="
...
curl -sI http://juice.6v6.lab | grep -i set-cookie
echo '---'
# 다양한 endpoint 의 쿠키 차이
for path in "/" "/rest/products/search?q=test" "/api/Users"; do
  echo "[$path]"
  curl -sI "http://juice.6v6.lab$path" | grep -i 'set-cookie' || echo '(no Set-Cookie)'
done
```
...
curl -s http://juice.6v6.lab/nonexistent_path_12345

# 잘못된 API 요청으로 에러 유도
curl -s http://juice.6v6.lab/api/Products/abc
...
```bash
# HTML에서 JS 파일 경로 추출
curl -s http://juice.6v6.lab | grep -oE 'src="[^"]*\.js"' | head -10
echo '---'
...
curl -s "http://juice.6v6.lab/$MAIN_JS" | grep -oE '/api/[a-zA-Z/]+|/rest/[a-zA-Z/]+' | sort -u | head -10
echo '---'
# 노출된 secret/key 패턴 정규식 검색
curl -s "http://juice.6v6.lab/$MAIN_JS" | grep -oE '(AKIA[0-9A-Z]{16}|sk_live_[0-9a-zA-Z]{24,}|api[_-]?key[\"'\'' :=]+[a-zA-Z0-9]{16,})' | head -5
```
...
> pip install jsbeautifier && python3 LinkFinder.py -i http://juice.6v6.lab/main.js -o cli
>
...
curl -vI https://juice.6v6.lab 2>&1 | head -8
```
