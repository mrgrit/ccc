...
- Bastion Experience 자동 승급 메커니즘을 활용한 지식 축적 체계를 구현한다
...
| fw | 10.20.30.1 | 방화벽/HAProxy/Suricata ext | `ssh 6v6-fw` |
...
| **Experience** | Bastion에 축적되는 에이전트 경험/지식 | 인시던트 대응 결과, 분석 리포트 |
...
| Bastion 호환 | 별도 구축 | 기존 DB 활용 |

Bastion는 벡터 DB 없이 PostgreSQL FTS로 RAG를 구현한다.
...
curl -s -X POST "http://localhost:9100/projects/${RAG_PID}/dispatch" \
  ...
  -H "X-API-Key: $BASTION_API_KEY" \
  -d '{
    "command": "PGPASSWORD=bastion psql -h 127.0.0.1 -U bastion -d bastion -t -c \"SELECT title || ': ' || content FROM security_knowledge WHERE search_vector @@ to_tsquery('simple','ssh & brute') ORDER BY ts_rank(search_vector, to_tsquery('simple','ssh & brute')) DESC LIMIT 3\"",
...
EDUCATION_DIR = "/home/bastion/bastion/contents/education"
