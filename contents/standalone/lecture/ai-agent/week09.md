...
- Bastion dispatch 인젝션 방어 실습을 통해 입력 검증 체계를 구축한다
...
| 1:25-1:35 | — | 휴식 | — |
| 1:35-2:10 | Part 4 | Bastion dispatch 인젝션 방어 | 실습 |
...
| **dry_run** | 실제 실행 없이 결과를 시뮬레이션하는 모드 | Bastion critical 태스크 자동 dry_run |
...
### 1.3 Bastion에서의 위협 지점

Bastion는 Master → Manager → SubAgent 계층 구조를 사용한다.
...
> **실습 목적**: Bastion 플랫폼과 연동하여 에이전트가 실제 인프라에서 보안 작업을 수행하도록 구현하기 위해 수행한다
>
...
## Part 4: Bastion dispatch 인젝션 방어 (1:35-2:10)

### 4.1 dispatch 인젝션 공격 이해

Bastion의 dispatch API는 자연어 명령을 SubAgent에 전달한다.
...
Bastion는 `risk_level=critical` 태스크에 대해 자동으로 dry_run을 강제한다.
...
### 5.3 Bastion critical 태스크 승인 플로우
