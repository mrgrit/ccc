...
## 실제 사례 (WitFoo Precinct 6 — AWS API call 분포 = IAM 자동 enumeration baseline)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Cloud IAM 자동 Pivot — IAM 권한 열거·Role chain·STS* 학습 항목과 매핑되는 dataset 의 AWS API call 분포 (150K+).

### Case 1: AWS read-only API — IAM enumeration 의 직접 baseline

| API call | 건수 | 학습 매핑 |
|----------|------|----------|
| GetLogEvents | 39,639 | log access — *침투 후 흔적 확인* |
| DescribeInstanceStatus | 27,127 | EC2 정찰 |
| DescribeLoadBalancers | 15,062 | LB 정찰 |
| DescribeInstances | 14,992 | EC2 enumeration |
| ListClusters | 10,848 | ECS/EKS 정찰 |
| **AssumeRole** | **9,340** | **role chain** (T1078.004) |
| DescribeSecurityGroups | 5,858 | SG enumeration |
| DescribeVpcs | 5,079 | network 정찰 |
| GenerateDataKey | 3,286 | KMS key 사용 |
| Decrypt | 1,605 | KMS decrypt |
| **합계 (read-only + STS)** | **150,000+** | **enumeration 가능 표면** |

→ 정상 baseline 이 **150K+** = enumeration 도구가 *합법적으로 보이는 noise* 안에 숨을 수 있음.

### Case 2: AssumeRole 9,340 건 — role chain 추적

본 lecture *role chain* 학습 항목 = AssumeRole 의 시간 분포 + source/target role 매트릭스. dataset 의 9,340 건 baseline 으로:

```python
# 의사 코드 — dataset 의 AssumeRole 패턴 분석
[정상] 동일 user 가 1~2 role 로만 AssumeRole
[의심] 동일 user 가 5+ role 전환 (cross-account 의심)
[critical] 동일 IAM 권한이 *비표준 시간* (UTC 0~5시) 에 AssumeRole burst
```

### Case 3: KMS API — Decrypt + GenerateDataKey 비율

| API | 건수 | 비율 |
|-----|------|------|
| GenerateDataKey | 3,286 | 정상 (encrypt 시) |
| Decrypt | 1,605 | 정상의 *반* — encrypt 보다 적게 |

→ *Decrypt > GenerateDataKey* 시 의심 = 침투자가 *기존 암호화 데이터 복호화*. 본 dataset 비율 (1:0.49) 가 *baseline*.

**해석 — 본 lecture (Cloud IAM Pivot) 와의 매핑**

| 학습 항목 | 본 record 의 증거 |
|-----------|------------------|
| **enumeration 표면** | 150K read-only API = 정상 baseline. spike 시 자동 enumeration 의심 |
| **role chain (AssumeRole)** | 9,340 건 baseline 분포. user 별 role 다양성 측정 |
| **KMS abuse** | Decrypt:GenerateDataKey 비율 = 0.49 baseline. >1 시 침투 의심 |
| **CloudTrail audit** | dataset 의 모든 API call = CloudTrail 의 직접 표현 |

**학생 실습 액션**:
1. 본인 AWS 환경의 Describe* / Get* / List* 분포 baseline 측정 → dataset 150K 와 비교
2. AssumeRole 의 *user × target role* 매트릭스 작성 → 단일 user 5+ role 시 의심
3. KMS Decrypt:GenerateDataKey 비율 추적 — > 1 시 *침투 후 복호화* 가설 검증
