...
## 실습 환경 (6v6 4-tier, 본 과정 공통)

학생 PC 의 `~/.ssh/config` 의 ProxyJump 설정 후 `ssh 6v6-<name>` 으로 접속.

| 컨테이너 | 6v6 IP | 역할 | 접속 |
|---------|--------|------|------|
| bastion | 10.20.30.201 (ext) | Blue Agent · 관제 (Bastion 운영 에이전트, KG, A2A) | `ssh 6v6-bastion` (pw: ccc) |
| attacker | 10.20.30.202 (ext) | Claude Code 공격 세션 (pen-test 13 종 curl/nmap/nikto/whatweb/sqlmap/hydra/등) | `ssh 6v6-attacker` |
| fw | 10.20.30.1 (ext) + 10.20.31.1 (pipe) | nftables + HAProxy host-header · 네트워크 관찰 (dual NIC = router 모드) | `ssh 6v6-fw` (ProxyJump bastion) |
| ips | 10.20.31.2 (pipe) + 10.20.32.1 (dmz) | Suricata IPS · 네트워크 차단 (dual NIC = router 모드) | `ssh 6v6-ips` (ProxyJump fw) |
| web | 10.20.32.80 (dmz) + 10.20.40.80 (int) | 공격 표적 · CI runner 역할 (Apache + ModSecurity + JuiceShop/DVWA reverse, dual NIC = vhost 라우팅) | `ssh 6v6-web` (ProxyJump fw) |
| siem | 10.20.32.100 (dmz) | Wazuh manager (alerts.json + active-response) | `ssh 6v6-siem` (ProxyJump fw) |

> **registry 부재 처리.** 6v6 표준 16 컨테이너 에는 별도 registry 가 없으므로, 본 과목 의
> "내부 PyPI/npm 레지스트리" 실습 은 `web` 컨테이너 위 에 devpi (3141 포트) 또는
> verdaccio (4873) 를 추가 띄워 모사 한다. lab YAML 의 setup 단계 참고.
