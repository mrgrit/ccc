...
```bash
# API 키 없이 요청 → 인증 실패
curl -s http://localhost:9100/students | head -5

# API 키 포함 요청 → 인증 성공
curl -s -H "X-API-Key: ccc-api-key-2026" \
  http://localhost:9100/students | head -5
...
## 부록: 학습 OSS 도구 매트릭스 (Course6 Cloud-Container — Week 09 서버리스)

### 서버리스 / FaaS OSS 도구

| 영역 | OSS 도구 |
|------|---------|
| FaaS 플랫폼 | **OpenFaaS** / Knative / Fission / Apache OpenWhisk |
| 이벤트 게이트웨이 | **Knative Eventing** / Argo Events / Triggermesh |
| 보안 점검 | Trivy + Cosign / FaaS 별 자체 |
| 권한 분석 | OPA + 커스텀 규칙 |
| 모니터링 | OpenTelemetry / Jaeger / Grafana Tempo |

### 핵심 — OpenFaaS (가장 OSS 친화적 FaaS)
...
kubectl apply -k "github.com/openfaas/faas-netes/yaml_arm64?ref=master"

# 첫 함수 배포
faas-cli new --lang python3-flask hello
cd hello
...
# 호출
curl http://gateway:8080/function/hello -d "world"
```
...
# Fission
curl -Lo fission https://github.com/fission/fission/releases/latest/download/fission-1.20.0-linux
chmod +x fission && sudo mv fission /usr/local/bin/
fission install

# Trivy + cosign (서버리스 함수 컨테이너 점검 — week01-03 도구 재사용)
```

### 보안 점검 흐름

```bash
# Phase 1: 함수 컨테이너 CVE 스캔
faas-cli list
trivy image hello:latest --severity HIGH,CRITICAL

# Phase 2: 함수 권한 최소화
# /yaml/hello.yml 의 readOnlyRootFilesystem: true / runAsNonRoot: true

# Phase 3: 인증 강제
faas-cli login --gateway http://localhost:8080 --username admin --password Pa$$w0rd

# Phase 4: Secret 관리
faas-cli secret create db-password --from-literal=db-password=Pa$$w0rd
# 함수 내부에서 /var/openfaas/secrets/db-password 으로 mount

# Phase 5: Rate limiting / 인가 (OPA + nats messaging)
# Knative AuthZPolicy 또는 OPA Envoy filter
```

학생은 본 9주차에서 **OpenFaaS + Knative + Trivy + cosign + OPA** 5 도구로 서버리스 보안 (인증/권한/secret/rate-limit) 의 OSS 표준 흐름을 익힌다.
