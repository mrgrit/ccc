# Week 05 — Suricata IDS (2) — 룰 작성 심화 (pcre / fast_pattern / flowbits / threshold / suppression)

> **본 주차의 한 줄 요약**
>
> W04 의 단순 content 매칭에서 한 단계 위로 — **표현력** (pcre 정규식, content modifier
...
>   content 에 적용)
>
> 6v6 는 6.0.4 이므로 **전위 문법** 사용. 5.x 룰을 가져오면 silent fail.
...
Suricata 가 65,898 룰을 매 packet 마다 순회 평가하면 throughput 0. 대신 **각 룰의 1개
...
alert http $EXTERNAL_NET any -> $HOME_NET any (
    msg:"6v6 sqlmap UNION SELECT attempt (rate-limited)";
    flow:to_server,established;
...
## 10. 6v6 의 실 운영 적용 — Wazuh 통합 W09 예고

Suricata 의 eve.json 이 Wazuh agent → manager → dashboard 로 ship.

```
6v6-ips:/var/log/suricata/eve.json
  ↓ Wazuh agent
6v6-siem (manager) :1514
  ↓ analysisd decoder + 룰
6v6-wazuh-indexer (OpenSearch)
  ↓ dashboard
6v6-wazuh-dashboard :5601
```
...
- `-t 4` — 동시 4 thread.
- `-f` — 한 건 성공하면 즉시 종료.
- `ssh://10.20.32.80` — target SSH 서비스 (6v6-web dmz). 학습 환경에서는 fw forward chain 이 ext→dmz:22 를 의도적으로 허용한다.
...
1. `suppress` — ET POLICY 룰 중 6v6 환경에서 false-positive 인 1개 (justification 1줄)
...
## 부록 B — production-grade 룰 작성 체크리스트 (W04 확장)
