...
이 컨시어지의 역할을 web 세계에 그대로 옮긴 것이 **HAProxy** 다.

**HAProxy** (High Availability Proxy) 는 외부 web 요청을 받는 컨시어지다. 외부에서 들어오는 모든 요청은 먼저 HAProxy에 도착하며, HAProxy는 다음 네 가지를 판단한다.

- 이 요청은 어느 host에 대한 것인가? → `Host:` 헤더를 확인한다.
- 그 host에 적절한 내부 server는 어디인가? → backend를 선택한다.
- 같은 backend에 server가 여러 대라면 어디로 보낼까? → load balancing.
- 결정된 backend의 server로 요청을 전달한다.

**비유 매핑.**

| 호텔 컨시어지 | HAProxy |
|----------------|---------|
| 호텔 입구 | HAProxy listener (port 80, 443) |
| 방문객의 의도 | `Host:` 헤더 (예: `web.6v6.lab`) |
| 객실 그룹 (1층/2층/3층) | backend (`waf` / `dashboard` / `bastion` 등) |
| 객실 직접 안내 | 내부 server로 전달 |
| 의도 외 방문 거절 | 인증 실패 응답 |

**6v6 인프라에서의 HAProxy.**

6v6에서는 fw VM이 HAProxy로 가동되며, 다음 6 개 backend를 운영한다.

- `waf` — 정상 HTTP web 요청을 WAF로 전달한다.
- `waf_tls` — HTTPS 요청을 WAF로 전달한다.
- `dashboard` — Wazuh Dashboard 접근을 라우팅한다.
- `portal` — 운영자 portal로 라우팅한다.
- `bastion` — Bastion agent와의 통신을 라우팅한다.
- `bastion_api` — Bastion API 호출을 라우팅한다.

학생이 web browser에서 `https://dashboard.6v6.lab` 을 입력했을 때 일어나는 흐름은 다음 7 단계다.

1. 학생 host의 DNS가 `dashboard.6v6.lab` 을 IP (192.168.0.X) 로 해석한다.
2. browser가 X.X.X.X:443으로 HTTPS 요청을 보낸다.
3. fw VM의 HAProxy가 그 요청을 수신한다.
4. HAProxy가 `Host:` 헤더에서 `dashboard.6v6.lab` 을 확인한다.
5. HAProxy가 `dashboard` backend를 선택한다.
6. HAProxy가 내부 dashboard server (192.168.0.103:5601) 로 요청을 전달한다.
7. dashboard의 응답이 HAProxy를 거쳐 학생 browser로 돌아온다.

학생 입장에서는 "한 번 클릭"으로 1초도 걸리지 않지만, 내부에서는 이 7 단계 hop이 발생한다. 운영자가 침해를 추적하려면 이 hop을 모두 이해해야 한다.

### 0.5.2 Reverse Proxy — HAProxy를 일반화한 개념

HAProxy를 이해했다면 그 일반 개념인 Reverse Proxy도 자연스럽게 따라온다.

**Reverse Proxy** 는 외부 요청을 받아서 내부 server로 전달하고, 응답을 다시 외부로 돌려주는 중계자다. HAProxy는 Reverse Proxy의 한 구현이며, nginx, Apache, Traefik, Caddy 등도 같은 역할을 한다.
...
| Host 헤더 | 응답하는 사이트 | 용도 |
|----------|----------------|------|
| `dashboard.6v6.lab` | Wazuh Dashboard | 운영자/학생의 분석 |
...
```bash
# 학생 host에서 web VM으로 SSH (bastion 경유)
ssh -J 6v6-bastion ccc@web.6v6.lab
```

- `-J 6v6-bastion` — bastion을 ProxyJump로 지정.
- `ccc@web.6v6.lab` — 최종 target.
...
1. **Red** — attacker VM (192.168.0.112) 에서 재현 가능한 실 공격을 발생시킨다.
...
- **윤리 강제.** 학습 환경 안에서만 시도한다. attacker VM (192.168.0.112) 외부 시스템을 대상으로 한 시도는 절대 금지다.
...
- **Wazuh** — open-source SIEM이다. 학습 환경에서는 manager (192.168.0.103의 SIEM 컨테이너) 와 각 VM에 설치된 agent로 구성된다.
...
학생이 attacker VM (192.168.0.112) 에 들어가서 SSH brute force를 시도한다.

먼저 attacker VM에 SSH로 들어간다.
...
attacker VM 내부에서 6v6의 web VM (192.168.0.103) 의 SSH에 10번 시도한다. 학습 환경 안에서만 실행해야 한다.
...
예상 결과는 10번 모두 실패다. 학습 환경의 admin 비밀번호는 위에서 입력한 `wrongN` 이 아니기 때문이다.

**2. 발생하는 로그/아티팩트.**

먼저 web VM의 `/var/log/auth.log`에 다음과 같은 줄이 쌓인다.

```
May 12 14:30:15 web sshd[12345]: Failed password for invalid user admin from 192.168.0.112 port 54321 ssh2
May 12 14:30:16 web sshd[12346]: Failed password for invalid user admin from 192.168.0.112 port 54322 ssh2
... (10번 반복)
```

Wazuh agent가 이 파일을 실시간으로 모니터링하기 때문에, 같은 사건이 Wazuh manager의 `/var/ossec/logs/alerts/alerts.json`에 다음 형식으로 다시 기록된다.

```json
{"rule": {"id": "5710", "level": 5,
  "description": "Attempt to login using a non-existent user"},
 "agent": {"name": "web", "ip": "192.168.0.103"},
 "data": {"srcip": "192.168.0.112", "srcuser": "admin"},
...
- `rule.level` — 5 (medium).
- `agent.name` — web.
- `data.srcip` — 192.168.0.112 (학습 환경의 attacker VM).
...
5. Source IP 항목에 `192.168.0.112` 를 입력한다.
6. Timeout 항목에 `600` 초를 입력한다.
7. Submit을 클릭한다.

결과로 web VM에서 `iptables -A INPUT -s 192.168.0.112 -j DROP` 이 자동으로 적용된다. 600초가 지나면 자동으로 rollback된다.
...
- **OpenCTI에 IoC 등록.** attacker VM IP를 OpenCTI indicator로 등록해두면 다른 6v6 자산에서도 같은 IP가 감지될 때 자동으로 알람이 뜬다.
...
학생이 attacker VM에서 nmap으로 6v6의 web VM (192.168.0.103) 을 port scan한다.

```bash
# attacker VM 내부 (학습 환경 한정)
nmap -sS -p 1-1000 -T4 192.168.0.103
```
...
- URL: `https://dashboard.6v6.lab` (Wazuh Dashboard와 같은 instance다).
...
4. Search bar에 `event_type:alert AND src_ip:192.168.0.112` 를 입력하고 Enter를 누른다.
...
- src_ip는 192.168.0.112 하나로 일관된다. 학습 환경 attacker VM이다.
...
```
학생 host
  → ssh 6v6-bastion              (bastion 진입)
    → ssh fw                      (fw 진입, ProxyJump 활용)
       → sudo nft list ruleset    (현재 룰 확인)
       → sudo nft add rule inet filter input ip saddr 192.168.0.112 drop
       → sudo nft list ruleset    (적용 확인)
```
...
- **6v6 토폴로지 검토.** fw 단계에서 port scan을 미리 차단할 수 있는지 검토한다. nftables의 rate limit 옵션 추가도 고려한다.
...
학생이 attacker VM에서 6v6의 web VM (192.168.0.103) 로 단순 SQLi 시도를 한다.

```bash
# attacker VM 내부 (학습 환경 한정)
curl -G "http://192.168.0.103/login" \
