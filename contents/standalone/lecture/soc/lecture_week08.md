...
# 커널 오류 확인
ssh 6v6-bastion "journalctl -p err --no-pager 2>/dev/null | tail -10"
```
...
```bash
# Suricata 알림 통계
ssh 6v6-fw "cat /var/log/suricata/fast.log 2>/dev/null | wc -l"
...
```bash
# 웹 로그 기본 통계
ssh 6v6-web "wc -l /var/log/nginx/access.log 2>/dev/null || echo '0'"

# HTTP 상태코드 분포
ssh 6v6-web "awk '{print \$9}' /var/log/nginx/access.log 2>/dev/null | \
  sort | uniq -c | sort -rn | head -10"

# 웹 공격 패턴 검색
ssh 6v6-web "grep -iE 'union|select|script|alert|\.\./' /var/log/nginx/access.log 2>/dev/null | wc -l"

# 의심스러운 요청 샘플
ssh 6v6-web "grep -iE 'union|select|script|\.\./' /var/log/nginx/access.log 2>/dev/null | head -5"
...
ssh 6v6-fw "ls -lh /var/log/suricata/fast.log 2>/dev/null"  # 비밀번호 자동입력 SSH
ssh 6v6-siem "ls -lh /var/ossec/logs/alerts/alerts.json 2>/dev/null"  # 비밀번호 자동입력 SSH
```
