...
학생 PC 의 `~/.ssh/config` 의 ProxyJump 설정 + `/etc/hosts` 의 vhost 매핑 후
다음 표 의 컨테이너 에 `ssh 6v6-<name>` 으로 접속.
...
> 우리 실습 환경은 **자체 SOC** 모델로, siem 서버의 Wazuh가 SIEM 역할을 하고, Bastion가 관제 자동화를 지원한다.
...
| **SOAR** | 대응 자동화, Playbook 실행 | Bastion (bastion:8000) |
| **EDR** | 엔드포인트 탐지/대응 | Wazuh Agent |
| **CTI** | 위협 인텔리전스 | OpenCTI (siem:9400) |
| **Ticketing** | 사고 관리/추적 | Bastion 프로젝트 |
...
bastion (Agent) --+    (로그 수집/분석)   (로그 저장/검색)
                            |
                            v
                       Wazuh Dashboard
                       (시각화/알림 관리)
...
> 우리 실습 환경에서 **Bastion**가 SOAR 역할을 수행한다.
...
```json
{
  ...
  "full_log": "Mar 27 10:15:30 bastion sshd[12345]: Failed password for admin from 192.168.1.100 port 54321 ssh2"
}
```
...
Level: 5
Source IP: 10.20.30.201 (bastion)
Target: secu
Count: 3회
```
...
1. 알림 현황 요약
   ...
   - 에이전트별 분포: secu: ___건, web: ___건, bastion: ___건
...
| SOC 분석가가 매일 보는 호스트 | 어디서 오는 데이터 |
|------------------------------|------------------|
| Linux 서버 6대 (fw/ips/web/siem 등) | rsyslog + osquery + Suricata + ModSec + Wazuh agent |
| **Windows 사용자 PC 1대 (6v6-win)** | **Sysmon EID 1/3/22/11/... + Security 4624/4625** |

본 과목은 secuops 와 달리 "도구 운영" 이 아니라 **"운영자 = 분석가" 의 시각** 이다. SOC 분석가는
방화벽 룰을 잘 만드는 게 아니라 **이미 만들어진 인프라가 만들어낸 모든 로그를 시간순으로 묶어
사건을 푸는 사람**이다. Windows 엔드포인트가 들어옴으로써 사건의 **시작점** (직원 PC 의 우발적
행위) 이 비로소 SOC 의 시야에 들어왔다.

> 본 과목은 매 주차 끝에 "Windows 엔드포인트와 SOC" 단락을 둔다 — SOC 분석가의 시야에서 Windows
> 가 어느 자리를 차지하는지를 매번 확인.
