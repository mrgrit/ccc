...
```bash
# web 서버의 웹 로그 확인
ssh 6v6-web "ls -la /var/log/nginx/ /var/log/apache2/ /var/log/apache2/ 2>/dev/null"  # 비밀번호 자동입력 SSH

# 접근 로그 최근 내용
ssh 6v6-web "tail -20 /var/log/nginx/access.log 2>/dev/null || \
...
ssh 6v6-web "grep -iE 'union|select|or.1=1|drop.table|insert.into' /var/log/nginx/access.log 2>/dev/null | tail -10"

# XSS 시도 탐지 (<script>, alert, onerror 등)
ssh 6v6-web "grep -iE 'script|alert|onerror|onload' /var/log/nginx/access.log 2>/dev/null | tail -10"

# 디렉토리 탐색 시도 (../ 또는 %2e%2e)
ssh 6v6-web "grep -iE '\.\./|%2e%2e|/etc/passwd' /var/log/nginx/access.log 2>/dev/null | tail -10"

# 대량 404 에러 (디렉토리 스캐닝 의심)
ssh 6v6-web "grep ' 404 ' /var/log/nginx/access.log 2>/dev/null | \
  awk '{print \$1}' | sort | uniq -c | sort -rn | head -5"

# 대량 요청 IP (DDoS/스캐닝 의심)
ssh 6v6-web "awk '{print \$1}' /var/log/nginx/access.log 2>/dev/null | \
...
ssh 6v6-web "awk -F'\"' '{print \$6}' /var/log/nginx/access.log 2>/dev/null | \
  sort | uniq -c | sort -rn | head -10"

# 알려진 스캐너 도구 탐지
ssh 6v6-web "grep -iE 'nikto|sqlmap|nmap|dirbuster|gobuster|burp' /var/log/nginx/access.log 2>/dev/null | tail -5"
```
...
# 우선순위별 통계
ssh 6v6-fw "grep -oP 'Priority: [0-9]+' /var/log/suricata/fast.log 2>/dev/null | \
  sort | uniq -c | sort -rn"

# 출발지 IP별 통계 (공격자 식별)
ssh 6v6-fw "grep -oP '\\{\\w+\\} [0-9.]+' /var/log/suricata/fast.log 2>/dev/null | \
...
ssh 6v6-fw "tail -3 /var/log/suricata/eve.json 2>/dev/null | python3 -m json.tool 2>/dev/null | head -30"  # 비밀번호 자동입력 SSH
...
```bash
# 현재 방화벽 규칙 확인
ssh 6v6-fw "sudo nft list ruleset 2>/dev/null"

# 방화벽 로그 확인 (커널 로그에 기록됨)
ssh 6v6-fw "grep -i 'nft\|DROPPED\|REJECT\|firewall' /var/log/kern.log 2>/dev/null | tail -10"
ssh 6v6-fw "dmesg 2>/dev/null | grep -i 'nft\|DROP\|REJECT' | tail -10"

# iptables 로그 (레거시 환경)
ssh 6v6-fw "grep 'iptables\|nftables' /var/log/syslog 2>/dev/null | tail -10"
```
...
ssh 6v6-web "ls -la /var/log/apache2/ 2>/dev/null"  # 비밀번호 자동입력 SSH
ssh 6v6-web "journalctl -u apache2 --no-pager | tail -20 2>/dev/null | tail -20 || echo 'docker 로그 확인 필요'"  # 비밀번호 자동입력 SSH
...
echo "=== auth.log ==="
ssh 6v6-bastion "grep '$TARGET_TIME' /var/log/auth.log 2>/dev/null | head -5"  # 비밀번호 자동입력 SSH

echo "=== Suricata ==="
ssh 6v6-fw "grep '03/27/2026-10:1' /var/log/suricata/fast.log 2>/dev/null | head -5"  # 비밀번호 자동입력 SSH

echo "=== 웹 로그 ==="
ssh 6v6-web "grep '27/Mar/2026:10:1' /var/log/nginx/access.log 2>/dev/null | head -5"  # 비밀번호 자동입력 SSH
```
...
echo "[SSH 시도]"
ssh 6v6-bastion "grep '$SUSPECT_IP' /var/log/auth.log 2>/dev/null | tail -5"  # 비밀번호 자동입력 SSH

echo "[IPS 탐지]"
ssh 6v6-fw "grep '$SUSPECT_IP' /var/log/suricata/fast.log 2>/dev/null | tail -5"  # 비밀번호 자동입력 SSH

echo "[웹 접근]"
ssh 6v6-web "grep '$SUSPECT_IP' /var/log/nginx/access.log 2>/dev/null | tail -5"  # 비밀번호 자동입력 SSH
```
...
1. **Wazuh** > **Agents** 클릭
2. **web** 에이전트 (10.20.32.80)를 클릭하여 상세 페이지 이동
