...
| web | 10.20.32.80 | Juice Shop (Apache + ModSecurity) | `ssh 6v6-web` |
...
sequenceDiagram
    ...
    U->>KDC: 1. AS-REQ (사용자명, 타임스탬프)
    KDC-->>U: 2. AS-REP (TGT + 세션키)
    Note over U: TGT 획득 (krbtgt 해시로 암호화)
    U->>KDC: 3. TGS-REQ (TGT + SPN)
    KDC-->>U: 4. TGS-REP (서비스 티켓 + 세션키)
    Note over U: ST 획득 (서비스 해시로 암호화)
    U->>S: 5. AP-REQ (서비스 티켓)
    S-->>U: 서비스 응답
```
...
## 실제 사례 (WitFoo Precinct 6 — Linux Privesc 심화)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Linux Privesc 심화* 학습 항목 매칭.

### Linux Privesc 심화 의 dataset 흔적 — "GTFOBins + SUID"

dataset 의 정상 운영에서 *GTFOBins + SUID* 신호의 baseline 을 알아두면, *Linux Privesc 심화* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — process audit + 4688.

```mermaid
graph LR
    SCENE["Linux Privesc 심화 시나리오"]
    TRACE["dataset 흔적<br/>GTFOBins + SUID"]
    DETECT["탐지 / 분석"]
...
|---|---|
| 핵심 신호 | GTFOBins + SUID |
| 정량 baseline | process audit + 4688 |
| 학습 매핑 | Linux privesc 자동화 |

**자세한 해석**: Linux privesc 자동화. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | GTFOBins + SUID 의 spike |
...
1. **Linux Privesc 심화 = GTFOBins + SUID 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — process audit + 4688.
