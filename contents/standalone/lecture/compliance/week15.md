...
ssh 6v6-siem "cat /var/ossec/logs/alerts/alerts.json 2>/dev/null | wc -l"  # 비밀번호 자동입력 SSH

# 3. 로그 보관 기간
ssh 6v6-bastion "cat /etc/logrotate.conf | grep -E 'rotate|weekly'"  # 비밀번호 자동입력 SSH
```
...
# 방화벽 규칙
ssh 6v6-fw "sudo nft list ruleset 2>/dev/null"  # 비밀번호 자동입력 SSH
...
# TLS 버전
ssh 6v6-siem "echo | openssl s_client -connect localhost:443 2>/dev/null | grep -E 'Protocol|Cipher'"

# SSH 알고리즘
ssh 6v6-bastion "grep -E 'Ciphers|MACs|KexAlgorithms' /etc/ssh/sshd_config | grep -v '^#'"

# 파일 권한
ssh 6v6-bastion "stat -c '%a %n' /etc/shadow /etc/ssh/ssh_host_*_key 2>/dev/null"
```
...
# 격리/차단 능력
ssh 6v6-fw "sudo nft list ruleset 2>/dev/null | grep -c 'drop'"  # 비밀번호 자동입력 SSH
...
본 15주차는 **1-14주차의 모든 도구를 통합 운용** 하는 종합 평가.

### 통합 OSS 도구 매트릭스 (4-Compliance 전 영역)

| 영역 | 핵심 OSS 도구 |
|------|--------------|
| 자동 점검 | Lynis · OpenSCAP · Wazuh SCA · chef-inspec |
| 자산 관리 | osquery · netbox · GLPI · syft (SBOM) |
| 위험 관리 | OWASP Threat Dragon · pytm · OpenVAS · DefectDojo |
| 정책 / 절차 | pandoc · Mermaid · OSCAL · OPA · GitLab CE |
| 무결성 | aide · Tripwire · Wazuh FIM · velociraptor |
| 인증 / 권한 | Keycloak · OPA · Casbin · ssh-audit |
| 암호화 | Vault · sops · age · LUKS · testssl.sh |
| 로그 / 감사 | Wazuh · auditd · Suricata · jq · lnav |
| 백업 / DR | restic · borgbackup · Bareos · Velero · Pacemaker |
| 사고 대응 | TheHive · Velociraptor · Volatility · Wazuh AR |
| 클라우드 | Prowler · scout-suite · Trivy · kube-bench |
| PII / 프라이버시 | Presidio · scrubadub · pii-codex |
| 교육 / 인식 | gophish · Moodle · Atomic Red Team |
| 자동화 / SOAR | Shuffle · n8n · TheHive Cortex · ansible |
| GRC 통합 | eramba · simplerisk · DefectDojo |

### 종합 평가 시나리오 — 한 명령으로 전체 점검

```bash
#!/bin/bash
# /usr/local/bin/compliance-master-audit.sh
TS=$(date +%Y%m%d-%H%M)
DIR=/var/log/compliance/$TS
mkdir -p $DIR

# === Phase 1: OS 보안 baseline ===
sudo lynis audit system --quiet > $DIR/01-lynis.txt
sudo oscap xccdf eval --profile cis_level2_server \
  --report $DIR/02-cis.html ssg-ubuntu2204-ds.xml 2>&1 | head

# === Phase 2: 자산 인벤토리 ===
osqueryi "SELECT * FROM os_version;" --json > $DIR/03-os.json
osqueryi "SELECT name, version FROM rpm_packages;" --json > $DIR/04-pkg.json

# === Phase 3: 위협 / 취약점 ===
nuclei -u http://10.20.30.80 -severity high,critical -j > $DIR/05-nuclei.json
nikto -h http://10.20.30.80 -o $DIR/06-nikto.txt

# === Phase 4: PII / 프라이버시 ===
sudo find /var/log -type f -exec scrubadub-cli {} \; 2>/dev/null | grep -i 'PII' > $DIR/07-pii.txt

# === Phase 5: 무결성 ===
sudo aide --check > $DIR/08-aide.txt 2>&1

# === Phase 6: TLS / 암호화 ===
testssl.sh --severity HIGH https://10.20.30.80:443 > $DIR/09-tls.txt 2>&1

# === Phase 7: 백업 검증 ===
restic -r /backup/restic-repo check --read-data > $DIR/10-backup.txt 2>&1
restic snapshots >> $DIR/10-backup.txt

# === Phase 8: 로그 / 감사 ===
sudo aureport --auth -i > $DIR/11-audit-auth.txt
sudo jq -r 'select(.rule.level >= 10) | .rule.description' \
  /var/ossec/logs/alerts/alerts.json | sort | uniq -c | sort -rn > $DIR/12-wazuh-top.txt

# === Phase 9: 사용자 인식 (gophish) ===
curl -k -s https://gophish/api/campaigns -H "Authorization: Bearer $TOKEN" \
  | jq '.[] | {name, results}' > $DIR/13-phishing.json

# === Phase 10: 종합 보고서 (pandoc) ===
cat > $DIR/00-report.md << EOF
# 컴플라이언스 종합 점검 보고서
- 점검 일시: $(date '+%Y-%m-%d %H:%M')
- 점검 대상: 전사 인프라
- 사용 도구: 15+ OSS

## 요약 통계
- Lynis warnings: $(grep -c WARNING $DIR/01-lynis.txt)
- OpenSCAP failures: $(grep -c 'fail' $DIR/02-cis.html 2>/dev/null)
- Critical vuln: $(jq -r 'select(.severity=="critical")' $DIR/05-nuclei.json | wc -l)
- PII 발견: $(wc -l < $DIR/07-pii.txt)
- AIDE 변경: $(grep -c 'Changed' $DIR/08-aide.txt)
- 백업 무결성: $(grep -q 'no errors' $DIR/10-backup.txt && echo OK || echo FAIL)
EOF

pandoc $DIR/00-report.md -o $DIR/master-report.pdf \
  --pdf-engine=xelatex -V mainfont=NanumGothic

echo "=== 종합 점검 완료 — 결과: $DIR ==="
```
...
| 등급 | 기준 |
|------|------|
| A | 자동 점검 + 자동 시정 + 분기 보고서 + 위반 0건 |
| B | 자동 점검 + 분기 보고서 + 경미한 위반 < 5건 |
| C | 수동 점검 + 분기 보고서 + 위반 < 10건 |
| F | 위반 ≥ 10건 또는 점검 미실시 |

### 컴플라이언스 자동화 4 단계 (학생 목표)

1. **자동 점검** — Lynis + OpenSCAP + Wazuh SCA 매월
2. **자동 시정** — Wazuh AR + ansible-playbook
3. **자동 보고** — pandoc + Grafana dashboard
4. **자동 검증** — Atomic Red Team 분기별

학생은 본 15주차 종합 평가에서 **OSS 50+ 도구 통합 운영 + 자동 점검·시정·보고 사이클** 을 직접 구축하고 컴플라이언스 운영자 역할을 수행한다.
