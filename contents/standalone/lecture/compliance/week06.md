...
ssh 6v6-bastion "ls -la /etc/security/ 2>/dev/null"  # 비밀번호 자동입력 SSH
ssh 6v6-bastion "ls -la /etc/pam.d/ | head -10"  # 비밀번호 자동입력 SSH
```
...
ssh 6v6-bastion "who"             # 비밀번호 자동입력 SSH
ssh 6v6-bastion "last | head -20"  # 비밀번호 자동입력 SSH
```
...
```bash
# 각 사용자의 비밀번호 만료일 확인
ssh 6v6-bastion "sudo chage -l user 2>/dev/null"
```
...
# SUID 파일 점검
ssh 6v6-bastion "find /usr -perm -4000 -type f 2>/dev/null | wc -l"
```
...
ssh 6v6-siem "echo | openssl s_client -connect localhost:443 2>/dev/null | grep -E 'Protocol|Cipher'"  # 비밀번호 자동입력 SSH

echo "=== SSH 암호 알고리즘 ==="
ssh 6v6-bastion "ssh -Q cipher 2>/dev/null | head -10"  # 비밀번호 자동입력 SSH
```
...
```bash
# 최근 패키지 변경 이력
ssh 6v6-bastion "cat /var/log/dpkg.log 2>/dev/null | tail -10"  # 비밀번호 자동입력 SSH
ssh 6v6-bastion "cat /var/log/apt/history.log 2>/dev/null | tail -20"  # 비밀번호 자동입력 SSH
```
...
# WAF 상태 확인
ssh 6v6-web "systemctl status apache2 2>/dev/null | head -5 || systemctl is-active apache2"
...
ssh 6v6-siem "ls -la /var/ossec/logs/alerts/ 2>/dev/null | tail -3"
```
...
```bash
# 최근 보안 이벤트 확인
ssh 6v6-siem "tail -5 /var/ossec/logs/alerts/alerts.json 2>/dev/null | python3 -m json.tool 2>/dev/null | head -30"
```
