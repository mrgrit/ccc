...
```bash
curl -s "http://6v6-host:9200/evidence?asset=web&limit=15" | python3 -m json.tool
```
...
curl -s "http://6v6-host:9200/evidence?limit=50" \
  | python3 -c "
...
```bash
# 등록된 Playbook 목록
curl -s http://6v6-host:9200/playbooks | python3 -m json.tool
