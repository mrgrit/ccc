...
ALERTS='[
  ...
  {"time": "10:36:00", "rule": "550",  "src": "10.20.32.80",  "msg": "사용자 추가: hacker"}
]'
...
SCENARIO="다음은 웹 서버(10.20.32.80)에서 30분간 수집된 로그입니다:
10:00 - 정상 웹 트래픽
...
## 부록: 학습 OSS 도구 매트릭스 (Course7 AI Security — Week 04 데이터 중독/훈련 데이터 보안)

### 데이터 중독 / 훈련 데이터 OSS 도구

| 영역 | OSS 도구 |
|------|---------|
| 데이터 정제 / 검증 | **cleanlab** / great-expectations / pandera |
| 백도어 탐지 | **TrojAI Tools** / Neural-Cleanse / abs-detector / Backdoor-toolbox |
| 훈련 보호 (DP) | **opacus** (Meta) / **TensorFlow Privacy** / pydp (Google) |
| 적대적 데이터 생성 | **TextAttack** / OpenAttack / TrojaiUtility |
| Membership Inference | **MIA-Bench** / MIArt / pyrobustness |
| Federated Learning 보안 | **Flower** + DP / FedML + crypten |
| Poisoning 시뮬 | **TextAttack PoisonRecipe** / BadNets toolkit |
| SBOM 데이터셋 | **datasheets-tool** / model cards |

### 핵심 — cleanlab (label noise 자동 발견)
...
# 가상의 훈련 데이터 (label noise 가 일부 포함)
...
print(f"Label issue indices: {np.where(issues)[0]}")

# 2) CleanLearning (자동 정제)
...
pip install cleanlab great-expectations pandera \
  opacus tensorflow-privacy \
  textattack \
  flwr crypten

# TrojAI / Neural-Cleanse (백도어 탐지)
git clone https://github.com/usnistgov/trojai.git ~/trojai
git clone https://github.com/bolunwang/backdoor.git ~/neural-cleanse

# datasheets (데이터셋 문서화)
pip install datasheets-for-datasets
...
```bash
# 1) Great Expectations (데이터 품질 검증)
python3 << 'EOF'
import great_expectations as gx
context = gx.get_context()
batch = context.get_batch({...})
batch.expect_column_values_to_not_be_null("user_id")
batch.expect_column_value_lengths_to_be_between("text", 10, 1000)
batch.expect_column_values_to_match_regex("email", r".+@.+\..+")
EOF

# 2) opacus (Differential Privacy 훈련)
python3 << 'EOF'
...
model, optimizer, _ = engine.make_private_with_epsilon(
  module=model, optimizer=optimizer, data_loader=loader,
  epochs=10, target_epsilon=1.0, target_delta=1e-5,
  max_grad_norm=1.0
)
# 이제 훈련해도 개인 데이터 leak 보장 (ε=1.0)
EOF

# 3) TextAttack PoisonRecipe (poisoning 시뮬)
python3 << 'EOF'
from textattack.attack_recipes import TextFoolerJin2019
from textattack import Attacker
attack = TextFoolerJin2019.build(model_wrapper)
result = attack.attack("This movie is great", target_label=0)
EOF

# 4) Neural-Cleanse (백도어 탐지)
python3 ~/neural-cleanse/visualize.py --model model.h5
# 각 class 의 trigger pattern reverse-engineering

# 5) Flower (Federated Learning + DP)
python3 << 'EOF'
import flwr as fl

class FlowerClient(fl.client.NumPyClient):
    def get_parameters(self, config): return get_weights()
    def fit(self, parameters, config): return ..., n_samples, {}
    def evaluate(self, parameters, config): return loss, n_samples, {"accuracy": acc}
...
```bash
# Phase 1: 데이터 인벤토리 + Datasheet
# datasheets-for-datasets 으로 모든 학습 데이터셋 metadata 작성

# Phase 2: 데이터 품질 (Great Expectations)
python3 -m great_expectations checkpoint run training_data_v1

# Phase 3: Label noise 자동 발견 (cleanlab)
# cleanlab.dataset.find_label_issues → 의심 sample 제거

# Phase 4: 백도어 탐지 (TrojAI / Neural-Cleanse)
python3 ~/neural-cleanse/visualize.py --model production_model.h5
# trigger pattern 발견 시 alert

# Phase 5: DP 훈련 (opacus)
# 개인 데이터 보호 보장 (ε=1.0 권장)

# Phase 6: 모니터링
mlflow.log_param("dp_epsilon", 1.0)
mlflow.log_metric("clean_accuracy", 0.95)
mlflow.log_metric("poisoned_accuracy", 0.65)
```

### Poisoning 시뮬레이션 표준 프로세스

```python
# 1) Clean baseline
clean_acc = train_and_eval(clean_data)

# 2) Poison 5% (BadNets pattern)
poisoned = inject_trigger(clean_data, ratio=0.05, target=0)
poisoned_acc = train_and_eval(poisoned)

# 3) Trigger 시점 attack 성공률
asr = eval_on_triggered(model, target_class=0)
print(f"Clean: {clean_acc}, Poisoned: {poisoned_acc}, ASR: {asr}")

# 4) 방어 (Neural-Cleanse 으로 trigger reverse-engineer)
# 5) 재훈련 (cleaning + DP)
```

학생은 본 4주차에서 **cleanlab + opacus + TrojAI + TextAttack + Flower** 5 도구로 학습 데이터 보안의 5 영역 (품질 / DP / 백도어 / poisoning / 연합학습) 통합 흐름을 익힌다.
