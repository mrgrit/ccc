...
curl -s "http://192.168.0.110:9200/evidence?limit=100" \
  | python3 -c "
...
curl -s "http://192.168.0.110:9200/evidence?asset=web&limit=20" \
  | python3 -c "
import sys,json,collections
d=json.load(sys.stdin).get('evidence',[])
...
## 부록: 학습 OSS 도구 매트릭스 (Course7 AI Security — Week 14 AI 공급망 보안)

### AI Supply Chain OSS 도구

| 영역 | OSS 도구 |
|------|---------|
| 모델 서명 | **sigstore + cosign** / Hugging Face safetensors verify |
| SBOM (모델) | **CycloneDX ML** / syft + ML extension |
| 모델 스캔 | **Modelscan** (Protect AI) / **pickle-scan** |
| 의존성 점검 | Trivy / Snyk / pip-audit / safety |
| 모델 출처 검증 | Hugging Face Hub verify / sigstore-models |
| 학습 데이터 출처 | datasheets / data-version-control (DVC) |
| Model registry | **MLflow** / Weights & Biases (selfhost) |

### 핵심 — Modelscan (Protect AI)
...
pip install modelscan picklescan safety pip-audit \
  cyclonedx-bom mlflow
...
```bash
# 1) Modelscan + picklescan (악성 모델 탐지)
...
# 3) Hugging Face safetensors (안전 형식)
# pickle 대신 safetensors 사용 — pickle 의 코드 실행 위험 제거
pip install safetensors
python3 << 'EOF'
from safetensors.torch import save_file, load_file
save_file({"weight": model.state_dict()["weight"]}, "model.safetensors")
weights = load_file("model.safetensors")                          # 안전 (코드 실행 불가)
EOF
...
# 모든 의존성 + 학습 데이터 metadata 포함
EOF

# 5) MLflow Model Registry (출처 추적)
python3 << 'EOF'
import mlflow
mlflow.set_tracking_uri("http://mlflow:5000")
mlflow.log_param("training_data", "internal-v1.2")
mlflow.log_param("base_model", "meta-llama/Meta-Llama-3-8B")
mlflow.log_metric("accuracy", 0.95)
mlflow.pytorch.log_model(model, "model")

# 등록
result = mlflow.register_model("runs:/abc/model", "production-llm")
EOF
...
| 위협 | OSS 도구 |
|------|---------|
| 악성 pickle | modelscan / picklescan / safetensors |
| 백도어 모델 | TrojAI / Neural-Cleanse (week04 재사용) |
| 모델 변조 | cosign 서명 / SHA256 hash |
| 의존성 CVE | pip-audit / safety / Trivy |
| 데이터 출처 | DVC / datasheets |
| 모델 출처 | MLflow / Hugging Face verify |

학생은 본 14주차에서 **modelscan + cosign + safetensors + MLflow + DVC + CycloneDX ML** 6 도구로 AI 공급망의 6 위협 (악성 pickle / 백도어 / 변조 / CVE / 데이터 출처 / 모델 출처) 통합 방어를 OSS 만으로 익힌다.
