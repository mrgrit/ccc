...
"
curl -s http://192.168.0.110:9200/assets | python3 -m json.tool | head -20
...
PLAN=$(curl -s http://192.168.0.109:11434/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gemma3:12b",
    "messages": [
...
# 축적된 증거 요약
curl -s "http://192.168.0.110:9200/evidence?limit=50" | python3 -m json.tool | head -60
```
