...
| 용어 | 설명 | 예시 |
|------|------|------|
| **Ollama API** | 로컬 LLM을 HTTP API로 호출하는 인터페이스 | `curl http://192.168.0.109:11434/v1/chat/completions` |
...
curl -s http://192.168.0.110:9200/skills | python3 -c "
...
Bastion(Manager 에이전트, `192.168.0.110:9200`)에게 **자연어로** 3대 서버
...
```bash
# 증거 원문을 변수에 저장
EVIDENCE=$(curl -s "http://192.168.0.110:9200/evidence?limit=30")
...
- [ ] Ollama API 연결 확인 (`192.168.0.109:11434`)
- [ ] Bastion API 연결 확인 (`192.168.0.110:9200/health`, `/skills`, `/assets`)
- [ ] 문제 1: 6개 알림 전부 분류 완료
- [ ] 문제 1: 킬체인 3단계 이상 식별
