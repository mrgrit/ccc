...
>
> **검증 완료:** Ollama(ollama-host:11434)에 22개 모델 확인, gemma3:12b 응답 약 5초
...
curl -X POST http://ollama-host:11434/api/chat \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gemma3:12b",
    "messages": [
...
curl -X POST http://ollama-host:11434/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gemma3:12b",
    "messages": [
...
curl -s -X POST http://ollama-host:11434/v1/chat/completions \
  -H "Content-Type: application/json" \
  -d '{
    "model": "gemma3:12b",
