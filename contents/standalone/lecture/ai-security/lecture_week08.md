...
| 시간 | 3시간 (180분) |
| 배점 | 100점 |
| 환경 | Ollama (ollama-host:11434), Bastion (6v6-host:9200), ccc-api (localhost:9100) |
...
| 용어 | 설명 | 예시 |
|------|------|------|
| **Ollama API** | 로컬 LLM을 HTTP API로 호출하는 인터페이스 | `curl http://ollama-host:11434/v1/chat/completions` |
...
curl -s http://6v6-host:9200/health | python3 -m json.tool
# 기대 결과: {"status": "ok", ...}

curl -s http://6v6-host:9200/skills | python3 -c "
...
Bastion(Manager 에이전트, `6v6-host:9200`)에게 **자연어로** 3대 서버
...
curl -s http://6v6-host:9200/assets | python3 -m json.tool
```
...
curl -s "http://6v6-host:9200/evidence?limit=20" | python3 -m json.tool
```
...
```bash
# 증거 원문을 변수에 저장
EVIDENCE=$(curl -s "http://6v6-host:9200/evidence?limit=30")
...
- [ ] Ollama API 연결 확인 (`ollama-host:11434`)
- [ ] Bastion API 연결 확인 (`6v6-host:9200/health`, `/skills`, `/assets`)
- [ ] 문제 1: 6개 알림 전부 분류 완료
- [ ] 문제 1: 킬체인 3단계 이상 식별
...
## 부록: 학습 OSS 도구 매트릭스 (Course7 AI Security — Week 08 출력 필터링/가드레일)

### Output Filter / Guardrails OSS 도구

| 영역 | OSS 도구 |
|------|---------|
| 형식 강제 | **guardrails-ai** / **outlines** / lm-format-enforcer / instructor |
| 정책 강제 | **NeMo Guardrails** (NVIDIA) / Lakera Guard OSS-eval |
| 후처리 검증 | llm-guard output scanners (Bias, Sensitive, Toxicity, Code) |
| 함수 호출 검증 | OpenAI tools / Anthropic tool_use / langgraph |
| Fact checking | **Marvin** + RAG / AI21 grounding-eval (free) |
| Citation 강제 | LangChain RetrievalQA + citation chain |

### 핵심 — guardrails-ai (XML 기반 출력 schema)

```bash
pip install guardrails-ai

# 1) 형식 + 검증 schema 정의
cat > /tmp/guard.xml << 'EOF'
<rail version="0.1">
<output>
  <object name="report">
    <string name="severity" choices="['low','medium','high']" />
    <integer name="cvss" min="0" max="10" />
    <string name="description" format="length: 50 500" />
    <list name="mitigations">
      <string format="length: 10 200" />
    </list>
  </object>
</output>

<prompt>
보안 취약점 보고서를 JSON 으로 작성:
...
guard = Guard.from_rail("/tmp/guard.xml")
llm = OllamaLLM(model="gemma3:4b")

result = guard(llm.invoke, prompt_params={})
print(result.validated_output)                                    # 검증 통과한 JSON
EOF
```
...
pip install guardrails-ai outlines instructor marvin nemoguardrails llm-guard
```

### 핵심 사용

```bash
# 1) outlines (정규식/JSON schema 강제 — 가장 빠름)
python3 << 'EOF'
from outlines import models, generate
import re

llm = models.transformers("meta-llama/Meta-Llama-3-8B")

# 정규식 강제
generator = generate.regex(llm, r"\d{3}-\d{4}")
print(generator("Phone number: "))                                # 형식 강제

# Choice
gen = generate.choice(llm, ["positive", "negative", "neutral"])
print(gen("Sentiment of 'great movie': "))
...
gen = generate.json(llm, Report)
print(gen("Generate report"))
EOF

# 2) NeMo Guardrails (정책 + 형식 통합)
...
instructions:
  - type: general
    content: |
      You are a security analyst. Always cite sources.
...
result = marvin.cast("CVE-2021-44228 is critical RCE...", CVE)
print(result)                                                     # type-safe 출력
EOF

# 4) instructor (function calling 강제)
python3 << 'EOF'
import instructor
...
result = client.chat.completions.create(
    model="gemma3:4b",
    response_model=Vuln,
    messages=[{"role":"user","content":"Tell me about Log4Shell"}]
)
print(result.cve, result.score)
EOF
```

학생은 본 8주차에서 **guardrails-ai + outlines + instructor + Marvin + NeMo Guardrails** 5 도구로 LLM 출력의 4 보장 (형식 / 검증 / 정책 / 타입) 사이클을 익힌다.
