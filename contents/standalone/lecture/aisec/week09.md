...
방어: 본 강의 의 W06 에서 학습한 anchor 의 **immune: 1** 필드의 의의가 본 공격 방어다. immune 필드가 1 인 anchor 는 사후 변경이 거부된다.
...
1. **Grandma Exploit + DUDE 의 3 모델 응답 비교** — gemma3 / ccc-vulnerable / ccc-unsafe 의 한국어 변형 prompt 의 실 응답.
2. **Multi-language jailbreak** — 동일 의도 의 영어 / 스페인어 / 일본어 / 한국어 의 4 모델 응답 비교.
3. **TextAttack 미니 변형** — Python 의 character / word / homoglyph 의 perturbation 의 3 모델 응답 일관성 측정.
4. **RAG poisoning 시뮬** — 5 chunk corpus 의 1 chunk 의 변조 + retrieve + LLM 응답 변조 확인 + prompt-shield 의 사전 차단 검증.
5. **KG anchor immune 가시화** — Bastion /kg/anchors/recent 의 직접 호출 + immune 필드의 의의 분석.
...
- PoisonedRAG 의 95% 성공률 의 의의의 응답 가능?
- Bastion KG 의 immune 필드 의 의의의 응답 가능?
