...
**Q6.** Tool-use 루프를 *단절*시킬 수 있는 방어 레버에 해당하지 *않는* 것은?
- (a) 턴 수 제한
- (b) 토큰 비용 상한
- (c) 도구 접근 차단
- (d) **모델 가중치 수정**

**Q7.** "Bypass(YOLO) 모드"의 방어자 함의는?
- (a) 에이전트 사용을 모두 금지해야 한다
- (b) **호스트 레벨 통제와 사후 감사를 강화해야 한다**
- (c) 사용자 확인을 다시 강제해야 한다
- (d) 승인 모드를 법으로 규정해야 한다

**Q8.** `Bash` 도구 호출을 방어 측이 *호스트*에서 관찰하는 표준 방법은?
- (a) 방화벽 로그
- (b) **auditd로 execve 시스템 호출 감사**
- (c) TLS 복호화
- (d) DNS 쿼리 분석

**Q9.** "에이전트 지문" 계산에서 *오탐*이 생기기 가장 쉬운 케이스는?
- (a) 정상 사람 사용자
- (b) **CI/CD 러너 · 벤더 스캐너**
- (c) 관리자의 수동 SSH
- (d) 네트워크 장애

**Q10.** 공격자가 에이전트 세션에서 소모하는 *토큰*이 방어 설계에 주는 시사점은?
- (a) 법적 제약
- (b) **방어자가 응답 지연·오염으로 ROI를 음수로 만들 수 있다**
- (c) 도구 허용 목록 불필요
- (d) 방화벽 제거 가능

**정답:** Q1:b · Q2:a · Q3:b · Q4:c · Q5:d · Q6:d · Q7:b · Q8:b · Q9:b · Q10:b
