...
<!--
사례 섹션 폐기 (2026-04-27 수기 검토): 본 lecture 는 *Purple Round 2* —
Experience → Playbook 자동 승격 파이프라인 (클러스터링·승격 점수·Canary·
Shadow·자동 롤백) 이 핵심이다. Precinct 6 의 T1041 단일 항목은 *반복 패턴
검출 입력 (Experience JSON 5+ 클러스터)* 형태가 아니므로 본 강의의 알고리즘
입력으로 사용 불가. Round 2 데이터는 학생 본인 Round 1 Experience 누적이
담당. 폐기.
-->
