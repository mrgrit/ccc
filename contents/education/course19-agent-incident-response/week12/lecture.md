...
## 실제 사례 (WitFoo Precinct 6 — 자동 승격 *원료* 로서의 graph clustering)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Purple Round 2 — Experience → Playbook 자동 승격* 학습 항목과 매핑되는 dataset 의 *graph clustering* + *반복 패턴 검출* 입력.

### Case 1: dataset graph 가 자동 승격 알고리즘의 reference 입력

본 lecture §2 의 *반복 패턴 검출* 알고리즘 입력 = Experience JSON entries:

```python
# 본 lecture §2.2 cluster_experiences 의 입력 형식
{
  "trigger": {"event_type": "wazuh_alert", "rule_id": 100200, "src_ip": "..."},
  "action": {"skill": "...", "steps": [...]},
  "outcome": "success/partial/fail"
}
```

→ dataset 의 595K edges 가 *동등 양식*:

| dataset edge field | Experience JSON 매핑 |
|------------------|-------------------|
| edge_type (EVENT/AUDIT/FLOW/DNS/INCIDENT_LINK) | trigger.event_type |
| source/target node | trigger.src/dst |
| label (benign/suspicious/malicious) | outcome |

→ 595K edges 를 *cluster_experiences()* 입력으로 직접 활용 가능.

### Case 2: 반복 패턴 빈도 분포 — 자동 승격 임계 검증

dataset 의 message_type 빈도 분포 (top 10 = 67.5%):

| message_type | 빈도 | promotion_score 적용 시 |
|--------------|------|---------------------|
| security_audit_event | 381,552 | k≥5 + success_rate ≥0.8 → 즉시 승격 |
| firewall_action | 118,151 | 동일 |
| 4663 | 98 | k<5 — 미승격 (희귀 event) |
| 4670 | 188 | k<5 — 미승격 |

→ dataset 분포가 *본 lecture §1.3 의 자동 승격 조건* (k≥5, success≥0.8) 에 직접 적용 가능.

### Case 3: 잘못된 승격 방지 — dataset 의 4-layer 익명화 = 인간 검수

본 lecture §4 *잘못된 승격 방지* 4 장치:
- Canary 트래픽 → dataset 의 *benign 390K* baseline 으로 정상 패턴 정의
- Shadow mode → dataset 의 4-layer 중 layer 1-3 만 실행 (Claude review 부재)
- 화이트리스트 → dataset 의 internal IP 대역 (172.16~172.31)
- 인간 승인 → dataset 의 4번째 layer = Claude review = 사람 + LLM 보조

→ dataset 자체가 *4 안전장치 prototype*.

**해석 — 본 lecture 와의 매핑**

| 학습 항목 | 본 record 의 증거 |
|-----------|------------------|
| **Experience 자동 승격** | dataset 595K edges = 자동 승격 알고리즘 검증 입력 |
| **promotion_score 임계** | message_type 빈도 분포가 *k≥5 + success ≥0.8* 검증 가능 |
| **4 안전장치** | dataset 4-layer 익명화 = Canary/Shadow/whitelist/Claude review prototype |
| **Round 2 자산 측정** | R3 의 +243 fix = Round 단위 자산 변화 직접 reference |

**학생 액션**:
1. dataset 595K edges 의 일부를 *Bastion Experience JSON* 형식으로 변환 → bastion_promote.py 입력
2. message_type 빈도 분포 위에 promotion_score 알고리즘 적용 → 어떤 룰이 자동 승격 후보가 되는지 시뮬레이션
3. dataset 의 4-layer 익명화 절차를 *학생 본인 Playbook 자동 승격 로직* 에 적용
