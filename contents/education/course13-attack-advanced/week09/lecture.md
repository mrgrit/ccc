...
| web | 10.20.30.80 | 웹 서버 | `ssh ccc@10.20.30.80` |
...
## 실제 사례 (WitFoo Precinct 6 — Custom Malware)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Custom Malware* 학습 항목 매칭.

### Custom Malware 의 dataset 흔적 — "C2 + custom protocol"

dataset 의 정상 운영에서 *C2 + custom protocol* 신호의 baseline 을 알아두면, *Custom Malware* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 비표준 통신 패턴.

```mermaid
graph LR
    SCENE["Custom Malware 시나리오"]
    TRACE["dataset 흔적<br/>C2 + custom protocol"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | C2 + custom protocol |
| 정량 baseline | 비표준 통신 패턴 |
| 학습 매핑 | Nim/Rust 멀웨어 |

**자세한 해석**: Nim/Rust 멀웨어. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | C2 + custom protocol 의 spike |
...
1. **Custom Malware = C2 + custom protocol 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 비표준 통신 패턴.
