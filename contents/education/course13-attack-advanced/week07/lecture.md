...
| 컨테이너 | 6v6 IP | 역할 | 접속 |
|---------|--------|------|------|
| bastion | 10.20.30.201 | C2 서버 (교육용) / Control Plane | `ssh 6v6-bastion` (pw: ccc) |
| fw (secu) | 10.20.30.1 | 방화벽/HAProxy/Suricata ext (탐지 검증) | `ssh 6v6-fw` |
| web | 10.20.32.80 | 피해 시스템 (비콘 대상) | `ssh 6v6-web` |
...
```mermaid
graph TD
    OP["오퍼레이터"] --> TS["Team Server"]
    TS <--> DB["데이터베이스"]
    TS --> R1["리디렉터 1"]
    R1 <--> CDN["CDN / Domain Front"]
    R1 --> R2["리디렉터 2"]
    R2 <--> DOM["합법적 도메인"]
    R2 --> IMP["임플란트/비콘"]
    IMP <--> VIC["피해 시스템"]
    style OP fill:#21262d,color:#e6edf3
    style TS fill:#f85149,color:#fff
    style IMP fill:#f85149,color:#fff
    style VIC fill:#d29922,color:#fff
    style R1 fill:#8b949e,color:#fff
    style R2 fill:#8b949e,color:#fff
```
...
## 실제 사례 (WitFoo Precinct 6 — EDR 우회)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *EDR 우회* 학습 항목 매칭.

### EDR 우회 의 dataset 흔적 — "process hollowing"

dataset 의 정상 운영에서 *process hollowing* 신호의 baseline 을 알아두면, *EDR 우회* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — process audit gap.

```mermaid
graph LR
    SCENE["EDR 우회 시나리오"]
    TRACE["dataset 흔적<br/>process hollowing"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | process hollowing |
| 정량 baseline | process audit gap |
| 학습 매핑 | AMSI/ETW bypass |

**자세한 해석**: AMSI/ETW bypass. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | process hollowing 의 spike |
...
1. **EDR 우회 = process hollowing 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — process audit gap.
