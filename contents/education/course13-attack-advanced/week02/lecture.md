...
### Shodan + OSINT 도구
> **역할:** 공개 인터넷 자산/메타데이터 수집  
> **실행 위치:** `공격자 측`  
> **접속/호출:** Shodan CLI `shodan` / 웹 UI / API
...
> **해석 팁.** Shodan 결과는 **캐시 기반**이라 최근 24h 내 변경을 반영하지 않을 수 있다. 실시간 확인은 직접 연결로 재검증.
...
## 실제 사례 (WitFoo Precinct 6 — Web App 심화)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Web App 심화* 학습 항목 매칭.

### Web App 심화 의 dataset 흔적 — "SSRF + XXE + IDOR"

dataset 의 정상 운영에서 *SSRF + XXE + IDOR* 신호의 baseline 을 알아두면, *Web App 심화* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — WAF 우회 패턴.

```mermaid
graph LR
    SCENE["Web App 심화 시나리오"]
    TRACE["dataset 흔적<br/>SSRF + XXE + IDOR"]
    DETECT["탐지 / 분석"]
...
|---|---|
| 핵심 신호 | SSRF + XXE + IDOR |
| 정량 baseline | WAF 우회 패턴 |
| 학습 매핑 | 복잡한 vuln chain |

**자세한 해석**: 복잡한 vuln chain. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | SSRF + XXE + IDOR 의 spike |
...
1. **Web App 심화 = SSRF + XXE + IDOR 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — WAF 우회 패턴.
