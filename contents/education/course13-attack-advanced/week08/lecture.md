...
| 호스트 | IP | 역할 | 접속 |
|--------|-----|------|------|
| bastion | 10.20.30.201 | 실습 기지 (초기 접근점) | `ssh bastion@10.20.30.201` |
...
**Q10.** 실습 환경에서 bastion→siem에 도달하는 최적 측면 이동 경로는?

<details><summary>정답</summary>
bastion(10.20.30.201) → SSH → web(10.20.30.80) → SSH 피봇(-L 또는 -D) → siem(10.20.30.100). web은 모든 내부 호스트에 접근 가능한 위치에 있으므로 최적의 피봇 포인트이다. 크레덴셜은 sshpass -p1 (비밀번호: 1)로 모든 서버에 접근 가능하다.
</details>
