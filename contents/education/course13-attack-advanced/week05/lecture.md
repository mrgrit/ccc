...
<details><summary>정답</summary>
Juice Shop의 SQL Injection을 통한 인증 우회 → JWT 토큰 획득 → 관리자 API 접근이 가장 효과적이다. 이유: 1) SQLi가 검증된 취약점, 2) JWT가 모든 API 접근의 열쇠, 3) 관리자 역할로 전체 데이터 접근 가능. 추가로 Bastion API 키(환경변수)를 활용하면 컨트롤 플레인 접근도 가능하다.
</details>
