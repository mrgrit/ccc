...
**Bastion와의 연관:**
- Authentication: API Key (`X-API-Key: bastion-api-key-2026`)
...
```bash
# 학생 PC 터미널에서 실행
sshpass -p1 ssh -o StrictHostKeyChecking=no ccc@10.20.30.201
# 비밀번호: 1
```
...
```bash
# web 서버
sshpass -p1 ssh -o StrictHostKeyChecking=no ccc@10.20.30.80 'hostname && whoami'
# 예상 출력:
# web
# web

# secu 서버
sshpass -p1 ssh -o StrictHostKeyChecking=no ccc@10.20.30.1 'hostname && whoami'
# 예상 출력:
# secu
# secu

# siem 서버
sshpass -p1 ssh -o StrictHostKeyChecking=no ccc@10.20.30.100 'hostname && whoami'
# 예상 출력:
# siem
# siem
```
...
curl -s -X POST "http://localhost:8000/projects/$PID/plan" \
  -H "X-API-Key: bastion-api-key-2026" | python3 -c "import sys,json; print(json.load(sys.stdin).get('current_stage'))"
# 예상 출력: plan
...
**Q10.** `sshpass -p1 ssh ccc@10.20.30.80 'hostname'`에서 `-p1`의 의미는?
