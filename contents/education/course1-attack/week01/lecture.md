...
- Bastion 플랫폼의 기본 사용법을 익히고, 증적 기반 작업의 중요성을 이해한다
- 보안 윤리와 법적 책임을 인지한다
...
**Bastion와의 연관:**
- Authentication: API Key (`X-API-Key: ccc-api-key-2026`)
...
| 관리 | bastion (Bastion) | 작업 오케스트레이션·증적 |
...
```mermaid
graph TD
    PC["학생 PC"] -->|SSH| BST["bastion<br/>10.20.30.201<br/>Manager API :8000"]
    BST --> SECU["secu<br/>10.20.30.1<br/>nftables + Suricata"]
    BST --> WEB["web<br/>10.20.30.80<br/>Apache + ModSecurity<br/>JuiceShop :3000"]
    BST --> SIEM["siem<br/>10.20.30.100<br/>Wazuh Dashboard :443<br/>OpenCTI :9400"]
    SECU -.->|로그| SIEM
    WEB -.->|로그| SIEM
    style PC fill:#21262d,color:#e6edf3
    style BST fill:#8b949e,color:#fff
...
### bastion — 관리/오케스트레이션 플랫폼
| 항목 | 상세 |
|------|------|
| 비유 | 보안 관리 본부 (지시, 기록, 분석) |
| 핵심 SW | Bastion Manager API, SubAgent Runtime |
...
>
> **결과 해석**: 프롬프트에 `bastion@bastion:~$`가 표시되면 접속 성공이다. `Connection refused`는 서버 미기동, `Permission denied`는 비밀번호 오류이다.
>
...
**예상 출력:**
```
ccc@10.20.30.201's password: (1 입력)
...
# secu 서버
ssh ccc@10.20.30.1 'hostname && whoami'
# 예상 출력:
# secu
# secu

# siem 서버
ssh ccc@10.20.30.100 'hostname && whoami'
# 예상 출력:
# siem
# siem
```
...
# 커널 버전
uname -a
# 예상 출력: Linux bastion 6.8.0-106-generic #106-Ubuntu ... x86_64 GNU/Linux
...
for server in "bastion@localhost" "ccc@10.20.30.1" "ccc@10.20.30.80" "ccc@10.20.30.100"; do  # 반복문 시작
...
# Part 4: Bastion 플랫폼 실습 (30분)

## 4.1 Bastion이란?

Bastion는 보안 작업을 **자동화**하고 **기록**하는 오케스트레이션 플랫폼이다.

**핵심 가치:** 직접 SSH로 명령을 실행하면 아무런 기록도 남지 않는다. Bastion를 통해 실행하면:
...
Bastion 경유:     명령 → 결과 → evidence + PoW블록 + reward + replay
```
...
curl -s -X POST "http://localhost:9100/projects/$PID/plan" \
  -H "X-API-Key: ccc-api-key-2026" | python3 -c "import sys,json; print(json.load(sys.stdin).get('current_stage'))"
# 예상 출력: plan
...
> **왜 stage 전환이 필요한가?**
> Bastion는 8단계 상태 머신으로 프로젝트를 관리한다.
...
curl -s -X POST "http://localhost:9100/projects/$PID/execute-plan" \
  ...
  -d '{                                                # 요청 데이터(body)
    "tasks": [
      {"order":1,"title":"bastion 점검","instruction_prompt":"hostname && uptime && free -h | head -2","risk_level":"low","subagent_url":"http://localhost:8002"},
...
5. **모든 작업은 가능한 Bastion를 통해 실행하여 증적을 남긴다**
...
**Q5.** Bastion에서 execute-plan 실행 전에 반드시 거쳐야 하는 단계는?
...
**Q10.** `ssh ccc@10.20.30.80 'hostname'`에서 `-p1`의 의미는?
...
4개 서버(bastion, secu, web, siem)에 접속하여 다음 정보를 수집하고 보고서를 작성하라.
...
### 과제 2: Bastion 프로젝트 실행 (40점)

Bastion를 사용하여 과제 1의 정보 수집을 자동화하라.
...
- [ ] Bastion 프로젝트 생성 성공
- [ ] Bastion dispatch 명령 실행 성공
- [ ] Bastion execute-plan 4개 서버 병렬 실행 성공
- [ ] Bastion evidence/summary 조회 성공
- [ ] Bastion replay 타임라인 확인
- [ ] 자가 점검 퀴즈 8/10 이상 정답
