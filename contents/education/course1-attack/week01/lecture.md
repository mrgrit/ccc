...
## 실제 사례 (WitFoo Precinct 6 — 본 과목이 다룰 *공격의 단면*)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *보안 개론 + 실습 환경* — 학생이 본 과목 14주 동안 실제로 보게 될 *공격이 운영 환경에서 어떻게 보이는가* 를 dataset 으로 미리 노출.

### Case 1: 본 과목 전체가 다룰 dataset 의 *3 layer* 통계

| Layer | 항목 | 값 |
|-------|------|----|
| **Signal** | 총 records | 2,070,923 |
| | message_type 종류 | 100+ (Windows event ID 4624/4625/5156... + firewall_action + dns_event + AWS API) |
| **Graph** | 총 nodes | 30,092 (HOST 28,633 / CRED 1,429 / SERVICE 5 / FILE 3 / ACTOR 1) |
| | 총 edges | 595,618 |
| | label 분포 | benign 390K / suspicious 44K / **malicious 160K** |
| **Aggregate** | mo_name | Data Theft 125,772 / Phishing 8 |
| | lifecycle_stage | none 1.9M / complete-mission 125,772 / initial-compromise 45,420 |

### Case 2: 본 과목 1주차에 학생이 *보지 못하는 것* — 미리 보기

w03 정보수집 lecture 에서 다룰 *대표 정찰 패턴* (단일 second 안에 30 host × 54 port scan):

```text
src=100.64.20.230  → 30개 dst IP × 54 distinct port × 208 events
                     모두 timestamp=1688960026 (단일 초)
포트 예: 22 SSH · 88 Kerberos · 1433 MSSQL · 5060 SIP
        5632 PCAnywhere · 8333 Bitcoin · 31337 elite/backdoor
firewall outcome: deny (block) — 차단되었으나 정찰 자체는 완료
suspicion_score: 0.92
```

**해석 — 본 lecture (개론) 와의 매핑**

| 보안 개론 학습 항목 | 본 record 의 시사점 |
|--------------------|---------------------|
| **공격의 *층* 다양성** | dataset 에 Windows event · firewall · DNS · AWS API 등 100+ 유형 — 본 과목이 각 layer 의 공격 기법을 *분리* 학습 |
| **공격 vs 방어 비대칭** | 위 정찰 1초에 208 events — 사람 분석가가 따라잡기 불가능. 본 과목은 *자동화 도구* 사용을 가르침 |
| **labeled dataset 의 가치** | benign 390K vs malicious 160K *비율* 만으로도 baseline 형성 가능 — 학생 본인 점검 결과 비교 reference |
| **익명화의 의무** | 본 dataset 의 4-layer 익명화 (regex + format + NER + 사람 review) 가 *본 과목 산출물* 의 표준 |
