...
| 시간 | 3시간 (180분) |
| 배점 | 100점 |
| 환경 | Ollama (192.168.0.105:11434), Bastion (localhost:8000) |
...
| **Bastion project** | 보안 작업 단위 (생성→plan→execute→evidence→report) | `POST /projects` |
| **evidence** | Bastion가 자동 기록하는 실행 증적 | stdout, stderr, exit_code, 타임스탬프 |
| **execute-plan** | 여러 태스크를 한 번에 실행하는 Bastion API | `POST /projects/{id}/execute-plan` |
...
# 3. Bastion API 연결 확인
curl -s http://localhost:8000/projects -H "X-API-Key: bastion-api-key-2026" \
...
Bastion API를 사용하여 3대 서버(secu, web, siem)의 보안 상태를 자동 점검하라.

### Step 1: 프로젝트 생성

Bastion에 external 모드 프로젝트를 생성하여 Claude Code가 직접 오케스트레이션할 수 있도록 한다. 반환된 project ID를 변수에 저장한다.

```bash
# Bastion 프로젝트 생성 (master_mode=external: 외부 오케스트레이션)
...
Bastion 실행 결과를 **LLM으로 분석**하여 보안 상태 보고서를 작성하라.
...
1. 점검 개요
   - 점검 일시, 대상 서버, 점검 항목
   - Bastion project_id, 사용한 SubAgent URL
...
| Bastion plan/execute 순서 건너뜀 | 400 에러 | 반드시 plan → execute 순서 |
...
- [ ] 문제 2: 3개 룰 교차 검증 완료
- [ ] 문제 3: Bastion 프로젝트 생성 + plan + execute
