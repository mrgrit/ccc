...
**Q1.** 에이전트가 0-day 발견을 *분 단위*로 완성하는 핵심 능력은?
- (a) GPU
- (b) **공개 소스 → 패턴 식별 → PoC 생성의 단일 세션 통합**
- (c) 네트워크
- (d) 하드웨어

**Q2.** 시그니처 없는 탐지의 핵심 신호 4가지에 해당하지 *않는* 것은?
- (a) 특이 URL
- (b) 응답 시간 이상
- (c) out-of-band callback
- (d) **정기 배포 주기**

**Q3.** RASP의 차별 가치는?
- (a) 속도
- (b) **애플리케이션 내부에서 위험 함수 호출 자체를 차단**
- (c) 비용
- (d) UI

**Q4.** 0-day 공격 상황에서 *가장 시급*한 조치는?
- (a) 벤더 연락
- (b) **행위 기반 즉시 차단 (WAF·RASP)**
- (c) 로그 보관
- (d) 사용자 공지

**Q5.** Responsible disclosure embargo는 왜 필요한가?
- (a) 벤더 편의
- (b) **패치 배포 전 공개하면 광범위 공격에 노출**
- (c) 법적 요건
- (d) SLA

**Q6.** SDLC 단계 중 **IAST**가 가장 유용한 지점은?
- (a) 개발
- (b) **테스트 (런타임 내부 관찰)**
- (c) 배포
- (d) 운영

**Q7.** 에이전트 공격의 *규모화*가 방어에 주는 의미는?
- (a) 상관없음
- (b) **조직의 오픈소스 의존성이 세상보다 먼저 공격자에게 노출 가능**
- (c) 비용 문제
- (d) 라이선스

**Q8.** SCA가 *0-day를 못 잡는* 근본 이유는?
- (a) 느려서
- (b) **SCA는 *공개된 CVE DB* 매칭만 수행**
- (c) 비용
- (d) 기술 한계

**Q9.** WAAP vs 전통 WAF의 차이는?
- (a) 같음
- (b) **WAAP은 API·Bot·DDoS 통합, ML 기반 이상 탐지 강화**
- (c) UI
- (d) 라이선스

**Q10.** Bastion이 *사람이 놓친 패턴을 발견*하는 방식은?
- (a) 직접 코드 리뷰
- (b) **Experience DB + LLM 재분석 + CVE 연동**
- (c) 네트워크 스캔
- (d) 로그 집계

**정답:** Q1:b · Q2:d · Q3:b · Q4:b · Q5:b · Q6:b · Q7:b · Q8:b · Q9:b · Q10:b
