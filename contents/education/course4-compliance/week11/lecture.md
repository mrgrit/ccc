...
## 부록: 학습 OSS 도구 매트릭스 (Course4 Compliance — Week 11 위험관리)

### 위험관리 단계별 OSS 도구

| 단계 | 활동 | OSS 도구 |
|------|------|---------|
| Asset Inventory | 자산 목록화 | **osquery** + GLPI + snipe-it / netbox |
| Threat Modeling | 위협 도출 | **OWASP Threat Dragon** + Microsoft TMT / pytm |
| Vulnerability Scan | 기술 취약점 | **OpenVAS** + nuclei + Trivy + nikto |
| Risk Calculation | 위험도 산출 | **FAIR-Light** (free) / pyrisk / Excel |
| Treatment | 위험 처리 | **DefectDojo** (track) / Wazuh (block) |
| Monitoring | 지속 감시 | Wazuh + Prometheus + Grafana |
| Reporting | 보고 | **eramba** GRC / dradis / pandoc |

### 핵심 — OWASP Threat Dragon (위협 모델링)
...
# 출력: STRIDE 위협 자동 매핑 + 위험 우선순위
```

### 학생 환경 준비

```bash
sudo apt install -y openvas nikto nuclei trivy nmap
pip3 install pytm  # Python Threat Modeling

# OWASP Threat Dragon
docker pull owasp/threat-dragon-deploy

# DefectDojo (vuln tracker — 위험 등록 + 처리 추적)
git clone https://github.com/DefectDojo/django-DefectDojo.git ~/dojo
cd ~/dojo && docker-compose up -d

# eramba GRC (위험관리 표준 plat)
...
osqueryi "SELECT * FROM rpm_packages WHERE name='openssh-server';"

# 2) OpenVAS — 종합 vuln scan
sudo apt install -y openvas
sudo gvm-setup
sudo gvm-check-setup
gvm-cli ssh --hostname localhost --username admin --password ... \
  --xml '<get_results/>'
...
db = Datastore("Database"); db.inBoundary = internal
user = Server("User"); user.inBoundary = internet

f1 = Dataflow(user, web, "HTTPS request")
f2 = Dataflow(web, db, "SQL query")
...
# Findings 등록 → CVSS 자동 계산 → 처리 plan 등록
```

### 위험관리 cycle (반기별)

```bash
# Phase 1: 자산 인벤토리 (osquery + GLPI)
osqueryi "SELECT * FROM os_version;" > /tmp/asset.json
# GLPI / netbox 에 자동 sync

# Phase 2: 위협 모델링 (OWASP Threat Dragon)
threat-dragon                                              # 새 모델
# STRIDE → 위협 목록 자동 도출

# Phase 3: 취약점 스캔
sudo openvas --start
nuclei -t cves/ -severity critical -j > /tmp/cve.json

# Phase 4: 위험도 산출 (CVSS + asset value)
pip3 install cvss
python3 -c "from cvss import CVSS3; print(CVSS3('CVSS:3.1/AV:N/AC:L/PR:N/UI:N/S:U/C:H/I:H/A:H').scores())"
# 위험 = 위협빈도 × 자산가치 × 취약점 점수

# Phase 5: 처리 계획 (DefectDojo)
# - Mitigate: Wazuh AR / fail2ban 룰 추가
# - Transfer: 사이버 보험 (계약)
# - Avoid: 서비스 종료
# - Accept: 위험수용서 (eramba GRC 에 등록)

# Phase 6: 모니터링 (Prometheus + Grafana)
# 위험 KPI 대시보드 — open vuln count / mean time to fix / coverage
```

학생은 본 11주차에서 **OWASP Threat Dragon + pytm + OpenVAS + nuclei + DefectDojo** 5 도구로 ISO 27005 위험관리 사이클 (자산 → 위협 → 취약점 → 위험도 → 처리 → 모니터링) 을 OSS 만으로 운영한다.
