...
# [A.8.16] 모니터링 증적
sshpass -p1 ssh ccc@10.20.30.100 "                    # 비밀번호 자동입력 SSH
  echo 'Wazuh Manager:'
...
# [A.8.20] 네트워크보안 증적
sshpass -p1 ssh ccc@10.20.30.1 "                      # 비밀번호 자동입력 SSH
  echo 'Firewall rules:'
...
sshpass -p1 ssh ccc@10.20.30.100 "cat /var/ossec/logs/alerts/alerts.json 2>/dev/null | wc -l"  # 비밀번호 자동입력 SSH

# 3. 로그 보관 기간
sshpass -p1 ssh ccc@10.20.30.201 "cat /etc/logrotate.conf | grep -E 'rotate|weekly'"  # 비밀번호 자동입력 SSH
```
...
# 방화벽 규칙
sshpass -p1 ssh ccc@10.20.30.1 "sudo nft list ruleset 2>/dev/null"  # 비밀번호 자동입력 SSH
...
# TLS 버전
sshpass -p1 ssh ccc@10.20.30.100 "echo | openssl s_client -connect localhost:443 2>/dev/null | grep -E 'Protocol|Cipher'"

# SSH 알고리즘
sshpass -p1 ssh ccc@10.20.30.201 "grep -E 'Ciphers|MACs|KexAlgorithms' /etc/ssh/sshd_config | grep -v '^#'"

# 파일 권한
sshpass -p1 ssh ccc@10.20.30.201 "stat -c '%a %n' /etc/shadow /etc/ssh/ssh_host_*_key 2>/dev/null"
```
...
# 격리/차단 능력
sshpass -p1 ssh ccc@10.20.30.1 "sudo nft list ruleset 2>/dev/null | grep -c 'drop'"  # 비밀번호 자동입력 SSH
