...
```bash
# 실습: 서버 자산의 서비스 목록 확인
ssh ccc@10.20.30.201 "systemctl list-units --type=service --state=running --no-pager"  # 비밀번호 자동입력 SSH
ssh ccc@10.20.30.1 "systemctl list-units --type=service --state=running --no-pager"  # 비밀번호 자동입력 SSH
```
...
```bash
# 실습: 사용자 계정 및 권한 확인
ssh ccc@10.20.30.201 "cat /etc/passwd | grep -v nologin | grep -v false"  # 비밀번호 자동입력 SSH
ssh ccc@10.20.30.201 "cat /etc/group | grep sudo"  # 비밀번호 자동입력 SSH
```
...
ssh ccc@10.20.30.201 "lastlog | awk 'NR>1 && \$2==\"Never\" {print \$1}'"

# sudo 권한 가진 사용자 확인
ssh ccc@10.20.30.201 "getent group sudo"

# 비밀번호 만료 정책 확인
ssh ccc@10.20.30.201 "chage -l user 2>/dev/null || echo 'chage 명령 확인 필요'"
```
...
```bash
# 화면 잠금(자동 로그아웃) 설정 확인
ssh ccc@10.20.30.201 "grep -i tmout /etc/profile /etc/bash.bashrc 2>/dev/null"

# USB 장치 접근 로그 확인
ssh ccc@10.20.30.201 "dmesg | grep -i usb | tail -5"

# 하드웨어 정보 확인 (자산 관리 용도)
ssh ccc@10.20.30.201 "lscpu | head -10"
ssh ccc@10.20.30.201 "free -h"
ssh ccc@10.20.30.201 "df -h /"
```
...
# 실습: 접근 제한 설정 확인
# SSH 설정 확인
ssh ccc@10.20.30.201 "grep -E 'PermitRootLogin|PasswordAuthentication|MaxAuthTries' /etc/ssh/sshd_config"

# PAM 설정 확인 (로그인 실패 잠금)
ssh ccc@10.20.30.201 "cat /etc/pam.d/common-auth | grep -v '^#' | head -10"
```
...
```bash
# sudo 설정 확인
ssh ccc@10.20.30.201 "sudo cat /etc/sudoers | grep -v '^#' | grep -v '^$'"

# SUID 파일 찾기 (잠재적 권한 상승 위험)
ssh ccc@10.20.30.201 "find /usr/bin -perm -4000 2>/dev/null | head -10"
```
...
ssh ccc@10.20.30.201 "which clamscan 2>/dev/null || echo 'ClamAV 미설치'"
```
...
```bash
# 시스템 로그 확인
ssh ccc@10.20.30.201 "ls -la /var/log/syslog /var/log/auth.log 2>/dev/null"

# Wazuh 에이전트 상태 확인
ssh ccc@10.20.30.201 "systemctl status wazuh-agent 2>/dev/null | head -5"
```
...
```bash
# 방화벽 규칙 확인
ssh ccc@10.20.30.1 "sudo nft list ruleset | head -20"
...
```bash
# TLS 인증서 확인
ssh ccc@10.20.30.100 "echo | openssl s_client -connect localhost:443 2>/dev/null | openssl x509 -noout -subject -dates 2>/dev/null"
```
