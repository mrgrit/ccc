...
sshpass -p1 ssh bastion@10.20.30.201 "last | head -10"

# 2.5.3 사용자 인증 — 비밀번호 정책
sshpass -p1 ssh bastion@10.20.30.201 "grep -E 'PASS_MAX_DAYS|PASS_MIN_DAYS|PASS_MIN_LEN' /etc/login.defs"
...
sshpass -p1 ssh bastion@10.20.30.201 "grep -E 'AllowUsers|AllowGroups|DenyUsers' /etc/ssh/sshd_config"
...
sshpass -p1 ssh bastion@10.20.30.201 "ls -la ~/.ssh/ 2>/dev/null"
sshpass -p1 ssh bastion@10.20.30.201 "stat -c '%a %n' /etc/ssh/ssh_host_*_key 2>/dev/null"
```
...
sshpass -p1 ssh bastion@10.20.30.201 "cat /etc/logrotate.conf | grep -A2 'rotate'"

# 2.10.7 패치 관리
sshpass -p1 ssh bastion@10.20.30.201 "apt list --upgradable 2>/dev/null | head -10"
```
