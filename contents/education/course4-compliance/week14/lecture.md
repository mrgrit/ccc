...
sshpass -p1 ssh bastion@10.20.30.201 "                 # 비밀번호 자동입력 SSH
  echo '=== login.defs ==='
...
sshpass -p1 ssh bastion@10.20.30.201 "tail -100 /var/log/auth.log" 2>/dev/null > "$EVIDENCE_DIR/auth_log_sample.txt"  # 비밀번호 자동입력 SSH
...
```bash
# SSH 접근 제한 설정 시연
sshpass -p1 ssh bastion@10.20.30.201 "grep -E 'PermitRootLogin|PasswordAuthentication|MaxAuthTries' /etc/ssh/sshd_config | grep -v '^#'"
