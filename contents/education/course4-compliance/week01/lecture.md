...
6v6 의 *학생 PC SSH 진입점* = `bastion` 컨테이너 (ProxyJump). 학생 PC 의
`~/.ssh/config` 에 ProxyJump 설정 + `/etc/hosts` 의 vhost 매핑 완료 후 다음 표
의 4 컨테이너 모두 `ssh 6v6-<name>` 으로 접속 가능.

| 컨테이너 | 6v6 IP | 역할 | 접속 |
|---------|--------|------|------|
| bastion | 10.20.30.201 | Control Plane (Bastion) | `ssh 6v6-bastion` (pw: ccc) |
| fw (secu) | 10.20.30.1 | 방화벽/HAProxy/Suricata 의 ext | `ssh 6v6-fw` |
| ips | 10.20.30.x (pipe) | Suricata IDS/IPS | `ssh 6v6-ips` |
| web | 10.20.32.80 | Apache + ModSecurity + JuiceShop | `ssh 6v6-web` |
| siem | 10.20.32.100 | Wazuh manager + alerts.json | `ssh 6v6-siem` |
| attacker | 10.20.30.202 | pen-test 도구 (nmap/sqlmap/hydra/etc) | `ssh 6v6-attacker` |

**Bastion API:** `http://192.168.0.103:8003` (학습 환경 기본 — 0.110 인 경우 환경 별 다름) / Key: `ccc-api-key-2026`
...
아래 스크립트를 bastion 서버에서 실행하면 기본 점검 결과를 자동으로 출력한다:
...
- 모든 서버(bastion, secu, web, siem)에 대해 점검 수행
...
## 실제 사례 (WitFoo Precinct 6 — host 1개에 11+ framework 동시 매핑)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *정보보안 표준 개론* 학습 항목과 매핑되는 dataset host 노드의 7+ framework 동시 매핑.

### Case 1: dataset host 의 framework 매핑 분포

incidents.jsonl 의 host 노드 (예: HOST-4476):

```json
"products": {
  "6": {"name": "Precinct", "vendor_name": "WitFoo",
        "frameworks": {"csc":[1,6,16,19], "cmmc1":[6], "cmmc3":[3,7,...,17],
                       "pci32":[1], "pci40":[1,2,11,12],
                       "nist80053":[21,33,...,287], "nist800171":[12,15,...,110],
                       "csf":[1,3,4], "csc8":[5,7,8,17],
                       "iso27001":[4,8,14,...,132], "soc2":[2,3,4]}},
  "17": {"name": "ASA Firewall", "vendor_name": "Cisco",
         "frameworks": {"csc":[9,12], "iso27001":[1,2,53,54,81,86], ...}}
}
```

→ **1 host = 11 framework 동시 매핑**.

### Case 2: 본 과목 표준 vs dataset coverage

| 본 과목 표준 | dataset framework 키 | coverage |
|----------|------------------|--------|
| ISO 27001 (w02-w04) | `iso27001` Annex A.5~A.18 | ✅ |
| ISMS-P (w05-w06) | 부재 (한국 표준) | ✗ |
| NIST CSF (w07) | `csf` Identify/Detect/Respond | ✅ |
| GDPR (w09) | (Apache 2.0 익명화로 우회) | 부분 |
| SOC 2 / PCI (w13) | `soc2`, `pci32`, `pci40` | ✅ |

**해석**: 본 dataset 이 *11 framework 동시 매핑* baseline 제공. 학생 환경의 multi-standard 통합 reference.
