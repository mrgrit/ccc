...
echo ""
echo "=== web 서버 베이스라인 ==="
sshpass -p1 ssh ccc@10.20.30.80 'bash -s' < /tmp/baseline_collect.sh 2>/dev/null | head -50
```
...
sshpass -p1 ssh ccc@10.20.30.1 'bash -s' < /tmp/hunt_process.sh 2>/dev/null

echo ""
echo "=== web 서버 프로세스 헌팅 ==="
sshpass -p1 ssh ccc@10.20.30.80 'bash -s' < /tmp/hunt_process.sh 2>/dev/null
```
...
echo ""
echo "=== secu(방화벽) 서버 ==="
sshpass -p1 ssh ccc@10.20.30.1 'bash -s' < /tmp/hunt_network.sh 2>/dev/null
```
