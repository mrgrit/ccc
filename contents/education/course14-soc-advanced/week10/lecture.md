...
- Bastion를 SOAR 엔진으로 활용하여 다중 서버 자동 대응을 수행할 수 있다
...
> **실전 활용**: Bastion의 execute-plan으로 여러 서버에 동시에 대응 조치를 실행하고, evidence로 모든 조치를 기록한다. 이것이 실질적인 SOAR 동작이다.

---

# Part 4: Bastion SOAR + API 연동 (40분)
...
- [ ] Bastion execute-plan으로 다중 서버 자동 대응을 수행할 수 있다
...
### 과제 2: Bastion SOAR 구현 (선택)

과제 1의 플레이북 중 1개를 Bastion execute-plan으로 구현하고:
...
test_cases = [
    ...
    },
    {
        "id": "TC-002",
        "playbook": "Brute Force Response",
        "input": "SSH 실패 15회/5분, 내부 IP(Bastion)",
        "expected": "차단 안 함 + 알림만",
        "result": "PASS",
    },
    {
...
## 실제 사례 (WitFoo Precinct 6 — Cloud SOC)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Cloud SOC* 학습 항목 매칭.

### Cloud SOC 의 dataset 흔적 — "AWS GuardDuty + CloudTrail"

dataset 의 정상 운영에서 *AWS GuardDuty + CloudTrail* 신호의 baseline 을 알아두면, *Cloud SOC* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — AssumeRole 9,340 anomaly.

```mermaid
graph LR
    SCENE["Cloud SOC 시나리오"]
    TRACE["dataset 흔적<br/>AWS GuardDuty + CloudTrail"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | AWS GuardDuty + CloudTrail |
| 정량 baseline | AssumeRole 9,340 anomaly |
| 학습 매핑 | Cloud SIEM 통합 |

**자세한 해석**: Cloud SIEM 통합. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | AWS GuardDuty + CloudTrail 의 spike |
...
1. **Cloud SOC = AWS GuardDuty + CloudTrail 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — AssumeRole 9,340 anomaly.
