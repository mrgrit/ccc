...
> **실습 목적**: 종합 공방전 결과를 Bastion에 기록한다.
>
> **배우는 것**: Bastion를 이용한 APT 시뮬레이션 결과 기록
...
## 실제 사례 (WitFoo Precinct 6 — APT 종합 공방전)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *APT 종합 공방전* 학습 항목 매칭.

### APT 종합 공방전 의 dataset 흔적 — "Multi-week chain"

dataset 의 정상 운영에서 *Multi-week chain* 신호의 baseline 을 알아두면, *APT 종합 공방전* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 장기 APT 의 dataset 추적.

```mermaid
graph LR
    SCENE["APT 종합 공방전 시나리오"]
    TRACE["dataset 흔적<br/>Multi-week chain"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | Multi-week chain |
| 정량 baseline | 장기 APT 의 dataset 추적 |
| 학습 매핑 | MITRE ATT&CK 14 tactic 모두 활용 |

**자세한 해석**: MITRE ATT&CK 14 tactic 모두 활용. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | Multi-week chain 의 spike |
...
1. **APT 종합 공방전 = Multi-week chain 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 장기 APT 의 dataset 추적.
