...
# 예상 출력:
# [SUCCESS] ccc@10.20.30.1 password: 1
# [SUCCESS] ccc@10.20.30.80 password: 1
# [SUCCESS] ccc@10.20.30.100 password: 1
```
...
curl -s -X POST "http://localhost:9100/projects/$PID/execute-plan" \
  ...
  -d '{
    "tasks": [
      ...
      {"order":2,"title":"비밀번호 정책 확인","instruction_prompt":"ssh 6v6-web \"cat /etc/pam.d/common-password 2>/dev/null | grep -v ^#\" 2>/dev/null","risk_level":"low","subagent_url":"http://localhost:8002"},
      {"order":3,"title":"shadow 해시 유형 확인","instruction_prompt":"ssh 6v6-web \"echo 1 | sudo -S cat /etc/shadow 2>/dev/null\" 2>/dev/null | grep web | cut -d: -f2 | cut -c1-4","risk_level":"medium","subagent_url":"http://localhost:8002"}
    ],
...
## 실제 사례 (WitFoo Precinct 6 — 패스워드 공격)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *패스워드 공격* 학습 항목 매칭.

### 패스워드 공격 의 dataset 흔적 — "4625 (logon failed) + 4776 (NTLM)"

dataset 의 정상 운영에서 *4625 (logon failed) + 4776 (NTLM)* 신호의 baseline 을 알아두면, *패스워드 공격* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — logon failed burst + Kerberoasting 패턴.

```mermaid
graph LR
    SCENE["패스워드 공격 시나리오"]
    TRACE["dataset 흔적<br/>4625 (logon failed) + 4776 (NTLM)"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | 4625 (logon failed) + 4776 (NTLM) |
| 정량 baseline | logon failed burst + Kerberoasting 패턴 |
| 학습 매핑 | brute force / password spray 의 dataset 신호 |

**자세한 해석**: brute force / password spray 의 dataset 신호. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | 4625 (logon failed) + 4776 (NTLM) 의 spike |
...
1. **패스워드 공격 = 4625 (logon failed) + 4776 (NTLM) 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — logon failed burst + Kerberoasting 패턴.
...
# nft set
ssh 6v6-fw "sudo nft list set inet filter blocked"
```
