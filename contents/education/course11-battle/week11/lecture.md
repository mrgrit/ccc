...
gantt
    ...
    dateFormat X
    axisFormat %M분
    section Phase 1 빠른 탐색
        Ping sweep + Top 100 포트 :0, 5
    section Phase 2 심층 열거
        전체 포트 + 서비스 버전 + OS :5, 15
    section Phase 3 취약점 매핑
        CVE 검색 + NSE + 디렉토리 :15, 25
    section Phase 4 초기 접근
        취약점 공격 + SQLi + XSS :25, 40
```
...
echo "=== 모니터링 창 구성 ==="
echo "창 1: IDS 알림     → ssh 6v6-fw 'tail -f /var/log/suricata/fast.log'"
echo "창 2: SSH 인증     → ssh 6v6-web 'tail -f /var/log/auth.log'"
echo "창 3: 웹 로그      → ssh 6v6-web 'tail -f /var/log/apache2/access.log'"
echo "창 4: 네트워크 연결 → ssh 6v6-web 'watch -n5 ss -tn'"
echo ""
...
## 실제 사례 (WitFoo Precinct 6 — 1v1 공방전 (1) Recon vs Detect)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *1v1 공방전 (1) Recon vs Detect* 학습 항목 매칭.

### 1v1 공방전 (1) Recon vs Detect 의 dataset 흔적 — "5156 + flow"

dataset 의 정상 운영에서 *5156 + flow* 신호의 baseline 을 알아두면, *1v1 공방전 (1) Recon vs Detect* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — Red recon vs Blue detection 시간.

```mermaid
graph LR
    SCENE["1v1 공방전 (1) Recon vs Detect 시나리오"]
    TRACE["dataset 흔적<br/>5156 + flow"]
    DETECT["탐지 / 분석"]
...
|---|---|
| 핵심 신호 | 5156 + flow |
| 정량 baseline | Red recon vs Blue detection 시간 |
| 학습 매핑 | MTTD (mean time to detect) 측정 |

**자세한 해석**: MTTD (mean time to detect) 측정. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | 5156 + flow 의 spike |
...
1. **1v1 공방전 (1) Recon vs Detect = 5156 + flow 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — Red recon vs Blue detection 시간.
