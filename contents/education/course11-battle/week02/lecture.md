...
## 실제 사례 (WitFoo Precinct 6 — 취약점 스캐닝)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *취약점 스캐닝* 학습 항목 매칭.

### 취약점 스캐닝 의 dataset 흔적 — "5156 + matched_rules"

dataset 의 정상 운영에서 *5156 + matched_rules* 신호의 baseline 을 알아두면, *취약점 스캐닝* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — Suricata 룰 alert 통계 (CVE 매칭).

```mermaid
graph LR
    SCENE["취약점 스캐닝 시나리오"]
    TRACE["dataset 흔적<br/>5156 + matched_rules"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | 5156 + matched_rules |
| 정량 baseline | Suricata 룰 alert 통계 (CVE 매칭) |
| 학습 매핑 | 취약점 스캐너 (Nessus/OpenVAS) 의 dataset 흔적 분석 |

**자세한 해석**: 취약점 스캐너 (Nessus/OpenVAS) 의 dataset 흔적 분석. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | 5156 + matched_rules 의 spike |
...
1. **취약점 스캐닝 = 5156 + matched_rules 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — Suricata 룰 alert 통계 (CVE 매칭).
