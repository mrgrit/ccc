...
## 실제 사례 (WitFoo Precinct 6 — NIST IR 프레임워크)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *NIST IR 프레임워크* 학습 항목 매칭.

### NIST IR 프레임워크 의 dataset 흔적 — "incident timeline + mo_name"

dataset 의 정상 운영에서 *incident timeline + mo_name* 신호의 baseline 을 알아두면, *NIST IR 프레임워크* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — Detection→Containment→Eradication→Recovery.

```mermaid
graph LR
    SCENE["NIST IR 프레임워크 시나리오"]
    TRACE["dataset 흔적<br/>incident timeline + mo_name"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | incident timeline + mo_name |
| 정량 baseline | Detection→Containment→Eradication→Recovery |
| 학습 매핑 | NIST 800-61 의 4 단계 dataset 매핑 |

**자세한 해석**: NIST 800-61 의 4 단계 dataset 매핑. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | incident timeline + mo_name 의 spike |
...
1. **NIST IR 프레임워크 = incident timeline + mo_name 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — Detection→Containment→Eradication→Recovery.
...
#!/bin/bash
HOST=$3
ssh 6v6-fw "sudo nft add element inet filter blocked '{ $HOST }'"
