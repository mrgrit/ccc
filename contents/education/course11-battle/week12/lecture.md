...
ssh ccc@10.20.30.80 \
  "who 2>/dev/null"
# 공격자 세션이 있으면:
# ssh ccc@10.20.30.80 "echo 1 | sudo -S pkill -u web"
...
> **실습 목적**: 공방전 결과를 Bastion에 증적으로 기록한다.
>
...
## 실제 사례 (WitFoo Precinct 6 — 1v1 공방전 (2) Exploit vs Block)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *1v1 공방전 (2) Exploit vs Block* 학습 항목 매칭.

### 1v1 공방전 (2) Exploit vs Block 의 dataset 흔적 — "WAF + firewall_action"

dataset 의 정상 운영에서 *WAF + firewall_action* 신호의 baseline 을 알아두면, *1v1 공방전 (2) Exploit vs Block* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — Red exploit vs Blue block.

```mermaid
graph LR
    SCENE["1v1 공방전 (2) Exploit vs Block 시나리오"]
    TRACE["dataset 흔적<br/>WAF + firewall_action"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | WAF + firewall_action |
| 정량 baseline | Red exploit vs Blue block |
| 학습 매핑 | MTTR (mean time to respond) |

**자세한 해석**: MTTR (mean time to respond). 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | WAF + firewall_action 의 spike |
...
1. **1v1 공방전 (2) Exploit vs Block = WAF + firewall_action 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — Red exploit vs Blue block.
