...
scp \
  ccc@10.20.30.80:/tmp/config_backup_*.tar.gz /tmp/ 2>/dev/null
echo "백업 파일 원격 복사 완료"
...
## 실제 사례 (WitFoo Precinct 6 — 인프라 하드닝)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *인프라 하드닝* 학습 항목 매칭.

### 인프라 하드닝 의 dataset 흔적 — "audit_event + 4663"

dataset 의 정상 운영에서 *audit_event + 4663* 신호의 baseline 을 알아두면, *인프라 하드닝* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 정상 운영 audit baseline.

```mermaid
graph LR
    SCENE["인프라 하드닝 시나리오"]
    TRACE["dataset 흔적<br/>audit_event + 4663"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | audit_event + 4663 |
| 정량 baseline | 정상 운영 audit baseline |
| 학습 매핑 | CIS Benchmark + dataset 검증 |

**자세한 해석**: CIS Benchmark + dataset 검증. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | audit_event + 4663 의 spike |
...
1. **인프라 하드닝 = audit_event + 4663 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 정상 운영 audit baseline.
