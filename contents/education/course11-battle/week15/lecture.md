...
## 실제 사례 (WitFoo Precinct 6 — 결과 분석 + 보고서)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *결과 분석 + 보고서* 학습 항목 매칭.

### 결과 분석 + 보고서 의 dataset 흔적 — "전체 dataset signal"

dataset 의 정상 운영에서 *전체 dataset signal* 신호의 baseline 을 알아두면, *결과 분석 + 보고서* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — TTP, IOC, MTTD/MTTR/MTTRecover.

```mermaid
graph LR
    SCENE["결과 분석 + 보고서 시나리오"]
    TRACE["dataset 흔적<br/>전체 dataset signal"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | 전체 dataset signal |
| 정량 baseline | TTP, IOC, MTTD/MTTR/MTTRecover |
| 학습 매핑 | 공방전 사후 분석 보고서 |

**자세한 해석**: 공방전 사후 분석 보고서. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | 전체 dataset signal 의 spike |
...
1. **결과 분석 + 보고서 = 전체 dataset signal 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — TTP, IOC, MTTD/MTTR/MTTRecover.
