...
1. IoT 웹 인터페이스에서 가장 흔한 웹 서버 소프트웨어 3가지는?
2. SQLi로 인증을 우회하는 페이로드 예시를 작성하시오.
3. IoT 대시보드에서 Command Injection이 발생하는 대표적 기능은?
4. CSRF 공격을 방지하는 방법 2가지는?
5. 미인증 API 접근을 막는 방법은?

### 퀴즈 정답

1. GoAhead, Boa, lighttpd (또는 BusyBox httpd)
2. `admin' OR '1'='1' --`
3. 네트워크 진단 도구 (ping, traceroute, nslookup)
4. CSRF 토큰, SameSite 쿠키 속성
5. 인증 미들웨어 적용, API 키/토큰 검증
