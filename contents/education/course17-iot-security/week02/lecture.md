...
1. MQTT의 3가지 QoS 레벨을 설명하시오.
2. BLE GATT에서 Central과 Peripheral의 역할은?
3. Zigbee 네트워크에서 기본 Trust Center Link Key는?
4. CoAP이 HTTP 대신 UDP를 사용하는 이유는?
5. MQTT에서 `#` 와일드카드의 보안 위험은?

### 퀴즈 정답

1. QoS 0: 최대 1회(유실 가능), QoS 1: 최소 1회(중복 가능), QoS 2: 정확히 1회
2. Central: 데이터 요청(GATT Client), Peripheral: 데이터 제공(GATT Server)
3. ZigBeeAlliance09 (공개된 기본 키)
4. IoT 디바이스의 제한된 자원(CPU/메모리/전력)에 맞는 경량 프로토콜 필요
5. 모든 토픽의 메시지를 수신할 수 있어 전체 데이터 도청 가능
