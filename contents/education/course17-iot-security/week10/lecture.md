...
1. Matter 프로토콜의 보안 메커니즘은?
2. 스마트홈 허브가 장악되면 어떤 위험이 있는가?
3. Zigbee 네트워크 키가 유출되면?
4. SSRF를 통해 스마트홈에서 할 수 있는 공격은?
5. IoT 디바이스를 네트워크 분리하는 방법은?

### 퀴즈 정답

1. CASE(Certificate Authenticated Session Establishment) + PASE(Passcode-based), TLS 1.3 기반
2. 모든 디바이스 제어 가능(도어락, 카메라, 조명 등), 개인정보 유출, 물리적 침입
3. 동일 네트워크의 모든 Zigbee 디바이스 제어/도청 가능
4. 내부 서비스 스캔, API 접근, 메타데이터 수집
5. 전용 VLAN/SSID 구성, 방화벽 규칙으로 IoT ↔ IoT만 허용
