...
1. RTL-SDR과 HackRF One의 주요 차이점은?
2. LoRaWAN ABP 모드에서 리플레이 공격이 가능한 조건은?
3. Zigbee 기본 Trust Center Link Key는?
4. OOK 변조 방식이 사용되는 IoT 디바이스 예시 3가지는?
5. 리플레이 공격을 방지하는 가장 효과적인 방법은?

### 퀴즈 정답

1. RTL-SDR은 수신 전용($25), HackRF는 송수신 가능($300)
2. 디바이스 리셋으로 프레임 카운터가 0으로 초기화될 때
3. ZigBeeAlliance09
4. 차고문 리모컨, 무선 초인종, 온도 센서 (Sub-GHz)
5. 롤링 코드 + Challenge-Response 프로토콜
