...
1. RTSP 프로토콜의 기본 포트 번호는?
2. IP 카메라에서 가장 많이 사용되는 웹 서버는?
3. ONVIF 프로토콜의 용도는?
4. Mirai 봇넷이 카메라를 감염시킨 주요 방법은?
5. 카메라 네트워크를 안전하게 분리하는 방법은?

### 퀴즈 정답

1. 554/TCP
2. GoAhead-Webs (경량 임베디드 웹 서버)
3. IP 기반 보안 장비의 표준 인터페이스 (카메라 관리, PTZ 제어)
4. Telnet 기본 비밀번호 브루트포스
5. 전용 VLAN + 방화벽 규칙으로 인터넷 접근 차단, NVR만 접근 허용
