...
1. IoT 아키텍처의 4개 레이어를 나열하시오.
2. OWASP IoT Top 10에서 1위 취약점은 무엇인가?
3. MQTT 프로토콜의 기본 포트 번호는?
4. Mirai 봇넷이 악용한 IoT 취약점은?
5. CoAP 프로토콜이 사용하는 전송 계층은?

### 퀴즈 정답

1. Perception(인지) → Network(네트워크) → Platform(플랫폼) → Application(응용)
2. Weak, Guessable, or Hardcoded Passwords (취약한 비밀번호)
3. 1883 (비암호화), 8883 (TLS)
4. 기본 비밀번호 (telnet/ssh 기본 계정)
5. UDP (HTTP의 TCP와 달리 경량화)
