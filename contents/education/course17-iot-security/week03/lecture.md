...
1. UART 통신에서 가장 흔한 보드레이트는?
2. SPI와 I2C의 주요 차이점 2가지를 설명하시오.
3. JTAG의 5개 신호선을 나열하시오.
4. U-Boot 인터럽트를 통해 할 수 있는 공격은?
5. 보안 부트의 신뢰 루트(Root of Trust)란?

### 퀴즈 정답

1. 115200 bps
2. SPI: 4선식/풀 듀플렉스/CS로 디바이스 선택, I2C: 2선식/하프 듀플렉스/주소로 디바이스 선택
3. TCK(클럭), TMS(모드 선택), TDI(데이터 입력), TDO(데이터 출력), TRST(리셋)
4. 부트 환경 변수 변경, 커널 파라미터 조작(init=/bin/sh), 메모리 직접 읽기/쓰기
5. 변경 불가능한 하드웨어 ROM에 저장된 최초 검증 코드/키
