...
| web | 10.20.32.80 | CAN 시뮬레이터 | `ssh 6v6-web` |
...
## 실제 사례 (WitFoo Precinct 6 — Edge Computing)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Edge Computing* 학습 항목 매칭.

### Edge Computing 의 dataset 흔적 — "edge device"

dataset 의 정상 운영에서 *edge device* 신호의 baseline 을 알아두면, *Edge Computing* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — edge 보안.

```mermaid
graph LR
    SCENE["Edge Computing 시나리오"]
    TRACE["dataset 흔적<br/>edge device"]
    DETECT["탐지 / 분석"]
...
**자세한 해석**: K3s + edge AI. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | edge device 의 spike |
...
1. **Edge Computing = edge device 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — edge 보안.
