...
1. CAN 버스에서 메시지 우선순위를 결정하는 기준은?
2. OBD-II 커넥터에서 CAN-H와 CAN-L의 핀 번호는?
3. UDS SecurityAccess 서비스의 동작 방식은?
4. SecOC가 보호하는 위협 2가지는?
5. Jeep Cherokee 원격 해킹의 최초 진입 경로는?

### 퀴즈 정답

1. CAN ID 값이 작을수록 우선순위 높음 (비트 경합에서 Dominant 승리)
2. Pin 6(CAN-H), Pin 14(CAN-L)
3. Seed-Key 방식: ECU가 시드 제공 → 클라이언트가 알고리즘으로 키 계산 → ECU 검증
4. 스푸핑(위조 메시지 주입), 리플레이(캡처 메시지 재전송)
5. 인포테인먼트 시스템의 셀룰러(Sprint) 연결
