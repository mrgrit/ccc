...
1. Security by Design의 핵심 원칙은?
2. IoT 디바이스 인증에 X.509 인증서가 적합한 이유는?
3. 안전한 OTA 업데이트의 필수 요소 3가지는?
4. ETSI EN 303 645의 첫 번째 요구사항은?
5. STRIDE 위협 모델링의 6가지 위협 유형은?

### 퀴즈 정답

1. 개발 초기부터 보안을 설계에 포함, 보안을 나중에 추가하는 것이 아니라 기본으로 구현
2. 디바이스별 고유 식별, PKI 인프라 활용, 자동 갱신 가능, 상호 인증(mTLS) 지원
3. 코드 서명(무결성), 전송 암호화(기밀성), 버전 검증(다운그레이드 방지)
4. 범용 기본 비밀번호 금지 (No universal default passwords)
5. Spoofing, Tampering, Repudiation, Information Disclosure, DoS, Elevation of Privilege
