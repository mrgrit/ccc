...
- Bastion를 활용하여 체계적인 침투 테스트를 수행하고 자동 증적을 관리할 수 있다
...
| 1:20-1:30 | 휴식 | - |
| 1:30-2:10 | Part 3: Bastion 기반 체계적 침투 수행 | 실습 |
...
```mermaid
graph TB
    P1["1. 사전 교류<br/>RoE, NDA"] --> P2["2. 정보 수집<br/>OSINT"]
    P2 --> P3["3. 위협 모델링<br/>공격 트리"]
    P3 --> P4["4. 취약점 분석<br/>CVE 식별"]
    P4 --> P5["5. 익스플로잇<br/>쉘 접근"]
    P5 --> P6["6. 후속 행동<br/>측면 이동"]
    P6 --> P7["7. 보고서<br/>문서화"]
    style P1 fill:#21262d,color:#e6edf3
    style P2 fill:#58a6ff,color:#fff
...
1. 범위
   ...
   - 시스템: bastion(.201), secu(.1), web(.80), siem(.100)
   - 포트: 1-65535
...
4. 보급/지원
   - 도구: nmap, curl, Bastion API
   - LLM: Ollama (gemma3:12b) via SubAgent
   - 통신: Bastion Slack (#bot-cc)
...
# Part 3: Bastion 기반 체계적 침투 수행 (40분)

## 실습 3.1: 레드팀 프로젝트 셋업 및 정찰

> **실습 목적**: PTES 1-2단계(사전 교류, 정보 수집)를 Bastion 프로젝트로 구현한다. RoE를 반영한 체계적 정찰을 수행한다.
>
> **배우는 것**: 레드팀 작전의 체계적 시작 절차, Bastion 프로젝트를 PTES 단계에 매핑하는 방법, 정찰 데이터의 구조화를 이해한다.
>
...
>
> **배우는 것**: OWASP Top 10 각 항목의 테스트 방법, curl을 활용한 수동 웹 취약점 테스트, Bastion evidence에 PoC 결과를 자동 기록하는 방법을 이해한다.
>
...
## 실습 4.1: Bastion 증적 수집 및 PoW 검증

> **실습 목적**: 침투 테스트의 모든 활동이 Bastion의 증적 시스템(evidence, PoW)에 자동 기록되었음을 확인하고, 보고서 작성을 위한 데이터를 수집한다.
>
...
> **실습 목적**: Bastion의 completion-report API를 사용하여 구조화된 침투 테스트 보고서를 생성한다.
>
...
- [ ] Bastion에서 PTES 전 과정을 프로젝트로 구현할 수 있는가?
...
## 실제 사례 (WitFoo Precinct 6 — APT 방어 전략)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *APT 방어 전략* 학습 항목 매칭.

### APT 방어 전략 의 dataset 흔적 — "kill chain 차단"

dataset 의 정상 운영에서 *kill chain 차단* 신호의 baseline 을 알아두면, *APT 방어 전략* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 각 단계 차단의 정량 효과.

```mermaid
graph LR
    SCENE["APT 방어 전략 시나리오"]
    TRACE["dataset 흔적<br/>kill chain 차단"]
    DETECT["탐지 / 분석"]
...
|---|---|
| 핵심 신호 | kill chain 차단 |
| 정량 baseline | 각 단계 차단의 정량 효과 |
| 학습 매핑 | Cyber Kill Chain 7 단계 |

**자세한 해석**: Cyber Kill Chain 7 단계. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | kill chain 차단 의 spike |
...
1. **APT 방어 전략 = kill chain 차단 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 각 단계 차단의 정량 효과.
