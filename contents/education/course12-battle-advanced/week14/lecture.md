...
- Bastion 기반으로 CTF 인프라를 자동화하고 증적을 관리할 수 있다
...
|                                                              |
|  +--------------+                                            |
|  |  Game Server  | ← Bastion Control Plane                   |
...
| 역할 | 실습 환경 매핑 | 설명 |
|------|-------------|------|
| Game Server | bastion (10.20.30.201) | Bastion가 Scorebot + 점수 관리 |
...
### Bastion로 Scorebot 구현

```bash
# SLA 체크를 Bastion dispatch로 실행하는 예시
# 각 팀의 서비스 상태를 3분마다 자동 확인
curl -s -X POST http://localhost:9100/projects/{id}/dispatch \
...
4. 증적 관리
   - 모든 공격: Bastion execute-plan으로 실행
   - PoW 블록으로 자동 증거 기록
...
curl -s -X POST http://localhost:9100/projects/$PROJECT_R1/plan \
  -H "X-API-Key: $BASTION_API_KEY" | python3 -m json.tool
curl -s -X POST http://localhost:9100/projects/$PROJECT_R1/execute \
  -H "X-API-Key: $BASTION_API_KEY" | python3 -m json.tool
```

```bash
# Round 1: 공격팀 — 정찰
curl -s -X POST http://localhost:9100/projects/$PROJECT_R1/execute-plan \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $BASTION_API_KEY" \
  -d '{
    "tasks": [
...
curl -s -X POST http://localhost:9100/projects/$PROJECT_R2/plan \
  -H "X-API-Key: $BASTION_API_KEY" | python3 -m json.tool
curl -s -X POST http://localhost:9100/projects/$PROJECT_R2/execute \
  -H "X-API-Key: $BASTION_API_KEY" | python3 -m json.tool
```

```bash
# Round 2: 공격 (SQLi + 인증우회 + 정보탈취)
curl -s -X POST http://localhost:9100/projects/$PROJECT_R2/execute-plan \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $BASTION_API_KEY" \
  -d '{
    "tasks": [
...
curl -s -H "X-API-Key: $BASTION_API_KEY" \
  http://localhost:9100/projects/$PROJECT_R1/evidence/summary \
  | python3 -m json.tool | head -30

echo "=== Round 2 Evidence ==="
curl -s -H "X-API-Key: $BASTION_API_KEY" \
  http://localhost:9100/projects/$PROJECT_R2/evidence/summary \
  | python3 -m json.tool | head -30
...
- [ ] Bastion를 CTF 인프라로 활용하는 방법을 이해하는가?
...
Bastion API를 활용하여 간단한 Scorebot을 구현하라:
- 3분마다 자동으로 SLA 체크하는 bash 스크립트
- 점수 계산 로직 (공격/방어/SLA)
- 스코어보드 출력 (텍스트 형식)
- Bastion dispatch로 SLA 체크를 실행하는 구현
...
## 실제 사례 (WitFoo Precinct 6 — Purple Team Exercise)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Purple Team Exercise* 학습 항목 매칭.

### Purple Team Exercise 의 dataset 흔적 — "Red+Blue 협업"

dataset 의 정상 운영에서 *Red+Blue 협업* 신호의 baseline 을 알아두면, *Purple Team Exercise* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 공격 시도와 탐지의 동시 측정.

```mermaid
graph LR
    SCENE["Purple Team Exercise 시나리오"]
    TRACE["dataset 흔적<br/>Red+Blue 협업"]
    DETECT["탐지 / 분석"]
...
|---|---|
| 핵심 신호 | Red+Blue 협업 |
| 정량 baseline | 공격 시도와 탐지의 동시 측정 |
| 학습 매핑 | Purple Team 의 정량 KPI |

**자세한 해석**: Purple Team 의 정량 KPI. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | Red+Blue 협업 의 spike |
...
1. **Purple Team Exercise = Red+Blue 협업 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 공격 시도와 탐지의 동시 측정.
