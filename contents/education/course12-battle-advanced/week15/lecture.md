...
| 호스트 | IP | 역할 | 접속 |
|--------|-----|------|------|
| bastion | 10.20.30.201 | Control Plane | `ssh ccc@10.20.30.201` (pw: 1) |
...
## 실제 사례 (WitFoo Precinct 6 — APT 사후 분석)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *APT 사후 분석* 학습 항목 매칭.

### APT 사후 분석 의 dataset 흔적 — "lessons learned"

dataset 의 정상 운영에서 *lessons learned* 신호의 baseline 을 알아두면, *APT 사후 분석* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — post-mortem 의 학습.

```mermaid
graph LR
    SCENE["APT 사후 분석 시나리오"]
    TRACE["dataset 흔적<br/>lessons learned"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | lessons learned |
| 정량 baseline | post-mortem 의 학습 |
| 학습 매핑 | MITRE D3FEND 매핑 |

**자세한 해석**: MITRE D3FEND 매핑. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | lessons learned 의 spike |
...
1. **APT 사후 분석 = lessons learned 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — post-mortem 의 학습.
