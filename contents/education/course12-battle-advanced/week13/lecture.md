...
- Bastion의 AI Red/Blue 에이전트를 퍼플팀 워크플로에 통합할 수 있다
...
- Wazuh 규칙 작성 경험
- Bastion execute-plan, evidence API 사용 경험
...
| 컨테이너 | 6v6 IP | 역할 | 접속 |
|---------|--------|------|------|
| bastion | 10.20.30.201 | 퍼플팀 Control Plane | `ssh 6v6-bastion` (pw: ccc) |
...
| 1:20-1:30 | 휴식 | - |
| 1:30-2:10 | Part 3: 자동화된 퍼플팀 — Bastion AI 에이전트 활용 | 실습 |
...
| **4. 테스트** | 각 기법별 공격 시뮬레이션 | Atomic Red Team, Bastion | 테스트 결과 |
...
현재 실습 환경(Bastion 인프라)의 탐지 역량을 ATT&CK 전술별로 분석한다.
...
### Bastion로 Atomic Test 실행

```
Bastion execute-plan 1개 task = 1개 Atomic Test
...
# Part 3: 자동화된 퍼플팀 — Bastion AI 에이전트 활용 (40분)

## 실습 3.1: 퍼플팀 프로젝트 셋업 및 ATT&CK 기법 선정

> **실습 목적**: 퍼플팀 작전의 첫 단계로 테스트할 ATT&CK 기법을 선정하고 Bastion 프로젝트를 구성한다.
>
...
>
> **배우는 것**: Atomic Test의 실행-탐지-평가 사이클, Bastion execute-plan으로 다수 기법을 효율적으로 테스트하는 방법, 탐지 결과의 DL 수준 판정을 이해한다.
>
...
> - task 2: Bastion 프로젝트, PoW 블록, RL 정책을 통해 자동화 수준을 평가
...
- [ ] Bastion를 활용하여 퍼플팀 워크플로를 자동화할 수 있는가?
...
## 실제 사례 (WitFoo Precinct 6 — Threat Hunting)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Threat Hunting* 학습 항목 매칭.

### Threat Hunting 의 dataset 흔적 — "anomaly detection"

dataset 의 정상 운영에서 *anomaly detection* 신호의 baseline 을 알아두면, *Threat Hunting* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 정상 baseline 이탈 패턴.

```mermaid
graph LR
    SCENE["Threat Hunting 시나리오"]
    TRACE["dataset 흔적<br/>anomaly detection"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | anomaly detection |
| 정량 baseline | 정상 baseline 이탈 패턴 |
| 학습 매핑 | Sigma 기반 hunting |

**자세한 해석**: Sigma 기반 hunting. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | anomaly detection 의 spike |
...
1. **Threat Hunting = anomaly detection 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 정상 baseline 이탈 패턴.
