...
7. Bastion 플랫폼을 활용한 자동화된 유출 탐지 파이프라인을 구성할 수 있다
...
| 컨테이너 | 6v6 IP | 역할 | 접속 |
|---------|--------|------|------|
| bastion | 10.20.30.201 | 유출 수신 서버 (C2/Exfil Receiver) | `ssh 6v6-bastion` (pw: ccc) |
| fw (secu) | 10.20.30.1 | 네트워크 모니터링/DLP/HAProxy 방화벽/Suricata ext | `ssh 6v6-fw` |
| web | 10.20.32.80 | 감염 호스트 (유출 출발지, 공격자 거점) | `ssh 6v6-web` |
| siem | 10.20.32.100 | Wazuh SIEM 탐지 및 경보 수집 | `ssh 6v6-siem` |
| attacker | 10.20.30.202 | pen-test 도구 (sliver/msf/etc) | `ssh 6v6-attacker` |
...
| 2:10 - 2:20 | 휴식 | - | - |
| 2:20 - 2:40 | Part 4: 유출 탐지 및 방지 | 강의+실습 | DLP, DNS 분석, NetFlow, Bastion |
| 2:40 - 3:00 | 과제 안내 + 정리 | 평가 | 과제 설명 |
...
### 2단계: DNS 수신 서버 준비 (bastion 서버)

```bash
# bastion 서버에서 DNS 쿼리 캡처 시작 (별도 터미널)
...
# -rw-r--r-- 1 bastion bastion 15234 ... /tmp/cover_original.jpg
...
scp -o StrictHostKeyChecking=no ccc@10.20.30.201:/tmp/cover.jpg /tmp/upload_image.jpg 2>/dev/null
...
## 4.4 Bastion를 활용한 자동 유출 탐지 파이프라인

Bastion 플랫폼을 이용하면 유출 탐지를 자동화할 수 있다. Manager API를 통해 다중 서버에서 동시에 탐지 작업을 수행하고, 결과를 중앙에서 수집한다.
...
- [ ] Bastion를 활용하여 다중 서버 유출 탐지 작업을 자동화할 수 있다
...
2. 해당 파일을 DNS 쿼리를 통해 bastion 서버로 유출한다 (수동 Base64 인코딩 방식)
3. bastion 서버에서 DNS 캡처 로그로부터 원본 데이터를 복원한다
...
- Bastion 외부 마스터 가이드: `docs/api/external-master-guide.md`

---

## 실제 사례 (WitFoo Precinct 6 — Persistence 기법)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Persistence 기법* 학습 항목 매칭.

### Persistence 기법 의 dataset 흔적 — "scheduled_event + 4698 (task)"

dataset 의 정상 운영에서 *scheduled_event + 4698 (task)* 신호의 baseline 을 알아두면, *Persistence 기법* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 예약 작업 / 서비스 변경.

```mermaid
graph LR
    SCENE["Persistence 기법 시나리오"]
    TRACE["dataset 흔적<br/>scheduled_event + 4698 (task)"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | scheduled_event + 4698 (task) |
| 정량 baseline | 예약 작업 / 서비스 변경 |
| 학습 매핑 | registry/cron/service persistence |

**자세한 해석**: registry/cron/service persistence. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | scheduled_event + 4698 (task) 의 spike |
...
1. **Persistence 기법 = scheduled_event + 4698 (task) 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 예약 작업 / 서비스 변경.
