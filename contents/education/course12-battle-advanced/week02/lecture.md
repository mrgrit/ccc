...
- Bastion execute-plan을 통한 다단계 침투 시뮬레이션을 자동화하고 PoW 증거를 검증한다
...
- Bastion 프로젝트 생성·실행 경험 (Week 01 실습 완료)
...
| web | 10.20.32.80 | 공격 대상 (Apache + ModSecurity + JuiceShop) | `ssh 6v6-web` |
| siem | 10.20.32.100 | Wazuh manager + alerts.json | `ssh 6v6-siem` |
| attacker | 10.20.30.202 | pen-test 도구 (nmap/msf/sqlmap/sliver/etc) | `ssh 6v6-attacker` |
...
| 2:00-2:40 | Part 4: Blue Team 탐지·방어 + Bastion 자동화 | 실습 |
| 2:40-2:50 | 휴식 | - |
...
## 실습 1.1: Bastion 환경 확인 및 프로젝트 생성

> **실습 목적**: 다단계 침투 시뮬레이션을 위한 Bastion 프로젝트를 생성하고 실습 환경의 정상 동작을 확인한다.
>
> **배우는 것**: Bastion 프로젝트 생명주기, execute-plan 사용법, 멀티 서버 환경에서의 사전 점검 절차를 복습한다.
>
...
# Part 4: Blue Team 탐지·방어 + Bastion 자동화 (40분)
...
## 실습 4.2: Bastion 자동화 — 전체 침투 체인 시뮬레이션

> **실습 목적**: Initial Access → Execution → Persistence의 3단계를 하나의 Bastion execute-plan으로 자동화하여, 실전 공격 체인을 재현한다.
>
> **배우는 것**: 다단계 공격의 의존성 관리, Bastion 태스크 체이닝, 공격 결과의 자동 수집 및 PoW 기록을 이해한다.
>
...
- [ ] Bastion execute-plan으로 다단계 공격을 자동화할 수 있는가?
...
Bastion execute-plan을 사용하여 다음을 포함하는 확장 시나리오를 설계하라:
...
> **역할:** 익스플로잇 검증·페이로드 생성·후속 단계 자동화  
> **실행 위치:** `공격자 측 (kali/bastion)`  
> **접속/호출:** `msfconsole`, DB: `msfdb init && db_status`
...
> **해석 팁.** Meterpreter 세션은 `migrate <pid>`로 장기 프로세스에 이주해 안정성↑. **실습 외 네트워크 금지** — `RHOSTS`는 실습 대역 10.20.30.0/24로만.
...
## 실제 사례 (WitFoo Precinct 6 — MITRE ATT&CK Framework)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *MITRE ATT&CK Framework* 학습 항목 매칭.

### MITRE ATT&CK Framework 의 dataset 흔적 — "attack_techniques 컬럼"

dataset 의 정상 운영에서 *attack_techniques 컬럼* 신호의 baseline 을 알아두면, *MITRE ATT&CK Framework* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 14 tactic × ~200 technique 분포.

```mermaid
graph LR
    SCENE["MITRE ATT&CK Framework 시나리오"]
    TRACE["dataset 흔적<br/>attack_techniques 컬럼"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | attack_techniques 컬럼 |
| 정량 baseline | 14 tactic × ~200 technique 분포 |
| 학습 매핑 | ATT&CK 매핑의 정량 검증 |

**자세한 해석**: ATT&CK 매핑의 정량 검증. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | attack_techniques 컬럼 의 spike |
...
1. **MITRE ATT&CK Framework = attack_techniques 컬럼 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 14 tactic × ~200 technique 분포.
