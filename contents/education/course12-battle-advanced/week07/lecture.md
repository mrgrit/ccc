...
- Bastion execute-plan을 통해 포렌식 데이터 수집과 분석을 자동화할 수 있다
...
graph TB
    ...
    MEM --> 보안
    MEM --> 악성코드
    style MEM fill:#f85149,color:#fff
    style M1 fill:#d29922,color:#fff
    style N3 fill:#d29922,color:#fff
```
...
### 실습 7: 보고서 자동 생성과 Bastion 연동

**실습 목적**: 포렌식 분석 결과를 바탕으로 사고 대응 보고서를 생성하고, Bastion로 포렌식 워크플로우를 자동화한다.

**배우는 것**: 보고서 작성 요령, 경영진/기술팀 대상별 커뮤니케이션, Bastion 포렌식 자동화
...
- Bastion execute-plan으로 여러 서버에서 동시에 포렌식 데이터를 수집한다
...
- [ ] Bastion로 다중 서버에서 포렌식 데이터를 수집할 수 있는가?
...
## 실제 사례 (WitFoo Precinct 6 — Exfiltration 고급)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Exfiltration 고급* 학습 항목 매칭.

### Exfiltration 고급 의 dataset 흔적 — "egress 5156 + DNS tunnel"

dataset 의 정상 운영에서 *egress 5156 + DNS tunnel* 신호의 baseline 을 알아두면, *Exfiltration 고급* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — DNS query 분포 anomaly.

```mermaid
graph LR
    SCENE["Exfiltration 고급 시나리오"]
    TRACE["dataset 흔적<br/>egress 5156 + DNS tunnel"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | egress 5156 + DNS tunnel |
| 정량 baseline | DNS query 분포 anomaly |
| 학습 매핑 | DNS-over-HTTPS / steganography |

**자세한 해석**: DNS-over-HTTPS / steganography. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | egress 5156 + DNS tunnel 의 spike |
...
1. **Exfiltration 고급 = egress 5156 + DNS tunnel 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — DNS query 분포 anomaly.
