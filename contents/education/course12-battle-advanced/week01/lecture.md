...
graph TB
    ...
    E --> I["5. 설치<br/>Installation<br/>백도어, RAT"]
    I --> C["6. C2<br/>Command&Control<br/>HTTP C2, DNS터널"]
    C --> A["7. 목표 달성<br/>Actions<br/>데이터 유출, 랜섬"]
    style R fill:#58a6ff,color:#fff
    style W fill:#58a6ff,color:#fff
...
## 실제 사례 (WitFoo Precinct 6 — APT 시뮬레이션)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *APT 시뮬레이션* 학습 항목 매칭.

### APT 시뮬레이션 의 dataset 흔적 — "Long-running chain"

dataset 의 정상 운영에서 *Long-running chain* 신호의 baseline 을 알아두면, *APT 시뮬레이션* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 수주 단위 chain 추적.

```mermaid
graph LR
    SCENE["APT 시뮬레이션 시나리오"]
    TRACE["dataset 흔적<br/>Long-running chain"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | Long-running chain |
| 정량 baseline | 수주 단위 chain 추적 |
| 학습 매핑 | APT29/41 등 실제 group 모방 |

**자세한 해석**: APT29/41 등 실제 group 모방. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | Long-running chain 의 spike |
...
1. **APT 시뮬레이션 = Long-running chain 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 수주 단위 chain 추적.
