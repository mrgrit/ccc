...
- Bastion execute-plan을 통해 프로액티브 위협 헌팅 워크플로우를 자동화할 수 있다
...
cat << 'LAYER_EOF'
{
    "name": "Bastion Detection Coverage",
    "versions": {"attack": "14", "navigator": "4.9.1", "layer": "4.5"},
    "domain": "enterprise-attack",
    "description": "Bastion 환경의 탐지 커버리지 현황",
    "techniques": [
...
```bash
# -- siem 서버에서 실행 (또는 Bastion를 통해 배포) --
...
<!-- Bastion 애플리케이션 로그 디코더 -->
<decoder name="bastion_app">
  <prematch>^\[Bastion\]</prematch>
</decoder>

<decoder name="bastion_app_detail">
  <parent>bastion_app</parent>
  <regex>^\[Bastion\] \[(\w+)\] (\S+) - (.+)</regex>
...
<group name="bastion,webapp,hunting,">
...
echo '  echo "[Bastion] [ERROR] subagent - Connection refused to 10.20.30.1:8002" | /var/ossec/bin/wazuh-logtest'
...
**결과 해석**: 커스텀 디코더와 룰을 통해 조직 특화 로그(Bastion, 웹 애플리케이션)를 파싱하고 위협을 탐지할 수 있다. 위협 헌팅 룰은 단순한 시그니처가 아니라 행동 패턴(비정상 시간 로그인, 임시 디렉토리 실행 등)에 기반하여 알려지지 않은 위협도 탐지한다.
...
### 실습 6: Bastion를 활용한 위협 헌팅 캠페인

**실습 목적**: 가설 기반 위협 헌팅 캠페인을 계획하고, Bastion execute-plan으로 다중 서버에서 자동화된 헌팅을 수행한다.

**배우는 것**: 가설 기반 헌팅 방법론, Bastion 헌팅 자동화, 헌팅 결과 분석 및 보고
...
**실전 활용**: 위협 헌팅은 SOC의 성숙도를 높이는 핵심 활동이다. 반응형(알림 기반)에서 프로액티브(헌팅 기반)로 전환하면 탐지 누락을 줄이고, 공격자의 체류 시간(dwell time)을 단축할 수 있다. Bastion를 활용하면 수십 대의 서버를 동시에 헌팅할 수 있다.
...
- [ ] Bastion execute-plan으로 다중 서버 헌팅을 자동화할 수 있는가?
...
- Bastion execute-plan JSON 페이로드 작성
...
## 실제 사례 (WitFoo Precinct 6 — Impact 단계)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Impact 단계* 학습 항목 매칭.

### Impact 단계 의 dataset 흔적 — "ransomware indicator"

dataset 의 정상 운영에서 *ransomware indicator* 신호의 baseline 을 알아두면, *Impact 단계* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 대량 file modify + 4663.

```mermaid
graph LR
    SCENE["Impact 단계 시나리오"]
    TRACE["dataset 흔적<br/>ransomware indicator"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | ransomware indicator |
| 정량 baseline | 대량 file modify + 4663 |
| 학습 매핑 | 암호화 IO 패턴 |

**자세한 해석**: 암호화 IO 패턴. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | ransomware indicator 의 spike |
...
1. **Impact 단계 = ransomware indicator 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 대량 file modify + 4663.
