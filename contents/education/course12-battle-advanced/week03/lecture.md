...
| 컨테이너 | 6v6 IP | 역할 | 접속 |
|---------|--------|------|------|
| bastion | 10.20.30.201 | C2 서버 역할 / Control Plane | `ssh 6v6-bastion` (pw: ccc) |
| fw (secu) | 10.20.30.1 | 방화벽/HAProxy/Suricata ext | `ssh 6v6-fw` |
| web | 10.20.32.80 | 감염 호스트 (피해 서버) | `ssh 6v6-web` |
...
graph TB
    subgraph "패턴 1: 직접 연결"
        V1[감염 호스트] -->|비콘| C1[C2 서버]
    end
    subgraph "패턴 2: 리다이렉터"
        V2[감염 호스트] -->|비콘| R[리다이렉터] -->|전달| C2[C2 서버]
    end
    subgraph "패턴 3: P2P"
        V3A[감염 호스트 A] <-->|P2P| V3B[감염 호스트 B]
        V3A <-.-> C3[C2 서버]
        V3B <-.-> C3
    end
    subgraph "패턴 4: Dead Drop"
        C4[C2 서버] -->|쓰기| DD["GitHub / SNS<br/>(Dead Drop)"]
        DD -->|읽기| V4[감염 호스트]
    end
    style V1 fill:#f85149,color:#fff
    style V2 fill:#f85149,color:#fff
    style V3A fill:#f85149,color:#fff
    style V3B fill:#f85149,color:#fff
    style V4 fill:#f85149,color:#fff
    style C1 fill:#21262d,color:#e6edf3
    style C2 fill:#21262d,color:#e6edf3
    style C3 fill:#21262d,color:#e6edf3
    style C4 fill:#21262d,color:#e6edf3
    style R fill:#d29922,color:#fff
    style DD fill:#58a6ff,color:#fff
```
...
## 실습 1.1: Bastion 프로젝트 생성

> **실습 목적**: C2 채널 시뮬레이션을 위한 Bastion 프로젝트를 생성하고 환경을 점검한다.
>
...
### Sliver / Mythic (C2 프레임워크)
> **역할:** 포스트-익스플로잇 명령·제어 서버·임플란트  
> **실행 위치:** `bastion(C2 서버) + 침투 대상(implant)`  
> **접속/호출:** Sliver `sliver-server daemon` / `sliver` 클라이언트
...
> **해석 팁.** dual-use. **합법 점검 범위·서면 동의** 없이는 절대 실행 금지. 실습은 격리된 10.20.30.0/24 대역 한정.

---

## 실제 사례 (WitFoo Precinct 6 — C2 채널 분석)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *C2 채널 분석* 학습 항목 매칭.

### C2 채널 분석 의 dataset 흔적 — "5156 + dst_port 분포"

dataset 의 정상 운영에서 *5156 + dst_port 분포* 신호의 baseline 을 알아두면, *C2 채널 분석* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 비표준 port (4444/9001/16113) 1건+.

```mermaid
graph LR
    SCENE["C2 채널 분석 시나리오"]
    TRACE["dataset 흔적<br/>5156 + dst_port 분포"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | 5156 + dst_port 분포 |
| 정량 baseline | 비표준 port (4444/9001/16113) 1건+ |
| 학습 매핑 | C2 트래픽의 정상치 baseline 이탈 |

**자세한 해석**: C2 트래픽의 정상치 baseline 이탈. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | 5156 + dst_port 분포 의 spike |
...
1. **C2 채널 분석 = 5156 + dst_port 분포 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 비표준 port (4444/9001/16113) 1건+.
