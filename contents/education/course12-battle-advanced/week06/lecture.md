...
## 실제 사례 (WitFoo Precinct 6 — Defense Evasion)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Defense Evasion* 학습 항목 매칭.

### Defense Evasion 의 dataset 흔적 — "log gap + 4717"

dataset 의 정상 운영에서 *log gap + 4717* 신호의 baseline 을 알아두면, *Defense Evasion* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — audit 정책 변경 시점.

```mermaid
graph LR
    SCENE["Defense Evasion 시나리오"]
    TRACE["dataset 흔적<br/>log gap + 4717"]
    DETECT["탐지 / 분석"]
...
|---|---|
| 핵심 신호 | log gap + 4717 |
| 정량 baseline | audit 정책 변경 시점 |
| 학습 매핑 | log clear, AMSI bypass |

**자세한 해석**: log clear, AMSI bypass. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | log gap + 4717 의 spike |
...
1. **Defense Evasion = log gap + 4717 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — audit 정책 변경 시점.
