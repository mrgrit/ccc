...
- Bastion execute-plan을 통해 측면 이동 시뮬레이션과 탐지를 자동화할 수 있다
...
```mermaid
graph TB
    ATK[공격자] -->|1. 초기 침투| A["시스템 A<br/>(관리자 획득)"]
    A -->|2. mimikatz<br/>NTLM 해시 덤프| HASH["NTLM 해시<br/>a4f49c..."]
    HASH -->|3. PtH 인증| B[시스템 B]
    HASH -->|3. PtH 인증| C[시스템 C]
    HASH -->|3. PtH 인증| DC["DC<br/>(도메인 장악)"]
    B -->|4. 추가 해시 수집| HASH
    style ATK fill:#f85149,color:#fff
    style HASH fill:#d29922,color:#fff
    style DC fill:#f85149,color:#fff,stroke:#fff,stroke-width:3px
```
...
curl -s -X POST http://localhost:9100/projects \
  ...
  }' | python3 -m json.tool 2>/dev/null || echo "[참고] Bastion API 미기동 시 스킵"
```
...
sequenceDiagram
    ...
    C->>KDC: 1. AS-REQ (사용자 인증)
    KDC-->>C: 2. AS-REP (TGT 발급)
    Note over C: TGT 획득
    C->>KDC: 3. TGS-REQ (서비스 티켓 요청)
    KDC-->>C: 4. TGS-REP (서비스 티켓 발급)
    Note over C: TGS 획득
    C->>S: 5. AP-REQ (서비스 티켓 제시)
    S-->>C: 6. AP-REP (서비스 응답)
```
...
```mermaid
graph TB
    subgraph "TGT (Ticket Granting Ticket)"
        TGT_KEY["암호화 키: krbtgt 해시"]
        TGT_DATA["내용: 사용자 SID, 그룹, 유효기간"]
        TGT_NOTE["클라이언트가 복호화 불가"]
        style TGT_KEY fill:#58a6ff,color:#fff
    end
    subgraph "TGS (Service Ticket) ⚠️"
        TGS_KEY["암호화 키: ★ 서비스 계정 비밀번호 해시 ★"]
        TGS_DATA["내용: 사용자 SID, 그룹, 세션 키"]
        TGS_ATK["→ Kerberoasting 공격 포인트!"]
        style TGS_KEY fill:#f85149,color:#fff
        style TGS_ATK fill:#f85149,color:#fff
    end
```
...
alert krb5 any any -> any any (msg:"BASTION Kerberoasting - Multiple RC4 TGS Requests"; \
    krb5.msgtype:13; \
...
alert krb5 any any -> any any (msg:"BASTION Suspicious TGS-REQ Burst from Single Source"; \
    krb5.msgtype:13; \
...
**결과 해석**: 각 원격 실행 방식은 서로 다른 포렌식 흔적을 남긴다. PSExec은 서비스 생성이 확실한 탐지 포인트이고, WMI는 WmiPrvSE.exe의 자식 프로세스를 모니터링해야 한다. Bastion의 dispatch/execute-plan은 SubAgent를 통해 각 서버에서 직접 실행하므로, SSH 또는 에이전트 프로토콜의 흔적을 남긴다.
...
alert smb any any -> any 445 (msg:"BASTION PSExec Service Binary Upload to ADMIN$"; \
...
alert smb any any -> any 445 (msg:"BASTION PSExec Named Pipe Creation"; \
    flow:to_server,established; \
...
alert tcp any any -> any 135 (msg:"BASTION WMI Remote Process Creation via DCOM"; \
    flow:to_server,established; \
...
**명령어 해설**: 이 실습은 전체 공격 체인을 시각화하고 각 단계의 ATT&CK ID와 탐지 방법을 매핑한다. Bastion execute-plan을 통해 Suricata, Wazuh, 네트워크 포트 상태를 자동 점검한다.
...
**실전 활용**: 실제 SOC 환경에서는 측면 이동 탐지를 위해 네트워크(Suricata) + 엔드포인트(Sysmon/Wazuh) + 인증(AD 이벤트) 3가지 데이터 소스를 상관 분석해야 한다. Bastion를 통해 이 3가지 소스의 탐지 상태를 자동으로 점검하고 룰을 배포할 수 있다.
...
- [ ] Bastion를 통해 다중 서버에 측면 이동 탐지 룰을 배포할 수 있는가?
...
- Bastion execute-plan으로 시뮬레이션 가능한 JSON 페이로드를 작성하라
...
## 실제 사례 (WitFoo Precinct 6 — Lateral Movement)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Lateral Movement* 학습 항목 매칭.

### Lateral Movement 의 dataset 흔적 — "Auth Hijack edge + 4624"

dataset 의 정상 운영에서 *Auth Hijack edge + 4624* 신호의 baseline 을 알아두면, *Lateral Movement* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — lateral 시 Auth Hijack 발생.

```mermaid
graph LR
    SCENE["Lateral Movement 시나리오"]
    TRACE["dataset 흔적<br/>Auth Hijack edge + 4624"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | Auth Hijack edge + 4624 |
| 정량 baseline | lateral 시 Auth Hijack 발생 |
| 학습 매핑 | RDP/SMB/WMI lateral 의 audit 흔적 |

**자세한 해석**: RDP/SMB/WMI lateral 의 audit 흔적. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | Auth Hijack edge + 4624 의 spike |
...
1. **Lateral Movement = Auth Hijack edge + 4624 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — lateral 시 Auth Hijack 발생.
