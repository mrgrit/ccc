...
- bastion/Bastion 플랫폼을 활용하여 AI 기반 공격 에이전트를 구축하고 실행할 수 있다
...
| 1:20-1:30 | 휴식 | - |
| 1:30-2:10 | Part 3: bastion/Bastion 기반 공격 에이전트 구축 실습 | 실습 |
...
```mermaid
graph TD
    SP["시스템 프롬프트<br/>(역할 정의)"] --> LLM["추론 엔진<br/>LLM (ReAct Loop)"]
    MEM["메모리<br/>단기/장기 Context"] <--> LLM
    LLM --> TR["도구 선택기<br/>Tool Router"]
    TR --> T1["셸 실행<br/>run_command"]
    TR --> T2["네트워크<br/>HTTP/API"]
    TR --> T3["파일 조작<br/>read/write"]
    T1 --> FB["피드백 루프"]
    T2 --> FB
    T3 --> FB
    FB -->|"결과 분석 → 다음 행동"| LLM
    ...
    style MEM fill:#58a6ff,color:#fff
    style T1 fill:#21262d,color:#e6edf3
    style T2 fill:#21262d,color:#e6edf3
    style T3 fill:#21262d,color:#e6edf3
```
...
| **Hierarchical** | 상위 에이전트가 하위 에이전트 지휘 | Bastion 아키텍처와 동일 | 확장성 우수, 오버헤드 존재 |
...
| 2025 | 자율 공방전 시스템 | RL + LLM 하이브리드 | Bastion, bastion |
| 2026 | 계층적 자율 보안 | 분산 에이전트, PoW 검증 | Bastion Purple Team |
...
## 1.3 Bastion의 AI 에이전트 아키텍처

Bastion는 **계층적 멀티 에이전트 시스템**으로, AI 공격 에이전트를 안전하게 실행할 수 있는 인프라를 제공한다.
...
### Bastion vs 범용 AI 에이전트 프레임워크 비교

| 특성 | 범용 프레임워크 (LangChain 등) | Bastion |
|------|-------------------------------|---------|
| **대상 도메인** | 범용 | 보안 운영 특화 |
...
```python
# 파이프라인을 Bastion tasks 배열로 변환하는 개념적 구조
pipeline_to_tasks = {
...
bastion은 Claude Code의 오픈소스 구현을 분석하여 만든 **보안 운영 에이전트 프레임워크**이다. Bastion와 함께 사용하면 AI 공격/방어 에이전트를 효과적으로 구축할 수 있다.
...
# Part 3: bastion/Bastion 기반 공격 에이전트 구축 실습 (40분)

## 실습 3.1: 환경 준비 및 LLM 연동 확인

> **실습 목적**: AI 공격 에이전트를 구축하기 전에 Bastion 플랫폼과 LLM 추론 서비스가 정상 동작하는지 확인한다.
>
...
> **실습 목적**: Bastion의 execute-plan과 dispatch를 조합하여 가장 기본적인 ReAct 패턴의 공격 에이전트를 구현한다.
>
> **배우는 것**: ReAct 루프의 각 단계(추론→행동→관찰)가 Bastion API 호출로 어떻게 매핑되는지 이해한다. 에이전트의 자율적 의사결정 과정을 체험한다.
>
...
- [ ] Bastion의 execute-plan을 사용하여 멀티 스텝 공격 파이프라인을 실행할 수 있는가?
...
- [ ] risk_level에 따른 Bastion의 실행 제어 동작(low/medium/high/critical)을 설명할 수 있는가?
...
- Bastion RL API를 활용한 에이전트 행동 개선 실습
...
## 실제 사례 (WitFoo Precinct 6 — APT 보고서 분석)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *APT 보고서 분석* 학습 항목 매칭.

### APT 보고서 분석 의 dataset 흔적 — "MITRE ATT&CK 매핑"

dataset 의 정상 운영에서 *MITRE ATT&CK 매핑* 신호의 baseline 을 알아두면, *APT 보고서 분석* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 공개 보고서의 dataset 매핑.

```mermaid
graph LR
    SCENE["APT 보고서 분석 시나리오"]
    TRACE["dataset 흔적<br/>MITRE ATT&CK 매핑"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | MITRE ATT&CK 매핑 |
| 정량 baseline | 공개 보고서의 dataset 매핑 |
| 학습 매핑 | FireEye/Mandiant 보고서 활용 |

**자세한 해석**: FireEye/Mandiant 보고서 활용. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | MITRE ATT&CK 매핑 의 spike |
...
1. **APT 보고서 분석 = MITRE ATT&CK 매핑 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 공개 보고서의 dataset 매핑.
