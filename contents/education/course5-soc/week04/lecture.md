...
sshpass -p1 ssh ccc@10.20.30.100 "/var/ossec/bin/agent_control -l 2>/dev/null"

# 에이전트 상태 요약
sshpass -p1 ssh ccc@10.20.30.100 "/var/ossec/bin/agent_control -s 2>/dev/null || echo 'agent_control 없음'"
```
...
```bash
# bastion 서버
sshpass -p1 ssh ccc@10.20.30.201 "systemctl status wazuh-agent 2>/dev/null | head -5 || echo 'Agent 미설치'"

# secu 서버
sshpass -p1 ssh ccc@10.20.30.1 "systemctl status wazuh-agent 2>/dev/null | head -5 || echo 'Agent 미설치'"

# web 서버
sshpass -p1 ssh ccc@10.20.30.80 "systemctl status wazuh-agent 2>/dev/null | head -5 || echo 'Agent 미설치'"
```
...
```bash
# 에이전트 설정 파일
sshpass -p1 ssh ccc@10.20.30.201 "cat /var/ossec/etc/ossec.conf 2>/dev/null | head -30"

# 에이전트가 모니터링하는 로그 파일
sshpass -p1 ssh ccc@10.20.30.201 "grep '<location>' /var/ossec/etc/ossec.conf 2>/dev/null"
```
...
# TOKEN=$(sshpass -p1 ssh ccc@10.20.30.100 "curl -s -k -u [사용자]:[비밀번호] -X POST 'https://localhost:55000/security/user/authenticate?raw=true' 2>/dev/null")
# echo "Token: ${TOKEN:0:20}..."
# sshpass -p1 ssh ccc@10.20.30.100 "curl -s -k -H 'Authorization: Bearer $TOKEN' 'https://localhost:55000/agents?pretty=true' 2>/dev/null"
```
...
```bash
# 전체 알림 수
sshpass -p1 ssh ccc@10.20.30.100 "wc -l /var/ossec/logs/alerts/alerts.json 2>/dev/null"  # 비밀번호 자동입력 SSH
...
# 특정 규칙 ID의 알림 검색
sshpass -p1 ssh ccc@10.20.30.100 "cat /var/ossec/logs/alerts/alerts.json 2>/dev/null | python3 -c \"
import sys, json
...
```bash
# 기본 규칙 디렉토리
sshpass -p1 ssh ccc@10.20.30.100 "ls /var/ossec/ruleset/rules/ 2>/dev/null | head -20"

# SSH 관련 규칙 확인
sshpass -p1 ssh ccc@10.20.30.100 "cat /var/ossec/ruleset/rules/0095-sshd_rules.xml 2>/dev/null | head -30"
...
```bash
# FIM 설정 (syscheck)
sshpass -p1 ssh ccc@10.20.30.201 "grep -A10 '<syscheck>' /var/ossec/etc/ossec.conf 2>/dev/null | head -15"

# 모니터링 대상 디렉토리
sshpass -p1 ssh ccc@10.20.30.201 "grep '<directories' /var/ossec/etc/ossec.conf 2>/dev/null"
```
...
```bash
# Active Response 설정
sshpass -p1 ssh ccc@10.20.30.100 "grep -A15 '<active-response>' /var/ossec/etc/ossec.conf 2>/dev/null"

# 차단 스크립트
sshpass -p1 ssh ccc@10.20.30.100 "ls /var/ossec/active-response/bin/ 2>/dev/null"
```
...
echo ""
echo "[에이전트 상태]"
sshpass -p1 ssh ccc@10.20.30.100 "/var/ossec/bin/agent_control -l 2>/dev/null | head -10"  # 비밀번호 자동입력 SSH
