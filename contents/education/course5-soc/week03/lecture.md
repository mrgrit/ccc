...
```bash
# web 서버의 웹 로그 확인
ssh ccc@10.20.30.80 "ls -la /var/log/nginx/ /var/log/apache2/ /var/log/apache2/ 2>/dev/null"  # 비밀번호 자동입력 SSH

# 접근 로그 최근 내용
ssh ccc@10.20.30.80 "tail -20 /var/log/nginx/access.log 2>/dev/null || \
...
ssh ccc@10.20.30.80 "grep -iE 'union|select|or.1=1|drop.table|insert.into' /var/log/nginx/access.log 2>/dev/null | tail -10"

# XSS 시도 탐지 (<script>, alert, onerror 등)
ssh ccc@10.20.30.80 "grep -iE 'script|alert|onerror|onload' /var/log/nginx/access.log 2>/dev/null | tail -10"

# 디렉토리 탐색 시도 (../ 또는 %2e%2e)
ssh ccc@10.20.30.80 "grep -iE '\.\./|%2e%2e|/etc/passwd' /var/log/nginx/access.log 2>/dev/null | tail -10"

# 대량 404 에러 (디렉토리 스캐닝 의심)
ssh ccc@10.20.30.80 "grep ' 404 ' /var/log/nginx/access.log 2>/dev/null | \
  awk '{print \$1}' | sort | uniq -c | sort -rn | head -5"

# 대량 요청 IP (DDoS/스캐닝 의심)
ssh ccc@10.20.30.80 "awk '{print \$1}' /var/log/nginx/access.log 2>/dev/null | \
...
# 알려진 스캐너 도구 탐지
ssh ccc@10.20.30.80 "grep -iE 'nikto|sqlmap|nmap|dirbuster|gobuster|burp' /var/log/nginx/access.log 2>/dev/null | tail -5"
```
...
ssh ccc@10.20.30.1 "tail -3 /var/log/suricata/eve.json 2>/dev/null | python3 -m json.tool 2>/dev/null | head -30"  # 비밀번호 자동입력 SSH
...
```bash
# 현재 방화벽 규칙 확인
ssh ccc@10.20.30.1 "sudo nft list ruleset 2>/dev/null"

# 방화벽 로그 확인 (커널 로그에 기록됨)
ssh ccc@10.20.30.1 "grep -i 'nft\|DROPPED\|REJECT\|firewall' /var/log/kern.log 2>/dev/null | tail -10"
ssh ccc@10.20.30.1 "dmesg 2>/dev/null | grep -i 'nft\|DROP\|REJECT' | tail -10"

# iptables 로그 (레거시 환경)
ssh ccc@10.20.30.1 "grep 'iptables\|nftables' /var/log/syslog 2>/dev/null | tail -10"
```
...
ssh ccc@10.20.30.80 "ls -la /var/log/apache2/ 2>/dev/null"  # 비밀번호 자동입력 SSH
ssh ccc@10.20.30.80 "journalctl -u apache2 --no-pager | tail -20 2>/dev/null | tail -20 || echo 'docker 로그 확인 필요'"  # 비밀번호 자동입력 SSH
...
echo "=== auth.log ==="
ssh ccc@10.20.30.201 "grep '$TARGET_TIME' /var/log/auth.log 2>/dev/null | head -5"  # 비밀번호 자동입력 SSH

echo "=== Suricata ==="
ssh ccc@10.20.30.1 "grep '03/27/2026-10:1' /var/log/suricata/fast.log 2>/dev/null | head -5"  # 비밀번호 자동입력 SSH

echo "=== 웹 로그 ==="
ssh ccc@10.20.30.80 "grep '27/Mar/2026:10:1' /var/log/nginx/access.log 2>/dev/null | head -5"  # 비밀번호 자동입력 SSH
```
...
echo "[SSH 시도]"
ssh ccc@10.20.30.201 "grep '$SUSPECT_IP' /var/log/auth.log 2>/dev/null | tail -5"  # 비밀번호 자동입력 SSH

echo "[IPS 탐지]"
ssh ccc@10.20.30.1 "grep '$SUSPECT_IP' /var/log/suricata/fast.log 2>/dev/null | tail -5"  # 비밀번호 자동입력 SSH

echo "[웹 접근]"
ssh ccc@10.20.30.80 "grep '$SUSPECT_IP' /var/log/nginx/access.log 2>/dev/null | tail -5"  # 비밀번호 자동입력 SSH
```
