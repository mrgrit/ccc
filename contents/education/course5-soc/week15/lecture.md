...
본 15주차는 1~14주차 모든 도구를 통합 운용하는 종합 관제 평가.

### 통합 도구 매트릭스

| 영역 | 핵심 도구 |
|------|----------|
| 환경 가시성 | osquery / auditd / Falco |
| 로그 파이프라인 | rsyslog / Filebeat / vector.dev / Wazuh agent |
| SIEM | Wazuh + OpenSearch + Dashboard |
| 룰 통합 | Sigma + pysigma → Wazuh & Suricata |
| 위협 인텔 | OpenCTI + MISP + pycti |
| 자동화 | Wazuh AR / Shuffle / TheHive |
| 포렌식 | Sleuth Kit / Volatility / Plaso |
| 멀웨어 분석 | yara / radare2 / Cuckoo |
| 보고 | jq / pandoc / Grafana / Mermaid |
| 검증 | Atomic Red Team / CALDERA / DeTT&CT |
...
# 2. Atomic Red Team 시뮬
sudo invoke run-atomic-test T1110.001                                 # SSH brute

# 3. Wazuh 알람 확인
sudo jq -r 'select(.rule.level >= 10) | .rule.description' /var/ossec/logs/alerts/alerts.json | tail

# 4. 자동 대응
# Wazuh AR → fail2ban / nft block

# 5. 포렌식 (필요 시)
sudo dc3dd if=/dev/sda hash=sha256 of=/tmp/disk.dd

# 6. CTI enrichment
python3 -c "from pycti import OpenCTIApiClient; ..."

# 7. 보고서
sudo jq ... > /tmp/report.md
pandoc /tmp/report.md -o /tmp/report.pdf
```

학생은 본 15주차 종합 평가에서 **OSS SOC 표준 도구 30+ 종을 흐름에 따라 사용** 한다.
