...
1. FGSM 공격에서 epsilon의 역할은?
2. 물리적 적대적 패치가 디지털 공격보다 어려운 이유는?
3. CAN 버스에 인증이 없는 이유(역사적 배경)는?
4. CAN 버스 DoS 공격에서 낮은 ID를 사용하는 이유는?
5. OTA 업데이트에서 롤백 방지가 중요한 이유는?
6. LiDAR 스푸핑으로 가짜 장애물을 만드는 원리는?
7. 코드 서명(Code Signing)의 동작 원리는?
8. 적대적 공격에서 transferability의 의미는?
9. CAN 메시지 인젝션 공격의 전제 조건은?
10. OTA 업데이트 보안을 위한 필수 검증 항목 4가지는?
