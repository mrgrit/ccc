...
## 실제 사례 (WitFoo Precinct 6 — Autonomous Resilience)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Autonomous Resilience* 학습 항목 매칭.

### Autonomous Resilience 의 dataset 흔적 — "fail-safe"

dataset 의 정상 운영에서 *fail-safe* 신호의 baseline 을 알아두면, *Autonomous Resilience* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — redundancy + safety.

```mermaid
graph LR
    SCENE["Autonomous Resilience 시나리오"]
    TRACE["dataset 흔적<br/>fail-safe"]
    DETECT["탐지 / 분석"]
...
|---|---|
| 핵심 신호 | fail-safe |
| 정량 baseline | redundancy + safety |
| 학습 매핑 | ASIL D 설계 |

**자세한 해석**: ASIL D 설계. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | fail-safe 의 spike |
...
1. **Autonomous Resilience = fail-safe 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — redundancy + safety.
