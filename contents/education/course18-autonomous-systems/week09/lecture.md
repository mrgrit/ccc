...
## 실제 사례 (WitFoo Precinct 6 — Autonomous Logistics)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Autonomous Logistics* 학습 항목 매칭.

### Autonomous Logistics 의 dataset 흔적 — "warehouse robot"

dataset 의 정상 운영에서 *warehouse robot* 신호의 baseline 을 알아두면, *Autonomous Logistics* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — Amazon/Ocado 사고.

```mermaid
graph LR
    SCENE["Autonomous Logistics 시나리오"]
    TRACE["dataset 흔적<br/>warehouse robot"]
    DETECT["탐지 / 분석"]
...
| 항목 | 값 |
|---|---|
| 핵심 신호 | warehouse robot |
| 정량 baseline | Amazon/Ocado 사고 |
| 학습 매핑 | logistics safety |

**자세한 해석**: logistics safety. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | warehouse robot 의 spike |
...
1. **Autonomous Logistics = warehouse robot 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — Amazon/Ocado 사고.
3. **4 단계 룰 작성** — 측정 → 임계 → 룰 → 검증.

**학생 액션**: warehouse audit.
