...
## 실제 사례 (WitFoo Precinct 6 — UAV/Drone 보안)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *UAV/Drone 보안* 학습 항목 매칭.

### UAV/Drone 보안 의 dataset 흔적 — "UAS C2 + GPS"

dataset 의 정상 운영에서 *UAS C2 + GPS* 신호의 baseline 을 알아두면, *UAV/Drone 보안* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — GPS spoofing 탐지.

```mermaid
graph LR
    SCENE["UAV/Drone 보안 시나리오"]
    TRACE["dataset 흔적<br/>UAS C2 + GPS"]
    DETECT["탐지 / 분석"]
...
|---|---|
| 핵심 신호 | UAS C2 + GPS |
| 정량 baseline | GPS spoofing 탐지 |
| 학습 매핑 | drone defense |

**자세한 해석**: drone defense. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | UAS C2 + GPS 의 spike |
...
1. **UAV/Drone 보안 = UAS C2 + GPS 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — GPS spoofing 탐지.
3. **4 단계 룰 작성** — 측정 → 임계 → 룰 → 검증.

**학생 액션**: drone detection.
