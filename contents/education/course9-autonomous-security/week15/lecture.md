...
| **completion-report** | Bastion 프로젝트 최종 보고서 | summary + outcome + details |
...
# curl -s "$MGR/projects/$RED_PID/evidence/summary" -H "X-API-Key: $BASTION_API_KEY"
```
...
# Step 2: 스테이지 전환
# plan 스테이지
curl -s -X POST $MGR/projects/$BLUE_PID/plan -H "X-API-Key: $BASTION_API_KEY" > /dev/null
# execute 스테이지
curl -s -X POST $MGR/projects/$BLUE_PID/execute -H "X-API-Key: $BASTION_API_KEY" > /dev/null
...
curl -s "$MGR/projects/$PURPLE_PID/evidence/summary" \
  -H "X-API-Key: $BASTION_API_KEY" > /tmp/purple_evidence.json
...
curl -s "$MGR/projects/$PURPLE_PID/evidence/summary" \
  -H "X-API-Key: $BASTION_API_KEY" > /tmp/final_evidence.json
...
전체 시험 과정을 종합하여 Bastion completion-report를 작성하라.
...
**보너스 3.** Bastion에서 completion-report의 work_details에 포함해야 하는 것은?
...
Bastion를 활용한 자율 보안 에이전트의 설계, 구현, 운용, 학습의 전체 사이클을 경험하셨습니다.
...
## 실제 사례 (WitFoo Precinct 6 — 기말고사 자율 Purple Team)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *기말: 자율 Purple Team (Red + Blue 통합) 구축* 학습 항목 매칭.

### Purple Team = "Red + Blue 의 자율 협업으로 보안 자기 강화"

Purple Team 은 — Red 자동 공격 + Blue 자동 방어 + *학습 결과를 공유* 해 양쪽 모두 자기 강화. dataset 환경에서 — Red 가 392 사례 학습 후 새 변형 공격 생성 → Blue 가 차단 시도 → 차단 실패 사례를 *Blue 의 RL 학습* 으로 강화. 사이클 반복.

```mermaid
graph LR
    DS["dataset 392 사례"]
    RED["Red Agent<br/>공격 생성"]
    BLUE["Blue Agent<br/>방어"]
    SHARE["학습 결과 공유"]
...
| KPI | 만점 임계 |
|---|---|
| Red 공격 생성 다양성 | 최소 5 종 변형 |
| Blue 차단율 (초기) | ≥85% |
| Blue 차단율 (RL 학습 후) | ≥95% |
| 사이클 수 (24h 내) | ≥10 cycle |

### Case 2: 만점 답안의 5축 보고서

1. Red Agent 아키텍처
2. Blue Agent 아키텍처
3. 학습 공유 메커니즘
4. dataset 392 사례 활용 방식
5. 24h 자율 운영 결과

### 이 사례에서 학생이 배워야 할 3가지

1. **Purple = Red + Blue + 학습 공유** — 양쪽 자기 강화.
2. **사이클 ≥10/24h = 만점 임계** — 자율 운영 검증.
3. **5축 보고서로 통합 정리** — 단편 시연 X.
