...
- 5분 주기 자동 관제 시스템을 Bastion로 구현한다
...
curl -s "$MGR/projects/$PID/evidence/summary" \
  -H "X-API-Key: $BASTION_API_KEY" > /tmp/blue_evidence.json
...
# plan 스테이지 전환
curl -s -X POST $MGR/projects/$CYCLE_PID/plan -H "X-API-Key: $BASTION_API_KEY" > /dev/null
# execute 스테이지 전환
curl -s -X POST $MGR/projects/$CYCLE_PID/execute -H "X-API-Key: $BASTION_API_KEY" > /dev/null
...
Bastion의 Notification API를 활용하여 Slack 알림을 설정한다.
...
```bash
# 3. Blue Agent 경보 알림 시뮬레이션 — Bastion를 통해 Slack 메시지 전송
curl -s -X POST $MGR/projects \
  -H "Content-Type: application/json" \
  -H "X-API-Key: $BASTION_API_KEY" \
  -d '{
...
# 4. 스테이지 전환
# plan 스테이지
curl -s -X POST $MGR/projects/$SLACK_PID/plan -H "X-API-Key: $BASTION_API_KEY" > /dev/null
# execute 스테이지
curl -s -X POST $MGR/projects/$SLACK_PID/execute -H "X-API-Key: $BASTION_API_KEY" > /dev/null
...
# 3. 스테이지 전환
# plan 스테이지
curl -s -X POST $MGR/projects/$FP_PID/plan -H "X-API-Key: $BASTION_API_KEY" > /dev/null
# execute 스테이지
curl -s -X POST $MGR/projects/$FP_PID/execute -H "X-API-Key: $BASTION_API_KEY" > /dev/null
