...
- Bastion의 reward 가중치(risk_penalty, speed_bonus)를 조절하여 에이전트 행동을 유도한다
...
| 0:30-1:00 | Part 2 | Bastion RL 파이프라인 | 이론+실습 |
...
## Part 2: Bastion RL 파이프라인 (0:30-1:00)
...
# 2. 스테이지 전환
# plan 스테이지
curl -s -X POST $MGR/projects/$CONS_PID/plan -H "X-API-Key: $BASTION_API_KEY" > /dev/null
# execute 스테이지
curl -s -X POST $MGR/projects/$CONS_PID/execute -H "X-API-Key: $BASTION_API_KEY" > /dev/null
...
# 6. 스테이지 전환
# plan 스테이지
curl -s -X POST $MGR/projects/$AGG_PID/plan -H "X-API-Key: $BASTION_API_KEY" > /dev/null
# execute 스테이지
curl -s -X POST $MGR/projects/$AGG_PID/execute -H "X-API-Key: $BASTION_API_KEY" > /dev/null
...
# 2. 스테이지 전환
# plan 스테이지
curl -s -X POST $MGR/projects/$MORE_PID/plan -H "X-API-Key: $BASTION_API_KEY" > /dev/null
# execute 스테이지
curl -s -X POST $MGR/projects/$MORE_PID/execute -H "X-API-Key: $BASTION_API_KEY" > /dev/null
...
# 4. 스테이지 전환
# plan 스테이지
curl -s -X POST $MGR/projects/$RL_PID/plan -H "X-API-Key: $BASTION_API_KEY" > /dev/null
# execute 스테이지
curl -s -X POST $MGR/projects/$RL_PID/execute -H "X-API-Key: $BASTION_API_KEY" > /dev/null
