...
## 실제 사례 (WitFoo Precinct 6 — Schedule과 Watcher)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *주기 작업 (Schedule) + 감시 (Watcher)* 학습 항목 매칭.

### 자율 시스템의 시간 의식 — "정해진 시간에 정해진 작업"

자율 에이전트가 *언제 무엇을 할지* 를 사람이 매번 지시할 수 없다. 대신 — *Schedule* 로 주기 작업을 정하고, *Watcher* 로 이벤트 트리거를 감지.

```mermaid
graph LR
    SCHED["Schedule<br/>매시 / 매일 / 매주"]
    WATCH["Watcher<br/>이벤트 트리거"]
    AGENT["에이전트"]
    ...
    style SCHED fill:#cce6ff
    style WATCH fill:#ffe6cc
```

### Case 1: dataset 운영의 5 주기 작업

| 주기 | 작업 | 입력 |
|---|---|---|
| 매분 | 신호 분류 | 새 dataset 신호 |
| 매시 | alert 묶음 보고 | 누적 critical |
| 매일 | KG 정리 | 392 사례 압축 |
| 매주 | playbook 갱신 | RL 학습 결과 |
| 매월 | 성능 리포트 | 30일 KPI 집계 |

### Case 2: Watcher 트리거 — 이상 패턴 감지

| 트리거 | 액션 | 학습 매핑 |
|---|---|---|
| critical 신호 발생 | 즉시 분석 | playbook 적용 |
| 5분 burst | 대응 escalation | 사람 개입 |
| baseline 이탈 | RL 재학습 | 모델 drift 대응 |

### 이 사례에서 학생이 배워야 할 3가지

1. **Schedule = 시간 기반, Watcher = 이벤트 기반** — 두 가지 결합.
2. **5 주기 작업으로 자율 운영** — 매분/시/일/주/월.
3. **Watcher 가 anomaly 의 첫 방어선** — 즉각 대응.
