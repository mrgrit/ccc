...
## 학습 목표
- Bastion 프로젝트의 6단계 생명주기를 완전히 이해한다
...
| 용어 | 영문 | 설명 | 비유 |
|------|------|------|------|
| **프로젝트** | Project | Bastion에서 하나의 작업 단위를 관리하는 컨테이너 | 업무 처리 건 |
...
## 전제 조건
- Week 01-02 완료 (Bastion API, LLM 기초)
- curl, JSON 기본 이해
- REST API 개념
...
Bastion 프로젝트는 다음 6단계를 순서대로 거친다:
...
> **실습 목적**: Bastion의 Playbook 시스템으로 보안 대응 절차를 자동화하기 위해 수행한다
>
...
2. **Evidence의 법적 가치**: Bastion evidence가 실제 보안 감사에서 증거로 인정받으려면 어떤 요건을 충족해야 하는가?
...
### 과제 1: 생명주기 완전 실습 (필수)
본인이 설계한 보안 점검 시나리오(최소 5개 task)를 Bastion 프로젝트로 생성하여 6단계 전체를 실행하라. evidence 요약과 replay 결과를 제출한다.

### 과제 2: Stage 전환 다이어그램 (필수)
Bastion 프로젝트의 State Machine을 직접 그리고, 각 전환에서 발생하는 API 호출과 제약 조건을 표로 정리하라.
...
## 실제 사례 (WitFoo Precinct 6 — Bastion 프로젝트 생명주기)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Bastion 프로젝트의 라이프사이클 (init → operate → retire)* 학습 항목 매칭.

### Bastion 프로젝트 = "특정 환경의 dataset 학습 + 운영 + 폐기"

각 학생/조직은 *자기 환경 전용 Bastion 프로젝트* 를 만든다. dataset 환경에서 — Bastion 프로젝트는 *(1) init 단계: dataset 392 사례 학습, (2) operate 단계: 일일 13K 신호 운영, (3) retire 단계: 학습 데이터 export + 다음 환경으로 transfer* 의 3 단계 라이프사이클.

```mermaid
graph LR
    INIT["init<br/>dataset 392 사례<br/>KG 초기 학습"]
    OPER["operate<br/>일일 13K 신호 운영<br/>RL 자기 개선"]
    RET["retire<br/>학습 결과 export<br/>다음 프로젝트로"]

    INIT --> OPER
    OPER --> RET
    RET -.->|새 프로젝트| INIT

    style INIT fill:#ffe6cc
    style OPER fill:#cce6ff
    style RET fill:#ccffcc
```

**그림 해석**: 라이프사이클이 *순환적* — retire 의 export 결과가 다음 프로젝트의 init 자료가 됨.

### Case 1: 프로젝트 단계별 dataset 활용

| 단계 | dataset 활용 | 산출물 |
|---|---|---|
| init | 392 사례 KG 학습 | KG node 50개 + playbook 5개 |
| operate | 13K 신호 분류 | 일일 alert + RL 학습 |
| retire | KG/playbook export | seed bundle |

**자세한 해석**:

init 단계에서 *392 사례를 KG node 50개로 압축 학습*. operate 에서 *RL 로 KG 가 진화*. retire 에서 *최종 KG 를 다른 프로젝트의 init 시드* 로 전달 — *전이 학습 (transfer learning)* 패턴.

### Case 2: seed bundle 의 transfer 효과

| 항목 | 신규 프로젝트 (시드 X) | 전이 프로젝트 (시드 O) |
|---|---|---|
| init 시간 | 2주 (392 사례 직접 학습) | 1일 (시드 import) |
| 초기 정확도 | ~70% | ~90% (이미 학습됨) |
| 운영 가능 시점 | 4주 후 | 1주 후 |

**자세한 해석**:

전이 학습으로 *4주 → 1주* 로 시간 단축. seed bundle 의 핵심 가치 — *과거 환경의 학습 결과를 새 환경에 즉시 적용*.

### 이 사례에서 학생이 배워야 할 3가지

1. **프로젝트 라이프사이클 = 3 단계 + 순환** — init/operate/retire 의 명확한 경계.
2. **각 단계의 dataset 활용 다름** — 학습/운영/export 의 명확한 분리.
3. **seed bundle = 전이 학습 자산** — 4주 → 1주 단축.
