...
## 실제 사례 (WitFoo Precinct 6 — SubAgent와 원격 실행)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *Master + SubAgent + 원격 실행 아키텍처* 학습 항목 매칭.

### Master/SubAgent 분리 — "각자의 역할 분담"

자율 시스템의 안정성을 위해 *Master AI (오케스트레이션) + SubAgent (실행)* 의 *역할 분리* 가 표준이다. dataset 환경에서 — Master 는 *전체 분석 plan* 을 짜고, 각 SubAgent 가 *원격 호스트에서 한 가지 분석* 을 실행 후 결과 보고. 단일 에이전트가 모든 일을 다 하면 *복잡도 폭증 + 실패 시 영향 큼*.

```mermaid
graph TB
    M["Master AI<br/>(plan + 조율)"]
    M --> S1["SubAgent 1<br/>signal parse"]
    M --> S2["SubAgent 2<br/>KG 검색"]
    M --> S3["SubAgent 3<br/>chain 분석"]
    M --> S4["SubAgent 4<br/>rule 생성"]
    S1 -->|결과| M
    S2 -->|결과| M
    S3 -->|결과| M
    S4 -->|결과| M
...
**그림 해석**: Master 가 *4 SubAgent 를 병렬* 호출. 4 SubAgent 동시 실행 시 — 단일 에이전트의 *4배 처리량*.

### Case 1: 단일 vs Master/Sub 분리의 정량 비교

| 항목 | 단일 에이전트 | Master/Sub 분리 |
|---|---|---|
| 신호 1건 처리 | 7.6초 | 2.5초 (병렬화) |
| 메모리 사용 | 단일 LLM context | 4 SubAgent 메모리 분산 |
| 실패 영향 | 모든 작업 정지 | SubAgent 1개만 실패 |

**자세한 해석**:

Master/Sub 분리로 *처리 시간 3배 단축* + *실패 격리*. dataset 13K 신호 운영에서 — 단일 에이전트로는 27시간, Master/Sub 으로는 9시간. 즉 *24h 운영 사이클 안에 처리 완료* 가능.

### Case 2: SubAgent 의 원격 실행 — 보안 고려사항

| 항목 | 설명 |
|---|---|
| 통신 채널 | mTLS (양방향 인증) |
| 권한 분리 | SubAgent 마다 다른 IAM |
| 결과 무결성 | hash chain |
| audit | 모든 호출 dataset 에 기록 |

**자세한 해석**:

원격 SubAgent 는 *네트워크 분리된 호스트에서 실행* 하므로 — *통신 채널 보안 + 권한 최소화* 가 필수. mTLS 로 *Master 도 SubAgent 가 인증된 것을 확인* (SubAgent 위장 공격 방어).

### 이 사례에서 학생이 배워야 할 3가지

1. **Master/Sub 분리 = 병렬화 + 실패 격리** — 처리 3배 + 안정성.
2. **mTLS + 권한 분리** — 원격 실행 보안.
3. **모든 호출 audit** — 자율 시스템의 추적 가능성.
