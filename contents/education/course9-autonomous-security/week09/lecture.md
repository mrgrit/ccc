...
- Bastion의 4계층 메모리 구조(Evidence, Task Memory, Experience, Retrieval)를 이해한다
...
| 용어 | 설명 | 예시 |
|------|------|------|
| **Evidence** | Bastion가 태스크 실행 시 자동 기록하는 증적 데이터 | stdout, stderr, exit_code, timestamp |
...
| **Manager API** | 프로젝트/실행/증적을 관리하는 중앙 API 서버 | http://localhost:9100 |
...
Evidence는 Bastion가 태스크를 실행할 때 자동으로 기록하는 원시 데이터이다.
...
실제 Bastion에서 Task Memory는 execute-plan 내부에서 자동 관리된다.
...
# 2. 스테이지 전환
# plan 스테이지
curl -s -X POST $MGR/projects/$PID2/plan -H "X-API-Key: $BASTION_API_KEY" > /dev/null
# execute 스테이지
curl -s -X POST $MGR/projects/$PID2/execute -H "X-API-Key: $BASTION_API_KEY" > /dev/null
...
# 2. 스테이지 전환
# plan 스테이지로 전환
curl -s -X POST $MGR/projects/$PID3/plan -H "X-API-Key: $BASTION_API_KEY" > /dev/null
# execute 스테이지로 전환
curl -s -X POST $MGR/projects/$PID3/execute -H "X-API-Key: $BASTION_API_KEY" > /dev/null
...
### 4.4 Bastion API를 통한 Experience 검색
...
# 2. 스테이지 전환
# plan 스테이지로 전환
curl -s -X POST $MGR/projects/$PID4/plan -H "X-API-Key: $BASTION_API_KEY" > /dev/null
# execute 스테이지로 전환
curl -s -X POST $MGR/projects/$PID4/execute -H "X-API-Key: $BASTION_API_KEY" > /dev/null
...
| step | 학습 항목 | OSS 도구 |
|------|----------|---------|
| s1 | NATS 기초 | **NATS** server + nats-py |
| s2 | RabbitMQ 비교 | **RabbitMQ** |
| s3 | Redis pub/sub | **Redis** Stack |
| s4 | Kafka (대규모) | **Apache Kafka** / Redpanda |
| s5 | Multi-agent coord | LangGraph / AutoGen |
| s6 | Service discovery | Consul / etcd |
| s7 | Health check | Prometheus + Grafana |
| s8 | 통합 multi-agent | NATS + AutoGen |

### 학생 환경 준비

```bash
# NATS (가장 가벼움 — 단일 binary)
...
# Kafka (KRaft mode — Zookeeper 불필요)
...
docker run -d -p 8500:8500 hashicorp/consul:latest agent -dev -client=0.0.0.0
```

### 핵심 — NATS (modern lightweight messaging)
...
async def main():
    ...
    js = nc.jetstream()                                   # JetStream (durable)
    
    # 2) Stream 생성 (durable)
    await js.add_stream(name="security", subjects=["security.>"])
    ...
    await js.publish("security.recon.host", json.dumps(recon_data).encode())
    
    # 4) Vuln agent — subscribing (recon 결과 받음)
    async def vuln_handler(msg):
        data = json.loads(msg.data.decode())
        print(f"Vuln agent received: {data['host']}")
        # 자동 nuclei 실행
        results = run_nuclei(data['host'])
        # 다음 agent (exploit) 에 전달
        await js.publish("security.vuln.found", json.dumps(results).encode())
        await msg.ack()
    
    await js.subscribe("security.recon.host", cb=vuln_handler, durable="vuln-agent")
    
    # 5) Exploit agent
    async def exploit_handler(msg):
        data = json.loads(msg.data.decode())
        # 자동 sqlmap / metasploit 시도
        # 결과 → reporter 에 전달
        await msg.ack()
    
    await js.subscribe("security.vuln.found", cb=exploit_handler, durable="exploit-agent")
    ...
    await js.subscribe("security.exploit.success", cb=report_handler, durable="reporter")
...
```python
# AutoGen + NATS 통합 (분산 multi-agent)
...
class DistributedAgent:
    def __init__(self, name, role):
        self.name = name
        self.role = role
        self.agent = AssistantAgent(name, system_message=f"You are a {role} specialist")
    
    async def listen(self, nats_url, subject):
        nc = await nats.connect(nats_url)
        js = nc.jetstream()
        
        async def handler(msg):
            request = json.loads(msg.data.decode())
            response = self.agent.generate_reply(messages=[{"role":"user","content":request["task"]}])
            await js.publish(f"{subject}.response", json.dumps(response).encode())
            await msg.ack()
        
        await js.subscribe(f"{subject}.request", cb=handler, durable=self.name)

# 시작 (각 agent 가 자체 NATS subject 구독)
recon_agent = DistributedAgent("recon", "reconnaissance")
exploit_agent = DistributedAgent("exploit", "exploitation")
asyncio.run(asyncio.gather(
    recon_agent.listen("nats://localhost", "security.recon"),
    exploit_agent.listen("nats://localhost", "security.exploit"),
))
```
...
curl -X PUT http://localhost:8500/v1/agent/service/register \
    -d '{
        ...
        "Port": 8001,
        "Check": {
            "HTTP": "http://10.0.0.10:8001/health",
            "Interval": "10s"
        }
    }'

# 2) Service 발견
curl http://localhost:8500/v1/catalog/service/recon-agent
# 출력: 모든 healthy recon-agent 인스턴스

# 3) 자동 load balance
RECON=$(curl -s http://localhost:8500/v1/catalog/service/recon-agent | jq -r '.[0].ServiceAddress')
curl http://$RECON:8001/scan
```

학생은 본 9주차에서 **NATS + RabbitMQ + Redis + Kafka + Consul + AutoGen** 6 도구로 분산 multi-agent 의 4 패턴 (pub-sub / queue / stream / discovery) 통합 운영을 익힌다.
