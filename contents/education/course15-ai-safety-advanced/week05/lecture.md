...
# curl -s -X POST http://localhost:9100/projects/{id}/plan -H "X-API-Key: ccc-api-key-2026"
# curl -s -X POST http://localhost:9100/projects/{id}/execute -H "X-API-Key: ccc-api-key-2026"
```
...
## 실제 사례 (WitFoo Precinct 6 — AI Red Team)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0)
> 본 lecture *AI Red Team* 학습 항목 매칭.

### AI Red Team 의 dataset 흔적 — "Garak + PyRIT"

dataset 의 정상 운영에서 *Garak + PyRIT* 신호의 baseline 을 알아두면, *AI Red Team* 시도 시 발생하는 anomaly 를 정량으로 탐지할 수 있다. 핵심 정량 지표는 — 7 카테고리 차단율.

```mermaid
graph LR
    SCENE["AI Red Team 시나리오"]
    TRACE["dataset 흔적<br/>Garak + PyRIT"]
    DETECT["탐지 / 분석"]
...
|---|---|
| 핵심 신호 | Garak + PyRIT |
| 정량 baseline | 7 카테고리 차단율 |
| 학습 매핑 | 자동 Red Team |

**자세한 해석**: 자동 Red Team. 이 차이를 정량으로 측정해야 *공격 시도와 정상 운영의 구분* 이 가능. 학생이 baseline 숫자를 외워두면 — 운영 환경에서 anomaly 를 즉시 탐지할 수 있다.

### Case 2: 실전 적용 시나리오

| 단계 | dataset 활용 |
|---|---|
| 시도 식별 | Garak + PyRIT 의 spike |
...
1. **AI Red Team = Garak + PyRIT 의 anomaly** — 정량 신호로 탐지.
2. **baseline 숫자 외우기** — 7 카테고리 차단율.
