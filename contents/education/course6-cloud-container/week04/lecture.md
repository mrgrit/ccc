...
## 부록: 학습 OSS 도구 매트릭스 (Course6 Cloud-Container — Week 04 Docker 심화)

### Docker 심화 보안 도구

| 영역 | 도구 |
|------|------|
| Daemon 보안 | docker-bench-security / Aqua Daemon Mode |
| Capability 제거 | docker run --cap-drop / podman --cap-drop ALL |
| Seccomp | docker default seccomp / custom profile |
| AppArmor / SELinux | docker default apparmor / custom |
| User namespace | dockerd userns-remap / podman rootless |
| Read-only fs | --read-only + tmpfs |
| Resource 제한 | --memory --cpus / cgroups v2 |
| 네트워크 격리 | Docker user-defined network / Calico / Cilium |
| 이미지 검증 | Docker Content Trust / cosign / Notary |
| 런타임 보안 | Falco / Tracee / Tetragon (eBPF) |

### 핵심 — Docker 보안 옵션 모음

```bash
# 1) 권한 최소화 — capability 제거
docker run --rm --cap-drop=ALL --cap-add=NET_BIND_SERVICE alpine ping example.com
...
# 3) 사용자 namespace (root → unprivileged)
docker run --rm --user 1000:1000 alpine id
# uid=1000 gid=1000

# 4) Seccomp 기본 + custom profile
docker run --rm --security-opt seccomp=unconfined alpine     # 비추천
docker run --rm --security-opt seccomp=/path/to/profile.json alpine

# 5) AppArmor 프로파일
docker run --rm --security-opt apparmor=docker-default alpine

# 6) Resource 제한 (DoS 방어)
docker run --rm --memory=256m --cpus=0.5 --pids-limit=100 alpine

# 7) 네트워크 격리 (default bridge → user-defined)
docker network create --internal isolated-net                # 외부 통신 차단
docker run --network=isolated-net --name backend mysql:8

# 8) Docker Content Trust (이미지 서명 강제)
export DOCKER_CONTENT_TRUST=1
docker pull alpine:3.18                                       # 서명 없으면 거부
...
# Phase 2: 실행 중 컨테이너 자동 점검
docker ps -q | xargs -I{} docker inspect {} \
  | jq '{Name: .[].Name, Privileged: .[].HostConfig.Privileged, ReadOnly: .[].HostConfig.ReadonlyRootfs, CapAdd: .[].HostConfig.CapAdd}' \
  > /tmp/container-audit.json

# Phase 3: Privileged 컨테이너 식별 (경고)
docker ps -q | xargs -I{} docker inspect {} \
  | jq -r 'select(.[0].HostConfig.Privileged == true) | .[0].Name'

# Phase 4: cap-add 목록 (없어야 함)
docker ps -q | xargs -I{} docker inspect {} \
  | jq -r '.[0].HostConfig.CapAdd // []'

# Phase 5: 런타임 alert (Falco)
sudo journalctl -u falco -f | grep -E 'CRITICAL|HIGH'
```

### Falco rules 예 (컨테이너 보안)

```yaml
# /etc/falco/rules.local.yaml
- rule: Shell in Container
  desc: notice shell in container
  condition: container.id != host and (proc.name = bash or proc.name = sh)
  output: "Shell spawned in container (user=%user.name container=%container.name)"
  priority: WARNING

- rule: Write below /etc
  desc: write to /etc not allowed
  condition: container.id != host and fd.directory startswith /etc and evt.is_io_write=true
  output: "Write below /etc (file=%fd.name container=%container.name)"
  priority: ERROR
```

학생은 본 4주차에서 **Docker 보안 옵션 (cap-drop / read-only / userns / seccomp / apparmor) + docker-bench-security + Falco** 통합 흐름으로 컨테이너 권한 최소화·격리·런타임 모니터링의 3 축을 직접 구축한다.
