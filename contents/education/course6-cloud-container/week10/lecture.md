...
## 부록: 학습 OSS 도구 매트릭스 (Course6 Cloud-Container — Week 10 CI/CD 보안)

### CI/CD 파이프라인 보안 OSS 도구

| 영역 | OSS 도구 |
|------|---------|
| Secret 검색 | **gitleaks** / trufflehog / detect-secrets / git-secrets |
| Dependency | **Trivy** / Grype / OWASP Dependency-Check / Snyk-cli (free) |
| SAST | **SonarQube CE** / Semgrep / CodeQL / Bandit / Brakeman |
| DAST | **OWASP ZAP** / Nikto / nuclei |
| Container | Trivy / Grype / dockle / hadolint (week03 재사용) |
| IaC | **checkov** / tfsec / kics / terrascan / Snyk IaC |
| SBOM | **syft** / cyclonedx-cli |
| 서명 | **cosign** / sigstore + slsa-github-generator |
| GitOps | **Argo CD** / Flux CD |

### 핵심 — gitleaks + trufflehog (secret 누출 방지)
...
# 사용
gitleaks detect --source /repo --redact -v
gitleaks detect --no-git --source /code         # git 없는 코드
...
# slsa-github-generator (서명 자동화 GitHub Actions)
# .github/workflows/build.yml 에 통합
```

### 핵심 사용법

```bash
# 1) Semgrep — 빠른 SAST (수만 룰)
semgrep --config=auto /code
semgrep --config=p/owasp-top-ten /code
semgrep --config=p/secrets /code                 # secret 도 잡음

# 2) Bandit (Python)
bandit -r /code

# 3) Trivy fs (다목적 스캔)
trivy fs --scanners vuln,secret,config /code

# 4) checkov (IaC 다중 형식)
checkov -d /terraform/
checkov -d /k8s/ --framework kubernetes
checkov -d /helm/ --framework helm
checkov -d /Dockerfile

# 5) tfsec (Terraform 전용 — 빠름)
tfsec /terraform/

# 6) kics (Checkmarx OSS)
kics scan -p /terraform -o /tmp/kics-report

# 7) OWASP ZAP (DAST)
docker run -v $(pwd):/zap/wrk owasp/zap2docker-stable zap-baseline.py \
  -t http://target.com -r /zap/wrk/zap-report.html

# 8) cosign (이미지 서명 + 검증)
cosign generate-key-pair
cosign sign --key cosign.key registry/myimg:v1
cosign verify --key cosign.pub registry/myimg:v1

# 9) syft (SBOM 생성)
syft <image> -o spdx-json > sbom.json
syft scan dir:/code -o cyclonedx-json
```

### CI/CD 보안 흐름 (shift-left 전체)
...
dast:
  script:
    - docker run -v $PWD:/zap/wrk owasp/zap2docker-stable zap-baseline.py -t $URL

deploy:
  # Argo CD GitOps — Git push → 자동 배포
```
...
학생은 본 10주차에서 **gitleaks + Semgrep + Trivy + checkov + cosign + ZAP + Argo CD** 7 도구로 CI/CD 의 7 단계 보안 게이트를 OSS 만으로 구축한다.
