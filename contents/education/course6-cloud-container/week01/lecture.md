...
## 실제 사례 (WitFoo Precinct 6 — 공유 책임 모델 매핑)

> 출처: WitFoo Precinct 6 Cybersecurity Dataset (Apache 2.0, 2.07M signals)
> Sanitized: RFC5737 TEST-NET / ORG-NNNN / HOST-NNNN.
> 본 lecture *공유 책임 모델 + 컨테이너/클라우드 개론* 학습 항목 매칭.

### Dataset 으로 보는 "고객 책임 영역" 의 규모

dataset 에서 customer (사용자) 가 직접 통제해야 하는 cloud control-plane 활동만 추려보면 — **174,293 건의 Describe\* API 호출** + **9,340 건의 AssumeRole** + **39,639 건의 GetLogEvents** + **540 건의 ListContainerInstances** 가 단일 partition 에 기록된다. 모두 **고객이 발급한 IAM 자격증명** 으로 발생했고, 클라우드 업체는 단지 호출을 *수신·로깅* 했을 뿐이다.

```mermaid
graph TD
    subgraph CUST["고객 책임 (Customer)"]
        IAM[AssumeRole<br/>9,340건]
        DESC[Describe* API<br/>174,293건]
        LOG[GetLogEvents<br/>39,639건]
        ECS[ListContainerInstances<br/>540건]
    end
    subgraph PROV["업체 책임 (Provider)"]
        TRAIL[CloudTrail 수집<br/>인프라]
        S3[S3 저장]
        IDP[STS 발급]
    end
    IAM -->|호출 → 로깅| TRAIL
...
**해석**: 그림의 빨간 영역(고객) 이 본 dataset 의 cloud signal 99% 를 차지한다. *공유* 라는 단어는 50:50 분담을 의미하지 않으며 — 운영 책임은 압도적으로 고객에게 있다.

### Case 1: AssumeRole burst — IAM 권한 위임 hot-path

dataset 9,340 건의 `AssumeRole` 호출 중 STS 토큰을 발급받은 caller 의 분포를 보면:

| 항목 | 값 |
|---|---|
| message_type | `AssumeRole` |
| 총 호출 수 | 9,340 |
| 평균 호출 간격 | ~3.2초 (24h 기준) |
| 학습 매핑 | 공유 책임 모델 §2.1 — **IAM 은 항상 고객 책임** |

**해석**: 위 9,340 건이 모두 정상 운영이라 가정해도, 단 1 건이라도 노출된 access key 로 발급되면 그 시점부터 **클라우드 전체가 그 key 의 노예** 가 된다. Capital One (2019, lecture §2.3) 사고 패턴과 동일한 attack surface.

### Case 2: ListContainerInstances — ECS 운영 흔적

| 항목 | 값 |
|---|---|
| message_type | `ListContainerInstances` |
| 총 호출 수 | 540 |
| 의미 | ECS 클러스터 내 활성 컨테이너 호스트 enumeration |
| 학습 매핑 | 컨테이너/클라우드 개론 §3 — Docker 위 ECS 추상화 layer |

**해석**: 540 건은 적은 수치 같지만, **각 호출은 클러스터 전체 컨테이너 토폴로지** 를 노출시킨다. lecture 5.3 의 `docker ps` 와 동일한 정보지만, *원격에서 IAM key 만으로* 얻을 수 있다는 차이가 있다 — 즉 컨테이너 보안 = host 보안 + IAM 보안 의 중첩.

**학생 액션**: lab 환경 `docker inspect` 출력과 위 ECS list 결과의 정보량을 비교하여, **어느 쪽이 더 풍부한 attack surface 를 노출시키는가** 를 보고서로 작성한다.
